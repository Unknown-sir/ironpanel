...
## دستور آپدیت از ترمینال

برای آپدیت پنل روی سرور نصب‌شده، این دستور را اجرا کنید:

```bash
sudo bash /opt/ironpanel/scripts/update_from_github.sh
```

یا اگر فقط می‌خواهید سرویس‌ها بعد از آپدیت ری‌استارت شوند:
...
- اضافه شدن بخش **محدودیت سرعت** برای اعمال سقف Mbps به تفکیک پروتکل.
- اضافه شدن بخش **Routing Rules** برای اتصال یا عدم اتصال هر پروتکل به Outboundهای تعریف‌شده.
- اصلاح Update Manager تا بعد از تکمیل واقعی آپدیت، درصد روی **100٪** قرار بگیرد و پیام تکمیل نمایش داده شود.
- اضافه شدن دستور آپدیت ترمینالی به README.
