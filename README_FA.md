...
- Speed Limit مستقیم برای هر کاربر، مستقل از انتخاب پروتکل.
- ادمین برای همه کاربران و نماینده فقط برای کاربران خودش می‌تواند Mbps را تنظیم کند.
- مقدار کاربر روی همه پروتکل‌های او اولویت دارد و Runtime بلافاصله دوباره اعمال می‌شود.
...
- ادمین اصلی می‌تواند هر کاربر را فوراً قطع/وصل کند و نماینده فقط کاربران خودش را؛ تغییر وضعیت به Runtime پروتکل‌ها اعمال می‌شود.
- دکمه حذف گروهی فقط حساب‌هایی را حذف می‌کند که آخرین تغییر وضعیت آنها واقعاً غیرفعال‌سازی خودکار به علت پایان حجم یا تاریخ بوده است؛ کاربران دستی غیرفعال‌شده حذف نمی‌شوند.
- نصب/آپدیت پروتکل‌ها health-check-first شده است: پروتکل سالم `SKIP` می‌شود، Repair هر هسته timeout دارد و OpenVPN دیگر روی DH/CRL سالم بازسازی سنگین انجام نمی‌دهد.
- OpenVPN هنگام Repair، PKI مطابق CA فعال سرور را انتخاب می‌کند و اگر DH/Certificate فعال سالم باشد آن را reuse می‌کند تا Update روی تولید DH گیر نکند.
- دکمه‌های Copy صفحه Subscription در Safari آیفون fallback هم‌زمان `execCommand` دارند تا بدون وابستگی به Clipboard API هم کپی انجام شود.

## 🆕 تغییرات v19.10.18

- نصب و Repair پروتکل‌ها قبل از پایان نصب اعتبارسنجی می‌شود؛ OpenVPN از یک PKI مرجع واحد استفاده می‌کند و WireGuard قوانین کامل Forward/NAT را می‌سازد.
- در نسخه رایگان فقط OpenVPN و Xray از نظر لایسنس قابل استفاده‌اند، اما پیش‌نیازهای سایر پروتکل‌ها آماده می‌شوند. با ثبت Plus/Pro/Admin، پروتکل‌های مجاز بدون نصب مجدد فعال، Repair و Sync می‌شوند.
- در Settings می‌توان سیاست دائمی رمز خودکار کاربران را تعیین کرد: حداقل 3 کاراکتر، فقط حرف، فقط عدد یا حرف+عدد.
...
- v19.10.19: نصب/آپدیت روی سرور موجود ابتدا Health Check می‌گیرد و پروتکل سالم را Skip می‌کند؛ انتقال PKI اوپن‌وی‌پی‌ان نیز دیگر بکاپ حجیم و مسدودکننده نمی‌سازد.
