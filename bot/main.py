...
from app.core.models import Admin, AppSetting, SalesBotCustomer, SalesBotOrder, SalesBotPlan, VpnUser
...
def _parse_tg_ids(raw):
    out = set()
    text = str(raw or '').translate(_TG_DIGITS)
    for part in text.replace('\n', ',').replace(';', ',').replace('،', ',').replace('|', ',').split(','):
        norm = _normalize_tg_id(part)
        if norm:
            out.add(norm)
    return out
...
def _admin_ids():
    # Parse both fields, not just the first non-empty value. This prevents random
    # access-denied behavior when the admin bot and sales bot use different fields.
    return _parse_tg_ids(_setting('sales_bot_admin_ids', '')) | _parse_tg_ids(_setting('telegram_chat_id', ''))


def _is_admin(tg_id):
    return _normalize_tg_id(tg_id) in _admin_ids()
...
def _owner_protocol_allowlist():
    if SALES_BOT_OWNER_ID <= 0:
        return None
    reseller = Admin.query.filter_by(id=SALES_BOT_OWNER_ID, role='sub_admin', enabled=True).first()
    if not reseller:
        return set()
    return set(reseller.allowed_protocol_list())
...
def _protocol_list(raw):
    ...
    allowed = _owner_protocol_allowlist()
...
def _protocol_text(raw):
    ...
    if not protos:
        return 'بدون پروتکل مجاز' if SALES_BOT_OWNER_ID > 0 and str(raw or '').strip() else 'همه پروتکل‌های فعال پنل'
...
def _active_default_protocols():
    ...
    if SALES_BOT_OWNER_ID <= 0 and 'xray' not in protos:
        protos.append('xray')
    filtered = _protocol_list(','.join(protos))
    if SALES_BOT_OWNER_ID > 0 and not filtered:
        raise RuntimeError('هیچ پروتکل مجازی برای ربات فروش این نماینده تعریف نشده است.')
    return filtered



def _sales_bot_clean_name_prefix(value):
    text = str(value or '').strip()
    text = re.sub(r'[^A-Za-z0-9_-]+', '-', text)
    text = re.sub(r'-{2,}', '-', text).strip('-_')
    return text[:36]

def _sales_bot_random_5digit():
    return str(10000 + secrets.randbelow(90000))

def _sales_bot_config_username(order=None, plan=None, override_prefix=None):
    """Return a unique VPN username for sales-bot generated configs.

    v19.9.22: do not use Telegram chat ID as the username base. Repeated
    purchases from the same Telegram account used to collide with existing
    users and then confuse the admin/free-config workflow. By default the
    bot now creates a random five-digit username. If
    sales_bot_config_name_prefix_enabled=1, the configured/admin prefix is
    prepended as prefix-12345.
    """
    enabled = _setting('sales_bot_config_name_prefix_enabled', '0') == '1'
    prefix = ''
    if enabled:
        prefix = _sales_bot_clean_name_prefix(override_prefix or _setting('sales_bot_config_name_prefix', '') or '')
        if not prefix and plan is not None:
            prefix = _sales_bot_clean_name_prefix(getattr(plan, 'name', '') or '')
        if not prefix:
            prefix = 'cfg'
    for _ in range(80):
        suffix = _sales_bot_random_5digit()
        username = f'{prefix}-{suffix}' if enabled else suffix
        username = username[:78]
        if not VpnUser.query.filter_by(username=username).first():
            return username
    # Very unlikely fallback if all generated values collide.
    suffix = _sales_bot_random_5digit() + secrets.token_hex(2)
    username = (f'{prefix}-{suffix}' if enabled else suffix)[:78]
    return username
...
def _create_vpn_user_for_order(order, name_prefix=None):
    ...
    # v19.9.22: random five-digit config names, optionally prefix-12345.
    username = _sales_bot_config_username(order, plan, name_prefix)
    password = secrets.token_urlsafe(10)
    expires_at = None if int(plan.days or 0) <= 0 else datetime.utcnow() + timedelta(days=int(plan.days))
    selected_protocols = _protocol_list(plan.protocols) if str(plan.protocols or '').strip() else _active_default_protocols()
    if not selected_protocols:
        raise RuntimeError('پلن انتخاب‌شده هیچ پروتکل مجازی برای این نماینده ندارد.')
    protocols = ','.join(selected_protocols)
    u = VpnUser(
        username=username,
...
def _renew_vpn_user_for_order(order):
    ...
    if str(plan.protocols or '').strip() and not plan_protocols:
        raise RuntimeError('پلن انتخاب‌شده هیچ پروتکل مجازی برای این نماینده ندارد.')
    if plan_protocols:
        # Renewals follow the selected plan exactly; do not merge old protocols.
        u.protocols = ','.join(plan_protocols)
        u.protocol_permissions = ','.join(plan_protocols)
    u.enabled = True
    db.session.commit()
    _sync_after_user_change(u)
    order.status = 'approved'
...
def _receipt_file_parts(order):
    raw = str(getattr(order, 'receipt_file_id', '') or '').strip()
    if not raw:
        return '', ''
    if raw.startswith('photo:'):
        return 'photo', raw.split(':', 1)[1]
    if raw.startswith('document:'):
        return 'document', raw.split(':', 1)[1]
    # Legacy v19.9.20 receipts stored the raw file_id. Most payment slips are photos.
    return 'photo', raw


async def _notify_admins_receipt(context, order, caption, reply_markup=None):
    kind, fid = _receipt_file_parts(order)
    for aid in _admin_ids():
        try:
            chat_id = int(aid)
            if fid and kind == 'document':
                await context.bot.send_document(chat_id=chat_id, document=fid, caption=caption[:1024], reply_markup=reply_markup)
            elif fid:
                await context.bot.send_photo(chat_id=chat_id, photo=fid, caption=caption[:1024], reply_markup=reply_markup)
            else:
                await context.bot.send_message(chat_id=chat_id, text=caption, reply_markup=reply_markup)
        except Exception as e:
            log.warning('admin receipt notify failed %s: %s', aid, e)
            try:
                await context.bot.send_message(chat_id=int(aid), text=caption, reply_markup=reply_markup)
            except Exception:
                pass


def _auto_approve_enabled():
    return _setting('sales_bot_auto_approve_enabled', '0') == '1'


def _auto_approve_minutes():
    try:
        return max(1, int(float(_setting('sales_bot_auto_approve_minutes', '0') or 0)))
    except Exception:
        return 0


async def _approve_order_for_bot(context, order, approver='admin', notify_user=True, notify_admin=True):
    if not order or order.status not in ('pending_payment', 'receipt_sent'):
        return None, None, 'سفارش قابل تأیید نیست.'
    if order.order_type == 'wallet':
        new_bal = _wallet_add(order.telegram_id, order.amount)
        order.status = 'approved'
        order.approved_at = datetime.utcnow()
        order.admin_note = f'wallet approved by {approver}'
        db.session.commit()
        text = f'کیف پول شما شارژ شد ✅\nموجودی: {_fmt_money(new_bal, order.currency)}'
        if notify_user:
            try: await context.bot.send_message(chat_id=int(order.telegram_id), text=text)
            except Exception: pass
        if notify_admin:
            await _notify_admins(context, f'سفارش کیف پول #{order.id} توسط {approver} تأیید شد. Telegram ID: {order.telegram_id}')
        return None, None, text
    if order.order_type == 'renew':
        u, pwd = _renew_vpn_user_for_order(order)
        text = f'تمدید سفارش #{order.id} تأیید شد ✅\nسرویس: {u.username}\nلینک سابسکریپشن:\n{_subscription_url(u)}'
    else:
        u, pwd = _create_vpn_user_for_order(order)
        text = f'سفارش #{order.id} تأیید شد ✅\nسرویس شما ساخته شد.\nنام کاربری: {u.username}\nرمز سرویس‌ها: {pwd}\nلینک سابسکریپشن:\n{_subscription_url(u)}'
    order.admin_note = ((order.admin_note or '') + f'\napproved by {approver}').strip()
    db.session.commit()
    if notify_user:
        await context.bot.send_message(chat_id=int(order.telegram_id), text=text)
        if 'xray' in _protocol_list(u.protocol_permissions or u.protocols):
            await _send_xray_config(context, int(order.telegram_id), u)
    if notify_admin:
        await _notify_admins(context, f'سفارش #{order.id} برای Telegram ID {order.telegram_id} توسط {approver} تأیید شد. سرویس: {u.username}')
    return u, pwd, text


async def _auto_approve_loop(application):
    import asyncio
    while True:
        try:
            ...
            with flask_app.app_context():
                if not _auto_approve_enabled():
                    continue
                minutes = _auto_approve_minutes()
                if minutes <= 0:
                    continue
                threshold = datetime.utcnow() - timedelta(minutes=minutes)
                orders = _order_query().filter_by(status='receipt_sent').filter(SalesBotOrder.updated_at <= threshold).order_by(SalesBotOrder.id.asc()).limit(20).all()
                for order in orders:
                    try:
                        await _approve_order_for_bot(application, order, approver=f'auto-after-{minutes}m', notify_user=True, notify_admin=True)
                        await _notify_admins(application, f'✅ سفارش #{order.id} مشتری {order.telegram_id} به‌صورت خودکار پس از {minutes} دقیقه تأیید شد.')
                    except Exception as exc:
                        log.exception('auto approve failed for order %s: %s', getattr(order, 'id', '?'), exc)
        except Exception as exc:
            log.exception('auto approve loop failed: %s', exc)


async def _post_init(application):
    import asyncio
    asyncio.create_task(_auto_approve_loop(application))
...
async def show_admin(q):
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton('🤖 تایید خودکار', callback_data='admin:auto_approve'), InlineKeyboardButton('📊 آمار', callback_data='admin:stats')],
...
async def _show_plans(q, xray_only=False):
    ...
    kb = [[InlineKeyboardButton(str(p.name or f'Plan {p.id}')[:64], callback_data=f'buy:{p.id}')] for p in plans]
    kb.append(back_button('home'))
    await q.edit_message_text(title, reply_markup=InlineKeyboardMarkup(kb))


def _split_telegram_text(text, limit=3900):
    text = str(text or '')
    if len(text) <= limit:
        return [text]
    chunks, buf = [], ''
...
def _xray_config_lines(body):
    ...
    for raw in str(body or '').splitlines():
        ...
        if '://' in line:
            lines.append(line)
    return lines or ([str(body).strip()] if str(body or '').strip() else [])


async def _send_xray_config(context, chat_id, u):
    try:
        body = (xray_profile_text(u) or '').strip()
        if not body:
            await context.bot.send_message(chat_id=chat_id, text='کانفیگ Xray/V2Ray خالی است. تنظیمات Xray پنل را بررسی کنید.')
            return
        meta = xray_profile_meta(u)
        text = f'{meta}\n\n📋 متن کانفیگ V2Ray/Xray:\n{body}\n\nاین متن را در v2rayNG / Hiddify / Nekoray / Sing-box یا کلاینت‌های سازگار import کنید.'
        ...
        if _setting('sales_bot_qr_enabled', '1') == '1':
            try:
                import qrcode
                for idx, cfg in enumerate(_xray_config_lines(body), start=1):
                    img = qrcode.make(cfg)
                    bio = io.BytesIO()
                    img.save(bio, format='PNG')
                    bio.seek(0)
                    bio.name = f'{u.username}-xray-qr-{idx}.png'
                    suffix = f' #{idx}' if len(_xray_config_lines(body)) > 1 else ''
                    await context.bot.send_photo(chat_id=chat_id, photo=bio, caption=f'QR Code Xray/V2Ray{suffix}: {u.username}')
            except Exception as e:
                log.warning('Xray QR delivery failed: %s', e)
                await context.bot.send_message(chat_id=chat_id, text=f'متن کانفیگ ارسال شد، اما ساخت QR Code ناموفق بود: {e}')
    except Exception as e:
...
def _service_buttons(u, source='configs'):
    rows = []
    rows.append([InlineKeyboardButton(f'🔗 لینک Subscription: {u.username}', url=_subscription_url(u))])
    if 'xray' in _protocol_list(u.protocol_permissions or u.protocols):
        rows.append([InlineKeyboardButton(f'⚡ QR + متن Xray/V2Ray: {u.username}', callback_data=f'xraycfg:{u.id}:{source}')])
    return rows
    ...
    with flask_app.app_context():
        ...
        if data == 'trial':
            ...
            if not result:
                ...
                return
            await q.edit_message_text(f'تست رایگان ساخته شد ✅\nنام کاربری: {result.username}\nرمز سرویس‌ها: {extra}\nلینک سابسکریپشن:\n{_subscription_url(result)}\n\nبرای دریافت QR Code و متن Xray/V2Ray از دکمه «دریافت کانفیگ‌ها» استفاده کنید.', reply_markup=InlineKeyboardMarkup([back_button('home')]))
            return
        ...
        if data.startswith('xraycfg:'):
            ...
            await q.edit_message_text('کانفیگ و QR Code Xray/V2Ray ارسال شد ✅', reply_markup=InlineKeyboardMarkup([back_button('configs')]))
            return
        if data.startswith('renewuser:'):
            uid = int(data.split(':', 1)[1])
            plans = _plan_query().filter_by(active=True).order_by(SalesBotPlan.sort_order, SalesBotPlan.id).all()
            kb = [[InlineKeyboardButton(str(p.name or f'Plan {p.id}')[:64], callback_data=f'renewplan:{uid}:{p.id}')] for p in plans]
        ...
        if data.startswith('admin:planproto:'):
            ...
            if preset == 'all':
                ...
                protocols = ','.join(_protocol_list('xray'))
                if not protocols:
                    await q.edit_message_text('پروتکل Xray برای این نماینده مجاز نیست.', reply_markup=main_keyboard(True))
                    return
            elif preset == 'classic':
                ...
                if not protocols:
                    await q.edit_message_text('پروتکل Xray برای این نماینده مجاز نیست.', reply_markup=main_keyboard(True))
                    return
            p = SalesBotPlan(name=state['name'], days=int(state['days']), traffic_gb=int(state['traffic_gb']), price=float(state['price']), currency=state.get('currency') or _setting('sales_bot_currency', 'IRT'), protocols=protocols, connection_limit=int(state.get('connection_limit', 1)), created_by_telegram_id=str(user.id), owner_id=_owner_value(), active=True)
            db.session.add(p)
            db.session.commit()
            set_setting(f'sales_bot_state_{user.id}', '')
        ...
        if data.startswith('admin:plan:'):
            ...
            if not p:
                ...
                return
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton('✏️ ویرایش', callback_data=f'admin:editplan:{p.id}'), InlineKeyboardButton('فعال/غیرفعال', callback_data=f'admin:toggleplan:{p.id}')],
                [InlineKeyboardButton('حذف', callback_data=f'admin:deleteplan:{p.id}'), InlineKeyboardButton('⬅️ پلن‌ها', callback_data='admin:plans')]
            ])
            await q.edit_message_text(f'ID {p.id}\n{_fmt_plan(p)}\nوضعیت: {"فعال" if p.active else "غیرفعال"}', reply_markup=kb)
            return
        if data.startswith('admin:editplan:'):
            p = _plan_query().filter_by(id=int(data.split(':')[2])).first()
            if not p:
                await q.edit_message_text('پلن پیدا نشد.')
                return
            _set_json(f'sales_bot_state_{user.id}', {'step': 'edit_plan_line', 'plan_id': p.id})
            await q.edit_message_text(
                'برای ویرایش پلن، مقادیر جدید را در یک پیام با جداکننده | ارسال کنید:\n'
                'Name | Days | TrafficGB | Price | Currency | ConnectionLimit | Protocols | SortOrder | Active\n\n'
                f'مقدار فعلی:\n{p.name} | {p.days} | {p.traffic_gb} | {p.price} | {p.currency} | {p.connection_limit} | {p.protocols} | {p.sort_order} | {1 if p.active else 0}'
            )
            return
        if data == 'admin:auto_approve':
            enabled = _auto_approve_enabled()
            minutes = _auto_approve_minutes() or 30
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton('✅ فعال', callback_data='admin:auto_approve:on'), InlineKeyboardButton('❌ غیرفعال', callback_data='admin:auto_approve:off')],
                [InlineKeyboardButton('⏱ تنظیم زمان', callback_data='admin:auto_approve:set')],
                [InlineKeyboardButton('⬅️ بازگشت', callback_data='admin')]
            ])
            await q.edit_message_text(f'تأیید خودکار سفارش: {"فعال" if enabled else "غیرفعال"}\nزمان انتظار پس از ارسال رسید: {minutes} دقیقه', reply_markup=kb)
            return
        if data == 'admin:auto_approve:on':
            set_setting('sales_bot_auto_approve_enabled', '1')
            await q.edit_message_text('تأیید خودکار فعال شد.', reply_markup=InlineKeyboardMarkup([back_button('admin:auto_approve')]))
            return
        if data == 'admin:auto_approve:off':
            set_setting('sales_bot_auto_approve_enabled', '0')
            await q.edit_message_text('تأیید خودکار غیرفعال شد.', reply_markup=InlineKeyboardMarkup([back_button('admin:auto_approve')]))
            return
        if data == 'admin:auto_approve:set':
            _set_json(f'sales_bot_state_{user.id}', {'step': 'auto_approve_minutes'})
            await q.edit_message_text('زمان انتظار تأیید خودکار را به دقیقه ارسال کنید. نمونه: 30')
            return
        ...
        if data == 'admin:free_config':
            ...
            await q.edit_message_text('برای ساخت رایگان ارسال کنید: TelegramID PlanID Count [Name]\nمثال: 123456789 2 1 VIP\nاگر Name وارد شود و گزینه افزودن نام فعال باشد، کانفیگ به شکل VIP-12345 ساخته می‌شود.')
            return
        ...
        if data.startswith('approve:') or data.startswith('reject:'):
            ...
            if data.startswith('reject:'):
                ...
                return
            try:
                u, pwd, text = await _approve_order_for_bot(context, o, approver=str(user.id), notify_user=True, notify_admin=False)
...
async def receipt_handler(update, context):
    with flask_app.app_context():
        ...
        # v19.9.22: payment/receipt messages must take priority over stale admin
        # states such as free_config. Otherwise a buyer who is also an admin can
        # see the misleading TelegramID PlanID Count error while trying to buy.
        pending_row = AppSetting.query.filter_by(key=_bot_key(f'sales_bot_pending_order_{user.id}')).first()
        pending_order = None
        if pending_row and pending_row.value:
            try:
                pending_order = _order_query().filter_by(id=int(pending_row.value)).first()
            except Exception:
                pending_order = None
        if pending_order and pending_order.status in ('pending_payment', 'receipt_sent'):
            if state and state.get('step') == 'free_config':
                set_setting(f'sales_bot_state_{user.id}', '')
            file_id = ''
            if update.message.photo:
                ...
                pending_order.receipt_note = (pending_order.receipt_note or '') + '\n' + (update.message.text or '')
            if file_id:
                pending_order.receipt_file_id = file_id
            pending_order.status = 'receipt_sent'
            db.session.commit()
            plan = (_plan_query().filter_by(id=pending_order.plan_id).first() if pending_order.plan_id else None)
            kb = InlineKeyboardMarkup([[InlineKeyboardButton('✅ تأیید پرداخت', callback_data=f'approve:{pending_order.id}'), InlineKeyboardButton('❌ رد پرداخت', callback_data=f'reject:{pending_order.id}')]])
            await update.message.reply_text('رسید شما ثبت شد. پس از بررسی مدیر، نتیجه اعلام می‌شود.')
            caption = f'رسید جدید برای سفارش #{pending_order.id}\nTelegram ID: {pending_order.telegram_id}\nپلن: {plan.name if plan else "-"}\nنوع کانفیگ: {_protocol_text(plan.protocols if plan else "")}\nمبلغ: {_fmt_money(pending_order.amount, pending_order.currency)}'
            if pending_order.receipt_note:
                caption += f'\nتوضیح کاربر: {pending_order.receipt_note.strip()[-500:]}'
            await _notify_admins_receipt(context, pending_order, caption, kb)
            return
        if state:
            step = state.get('step')
            if step == 'plan_name':
            ...
            if step == 'plan_protocols_custom':
                protocols = ','.join(_protocol_list(text))
                if not protocols:
                    await update.message.reply_text('هیچ‌کدام از پروتکل‌های واردشده برای این نماینده مجاز نیست.')
                    return
                p = SalesBotPlan(name=state['name'], days=int(state['days']), traffic_gb=int(state['traffic_gb']), price=float(state['price']), currency=state.get('currency') or _setting('sales_bot_currency', 'IRT'), protocols=protocols, connection_limit=int(state.get('connection_limit', 1)), created_by_telegram_id=str(user.id), owner_id=_owner_value(), active=True)
                db.session.add(p)
                db.session.commit()
                ...
                return
            if step == 'edit_plan_line' and _is_admin(user.id):
                p = _plan_query().filter_by(id=int(state.get('plan_id') or 0)).first()
                if not p:
                    set_setting(f'sales_bot_state_{user.id}', '')
                    await update.message.reply_text('پلن پیدا نشد.', reply_markup=main_keyboard(True))
                    return
                parts = [x.strip() for x in text.split('|')]
                if len(parts) < 9:
                    await update.message.reply_text('فرمت درست:\nName | Days | TrafficGB | Price | Currency | ConnectionLimit | Protocols | SortOrder | Active')
                    return
                p.name = parts[0] or p.name
                p.days = int(parts[1] or 0)
                p.traffic_gb = int(parts[2] or 0)
                p.price = float(parts[3] or 0)
                p.currency = parts[4] or p.currency or _setting('sales_bot_currency', 'IRT')
                p.connection_limit = max(0, int(parts[5] or 1))
                allowed_edit_protocols = _protocol_list(parts[6])
                if not allowed_edit_protocols:
                    await update.message.reply_text('هیچ‌کدام از پروتکل‌های واردشده برای این نماینده مجاز نیست.')
                    return
                p.protocols = ','.join(allowed_edit_protocols)
                p.sort_order = int(parts[7] or 0)
                p.active = str(parts[8]).strip().lower() in ('1','true','on','yes','فعال')
                db.session.commit()
                set_setting(f'sales_bot_state_{user.id}', '')
                await update.message.reply_text(f'پلن ویرایش شد ✅\nID {p.id}\n{_fmt_plan(p)}', reply_markup=main_keyboard(True))
                return
            if step == 'auto_approve_minutes' and _is_admin(user.id):
                minutes = max(1, int(float(text or 0)))
                set_setting('sales_bot_auto_approve_minutes', str(minutes))
                set_setting(f'sales_bot_state_{user.id}', '')
                await update.message.reply_text(f'زمان تأیید خودکار روی {minutes} دقیقه ذخیره شد.', reply_markup=main_keyboard(True))
                return
            if step == 'trial_days':
            ...
            if step == 'free_config' and _is_admin(user.id):
                ...
                for _ in range(max(1,min(count,20))):
                    ...
                    db.session.add(o); db.session.commit()
                    u,pwd = _create_vpn_user_for_order(o, custom_prefix)
