...
from app.services.provisioning import get_setting, set_setting, sync_user, user_usage_summary, user_access_status, get_public_host, active_protocols, set_user_ip_limit
...
async def callbacks(update, context):
    ...
    with flask_app.app_context():
        ...
        if data == 'admin:wallet_orders':
            orders = SalesBotOrder.query.filter_by(order_type='wallet').filter(SalesBotOrder.status.in_(['pending_payment','receipt_sent'])).order_by(SalesBotOrder.id.desc()).limit(10).all()
            if not orders:
        ...
        if state:
            ...
            if step == 'wallet_amount':
                ...
                if amount <= 0:
                    ...
                    return
                o = SalesBotOrder(telegram_id=str(user.id), customer_id=c.id, order_type='wallet', status='pending_payment', amount=amount, currency=_setting('sales_bot_currency','IRT'))
            ...
            if step == 'free_config' and _is_admin(user.id):
                ...
                plan = SalesBotPlan.query.get(pid)
                if not plan:
                    await update.message.reply_text('پلن پیدا نشد.')
                    return
                made=[]
                cust = SalesBotCustomer.query.filter_by(telegram_id=str(tg)).first() or SalesBotCustomer(telegram_id=str(tg))
                db.session.add(cust); db.session.commit()
                for _ in range(max(1,min(count,20))):
                    o = SalesBotOrder(telegram_id=str(tg), customer_id=cust.id, plan_id=plan.id, order_type='free', status='pending_payment', amount=0, currency=plan.currency, admin_note=f'free by {user.id}')
                    db.session.add(o); db.session.commit()
                    u,pwd = _create_vpn_user_for_order(o)
