...
def _sales_bot_config_username(order=None, plan=None, override_prefix=None):
    ...
    v19.9.20: do not use Telegram chat ID as the username base. Repeated
...
def _create_vpn_user_for_order(order, name_prefix=None):
    ...
    # v19.9.20: random five-digit config names, optionally prefix-12345.
...
async def receipt_handler(update, context):
    with flask_app.app_context():
        ...
        # v19.9.20: payment/receipt messages must take priority over stale admin
        ...
        if pending_order and pending_order.status in ('pending_payment', 'receipt_sent'):
            ...
            await _notify_admins(context, f'رسید جدید برای سفارش #{pending_order.id}\nTelegram ID: {pending_order.telegram_id}\nپلن: {plan.name if plan else "-"}\nنوع کانفیگ: {_protocol_text(plan.protocols if plan else "")}\nمبلغ: {_fmt_money(pending_order.amount, pending_order.currency)}', kb)
            return
        if state:
            step = state.get('step')
            if step == 'plan_name':
