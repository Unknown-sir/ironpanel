...
from app.services.admin_bot import (
    admin_bot_settings, admin_bot_admin_ids, normalize_telegram_id, admin_bot_report_text,
...
def _is_admin(tg_id):
    return normalize_telegram_id(tg_id) in admin_bot_admin_ids()




def _admin_welcome_text():
    return (get_setting('admin_bot_welcome_text', '') or 'به ربات مدیریتی IronPanel خوش آمدید ✅\nاز دکمه‌های زیر برای مشاهده کاربران آنلاین، اطلاعات کاربران، گزارش پنل و دریافت بکاپ استفاده کنید.').strip()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    with flask_app.app_context():
        if not _is_admin(update.effective_user.id):
            # v19.9.28: avoid spamming users/admins with misleading access-denied
            # messages when they accidentally open the wrong bot or tap an old button.
            log.info('ignored unauthorized admin-bot /start from telegram_id=%s', update.effective_user.id)
            return
        await update.effective_message.reply_text(_admin_welcome_text(), reply_markup=_kb())
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with flask_app.app_context():
        if not _is_admin(q.from_user.id):
            # Silently answer unauthorized callbacks; editing the message can confuse
            # legitimate admins if Telegram IDs were pasted with localized digits.
            await q.answer('دسترسی این دکمه برای شما فعال نیست.', show_alert=False)
            log.info('ignored unauthorized admin-bot callback from telegram_id=%s data=%s', q.from_user.id, q.data)
            return
        data = q.data or ''
        if data in ('ab:home', 'ab:refresh'):
            await q.edit_message_text(_admin_welcome_text(), reply_markup=_kb())
            return
        if data == 'ab:online':
