...
def ensure_columns(cur):
    ...
    if 'disabled_reason' not in cols:
        cur.execute("ALTER TABLE vpn_user ADD COLUMN disabled_reason VARCHAR(48) DEFAULT ''")
    try:
        cur.execute('PRAGMA table_info(admin)')
        admin_cols = {r[1] for r in cur.fetchall()}
        if admin_cols and 'reseller_used_bytes' not in admin_cols:
            cur.execute('ALTER TABLE admin ADD COLUMN reseller_used_bytes BIGINT DEFAULT 0')
        if admin_cols and 'disabled_reason' not in admin_cols:
            cur.execute("ALTER TABLE admin ADD COLUMN disabled_reason VARCHAR(40) DEFAULT ''")
    except Exception:
        pass
...
def main():
    ...
    cur = con.cursor()
    ensure_columns(cur)
    cur.execute('SELECT id, username, COALESCE(used_upload_bytes, COALESCE(used_upload_mb,0)*1048576,0), COALESCE(used_download_bytes, COALESCE(used_download_mb,0)*1048576,0), COALESCE(data_limit_mb,0), COALESCE(enabled,1), COALESCE(owner_id,0) FROM vpn_user')
    user = None
    ...
    if not user:
        con.close(); return 0
    uid, username, up0, down0, limit_mb, enabled, owner_id = user
    ...
    limit_bytes = int(limit_mb or 0) * 1048576
    charged_total_after = effective_usage_bytes(cur, new_up + new_down)
    will_disable = limit_bytes > 0 and charged_total_after >= limit_bytes
    if will_disable:
        factor = traffic_multiplier_factor(cur)
        raw_cap = max(1, int(limit_bytes / max(factor, 0.01)))
        total = max(0, new_up + new_down)
        if total > raw_cap:
            adjusted_up = min(new_up, int(raw_cap * (new_up / total))) if total else 0
            new_up = adjusted_up
            new_down = max(0, raw_cap - adjusted_up)
    ...
    delta_total = max(0, d_rx) + max(0, d_tx)
    if int(owner_id or 0) and delta_total > 0:
        try:
            cur.execute('UPDATE admin SET reseller_used_bytes=COALESCE(reseller_used_bytes,0)+? WHERE id=? AND role=\'sub_admin\'', (delta_total, int(owner_id)))
            cur.execute('SELECT username, COALESCE(traffic_quota_gb,0), COALESCE(reseller_used_bytes,0) FROM admin WHERE id=?', (int(owner_id),))
            rr = cur.fetchone()
            if rr and int(rr[1] or 0) > 0 and int(rr[2] or 0) >= int(rr[1]) * 1073741824:
                cur.execute("UPDATE admin SET enabled=0, disabled_reason='traffic_quota' WHERE id=?", (int(owner_id),))
                cur.execute("UPDATE vpn_user SET enabled=0, disabled_reason='reseller_traffic_quota' WHERE owner_id=? AND COALESCE(enabled,1)=1", (int(owner_id),))
                cur.execute('INSERT INTO activity_log(actor,action,target,details,created_at) VALUES(?,?,?,?,?)', ('system','reseller_quota_exhausted',rr[0],f'quota_gb={rr[1]}; used_bytes={rr[2]}',datetime.datetime.utcnow().isoformat()))
        except Exception:
            pass
    ...
    if int(limit_mb or 0) > 0 and (will_disable or charged_total >= int(limit_mb) * 1048576) and int(enabled or 0) == 1:
        cur.execute("UPDATE vpn_user SET enabled=0, disabled_reason='traffic_limit' WHERE id=?", (uid,))
