...
def main():
    ...
    if int(limit_mb or 0) > 0 and (new_up + new_down) >= int(limit_mb) * 1048576 and int(enabled or 0) == 1:
        cur.execute('UPDATE vpn_user SET enabled=0 WHERE id=?', (uid,))
        cur.execute('INSERT INTO activity_log(actor,action,target,details,created_at) VALUES(?,?,?,?,?)', ('system','auto_disable_user',username,'traffic_limit',datetime.datetime.utcnow().isoformat()))
