...
if [[ -x "$APP_DIR/scripts/install_protocol_prerequisites.sh" ]]; then
  APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" bash "$APP_DIR/scripts/install_protocol_prerequisites.sh" --runtime-binaries || true
fi
...
if ! command -v xray >/dev/null 2>&1 && [[ ! -x /usr/local/bin/xray ]]; then
  echo '[IronPanel] Xray binary is missing after installation attempts' >&2
  exit 20
fi
...
set +e
if [[ -x "$APP_DIR/scripts/upgrade_db_safe.sh" ]]; then bash "$APP_DIR/scripts/upgrade_db_safe.sh" >/tmp/ironpanel-xray-upgrade-db.log 2>&1; else "$FLASK" --app run.py upgrade-db >/tmp/ironpanel-xray-upgrade-db.log 2>&1; fi
UPGRADE_RC=$?
set -e
...
# Open active Xray TCP ports in the local firewall as a best-effort repair step.
XRAY_PORTS=$("$PY" - <<'PYPORTS' 2>/dev/null || true
from app import create_app
from app.services.xray import _configured_xray_ports
app=create_app()
with app.app_context():
    print(' '.join(str(p) for p in _configured_xray_ports()))
PYPORTS
)
for P in $XRAY_PORTS; do
  if command -v ufw >/dev/null 2>&1 && ufw status | grep -qi active; then ufw allow "${P}/tcp" >/dev/null 2>&1 || true; fi
