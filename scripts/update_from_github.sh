...
echo "Mode: fast source + database + systemd sync; critical ocserv/auth/node reconciliation runs automatically"
echo "Repo: $REPO_URL"
echo "Branch: $BRANCH"
echo "App: $APP_DIR"
...
if [[ "$UPDATE_BACKUP" == "1" ]]; then
  mkdir -p "$BACKUP_DIR"
  if [[ -d "$APP_DIR" ]]; then
    echo "IRONPANEL_PROGRESS 10% - creating optional source backup"
  ...
  if [[ -d "$ETC_DIR" ]]; then
    echo "IRONPANEL_PROGRESS 15% - creating optional config backup"
    tar -C "$(dirname "$ETC_DIR")" -czf "$BACKUP_DIR/ironpanel-etc-$STAMP.tar.gz" "$(basename "$ETC_DIR")" >/dev/null 2>&1 || true
  fi
else
  echo "IRONPANEL_PROGRESS 15% - automatic update backup skipped (set IRONPANEL_UPDATE_BACKUP=1 to enable)"
fi
...
# Full protocol reinstall is intentionally skipped here; upgrade.sh still performs
# critical ocserv/auth repair and queues a complete node runtime synchronization.
