...
echo "Mode: source + dependency + database + systemd sync; VPN repairs are manual/non-blocking"
echo "Repo: $REPO_URL"
echo "Branch: $BRANCH"
echo "App: $APP_DIR"
...
echo "Existing Git checkout detected; updating in-place."
  ...
  IRONPANEL_SKIP_CORE_REPAIR=1 IRONPANEL_DEFER_RESTART=1 IRONPANEL_FULL_SERVICE_SYNC=0 bash "$APP_DIR/upgrade.sh" --github-fast
else
...
echo "Non-Git install detected; cloning fresh release to temporary directory."
  ...
  cd "$WORK/ironpanel"
  IRONPANEL_SKIP_CORE_REPAIR=1 IRONPANEL_DEFER_RESTART=1 IRONPANEL_FULL_SERVICE_SYNC=0 bash upgrade.sh --github-fast
fi
...
# Do not run heavy protocol repairs during terminal update; they can block on broken packages/cores.
# Admin can run manual repair from Health & Repair after the source update is complete.
