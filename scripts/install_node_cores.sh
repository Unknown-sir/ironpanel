...
if [[ -x /opt/ironpanel-node/scripts/install_protocol_prerequisites.sh ]]; then
  log "Installing shared protocol prerequisites"
  APP_DIR=/opt/ironpanel-node ETC_DIR=/etc/ironpanel-node LOG="$LOG" bash /opt/ironpanel-node/scripts/install_protocol_prerequisites.sh --packages || true
  if has_proto openvpn; then
    APP_DIR=/opt/ironpanel-node ETC_DIR=/etc/ironpanel-node LOG="$LOG" bash /opt/ironpanel-node/scripts/install_protocol_prerequisites.sh --ensure-openvpn || { log "ERROR: OpenVPN PKI preparation failed"; exit 20; }
  fi
fi
BASE_PKGS=(curl ca-certificates iproute2 iptables openssl net-tools cron iptables-persistent unzip jq gnupg)
PKGS=("${BASE_PKGS[@]}")
...
missing=()
if has_proto openvpn; then
  command -v openvpn >/dev/null 2>&1 || missing+=(openvpn-binary)
  for f in /etc/openvpn/server/ca.crt /etc/openvpn/server/server.crt /etc/openvpn/server/server.key /etc/openvpn/server/dh.pem /etc/openvpn/server/tls-crypt.key; do [[ -s "$f" ]] || missing+=("openvpn-file:$f"); done
fi
