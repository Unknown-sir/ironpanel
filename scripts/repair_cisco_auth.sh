...
run_sync
log 'Repairing ocserv runtime with saved panel port settings'
if [[ -x "$APP_DIR/scripts/repair_ocserv.sh" ]]; then
  timeout 180 bash "$APP_DIR/scripts/repair_ocserv.sh"
fi
run_sync
...
# v19.9.20: keep Cisco/AnyConnect independent from session hook failures.
