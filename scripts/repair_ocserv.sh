...
flock -w 15 9 || { echo '[IronPanel] another ocserv repair is already running' >&2; exit 3; }

ETC_DIR=${ETC_DIR:-/etc/ironpanel}
DATABASE_URL_VALUE=${DATABASE_URL:-}
DB_FILE=${IRONPANEL_DB:-${DATABASE_URL_VALUE#sqlite:///}}
[[ -n "$DB_FILE" && "$DB_FILE" != "$DATABASE_URL_VALUE" ]] || DB_FILE="$ETC_DIR/ironpanel.db"

db_setting(){
  local key="$1" default="${2:-}"
  if [[ -s "$DB_FILE" ]] && command -v sqlite3 >/dev/null 2>&1; then
    local value
    value=$(sqlite3 "$DB_FILE" "select value from app_setting where key='$key' limit 1;" 2>/dev/null | head -1 || true)
    if [[ -n "$value" ]]; then printf '%s' "$value"; return 0; fi
  fi
  printf '%s' "$default"
}

# Read the saved panel settings directly from SQLite when the repair is started
# by Health Doctor or by hand. Previous builds only honored env vars; therefore
# a custom Cisco/OpenConnect port such as 1195 could be overwritten back to 8445
# and ocserv would not listen on the port shown in the panel/subscription.
DB_OCSERV_TCP=$(db_setting port_ocserv_tcp 8445)
DB_OCSERV_UDP=$(db_setting port_ocserv_udp "$DB_OCSERV_TCP")
DB_OCSERV_TRANSPORT=$(db_setting ocserv_transport tcp_udp)
DB_PUBLIC_HOST=$(db_setting tunnel_host "")
[[ -n "$DB_PUBLIC_HOST" ]] || DB_PUBLIC_HOST=$(db_setting public_host "")

PORT=${OCSERV_PORT:-${OCSERV_TCP:-$DB_OCSERV_TCP}}
UDP_PORT=${OCSERV_UDP:-$DB_OCSERV_UDP}
TRANSPORT=$(printf '%s' "${OCSERV_TRANSPORT:-$DB_OCSERV_TRANSPORT}" | tr '[:upper:]' '[:lower:]')
PUBLIC_HOST=${IRONPANEL_PUBLIC_HOST:-${PUBLIC_HOST:-${DB_PUBLIC_HOST:-$(hostname -f 2>/dev/null || hostname)}}}
...
# Detect the common 1195 conflict. OpenVPN TCP and ocserv cannot bind the same
# TCP port at the same time. We keep ocserv config authoritative, but write a
# clear log line so the admin knows what to change if the bind fails.
DB_OPENVPN_PROTO=$(db_setting openvpn_transport udp | tr '[:upper:]' '[:lower:]')
DB_OPENVPN_TCP=$(db_setting port_openvpn_tcp 1195)
if [[ "$DB_OPENVPN_PROTO" == "tcp" && "$DB_OPENVPN_TCP" == "$PORT" ]]; then
  log "WARNING: OpenVPN TCP and Ocserv are both configured for tcp/$PORT. Only one service can listen on a TCP port. Set OpenVPN to UDP or move one port."
fi

# v19.10.17: attach only a fail-safe disconnect spool hook. It never opens
# the database and always exits zero, so AnyConnect authentication is isolated.
HOOK_LINES='disconnect-script = /opt/ironpanel/scripts/ocserv_usage_disconnect.sh'

cat > /etc/ocserv/ocserv.conf.tmp <<OC
# Managed by IronPanel 19.10.25
...
chmod 755 /opt/ironpanel/scripts/ocserv_usage_disconnect.sh /opt/ironpanel/scripts/ironpanel_usage_event_hook.py 2>/dev/null || true
mkdir -p /var/lib/ironpanel/usage-events; chmod 700 /var/lib/ironpanel/usage-events 2>/dev/null || true
