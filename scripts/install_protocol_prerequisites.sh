...
build_or_repair_openvpn_pki(){
  ...
  # This avoids needless certificate generation during an update.
  if ! cert_key_match "$pki/issued/server.crt" "$pki/private/server.key" || ! server_chain_valid "$pki/ca.crt" "$pki/issued/server.crt"; then
    if cert_key_match /etc/openvpn/server/server.crt /etc/openvpn/server/server.key \
      ...
      timeout 120 env EASYRSA_BATCH=1 EASYRSA_REQ_CN=server ./easyrsa build-server-full server nopass >>"$LOG" 2>&1 \
        || { log 'ERROR: timed out or failed generating OpenVPN server certificate'; return 1; }
    fi
  fi
  if ! dh_valid "$pki/dh.pem"; then
    if dh_valid /etc/openvpn/server/dh.pem; then
      ...
      rm -f "$pki/dh.pem"
      timeout 180 env EASYRSA_BATCH=1 ./easyrsa gen-dh >>"$LOG" 2>&1 \
