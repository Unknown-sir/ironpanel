...
backup_openvpn_files(){
  ...
  mkdir -p "$dir"
  for f in /etc/openvpn/server /etc/openvpn/easy-rsa; do [[ -e "$f" ]] && cp -a "$f" "$dir/" 2>/dev/null || true; done
...
build_or_repair_openvpn_pki(){
  ...
  if cert_valid /etc/openvpn/server/ca.crt \
    ...
    && dh_valid /etc/openvpn/server/dh.pem; then
    log 'Existing OpenVPN CA, server certificate, private key and DH parameters are valid'
  elif restore_openvpn_from_existing_pki; then
    :
  else
    backup_openvpn_files
    local er pki
    ...
    cd "$er"
    pki="$er/pki"
    if ! cert_valid "$pki/ca.crt" || ! key_valid "$pki/private/ca.key"; then
      rm -rf "$pki"
      EASYRSA_BATCH=1 ./easyrsa init-pki >>"$LOG" 2>&1
      EASYRSA_BATCH=1 EASYRSA_REQ_CN=IronPanel-CA ./easyrsa build-ca nopass >>"$LOG" 2>&1
    fi
    if ! cert_key_match "$pki/issued/server.crt" "$pki/private/server.key"; then
    ...
    install -m 644 "$pki/dh.pem" /etc/openvpn/server/dh.pem
    [[ -s "$pki/crl.pem" ]] && install -m 644 "$pki/crl.pem" /etc/openvpn/server/crl.pem || true
    log 'Created/repaired the complete OpenVPN PKI'
  fi
  ...
  if [[ ! -s /etc/openvpn/server/crl.pem ]]; then
    local er=/etc/openvpn/easy-rsa
    if [[ -x "$er/easyrsa" && -d "$er/pki" ]]; then
      (cd "$er" && EASYRSA_BATCH=1 ./easyrsa gen-crl >>"$LOG" 2>&1) || true
      [[ -s "$er/pki/crl.pem" ]] && install -m 644 "$er/pki/crl.pem" /etc/openvpn/server/crl.pem || true
    fi
  fi
  if [[ -s /etc/openvpn/server/crl.pem ]] && ! openssl crl -in /etc/openvpn/server/crl.pem -noout >/dev/null 2>&1; then rm -f /etc/openvpn/server/crl.pem; fi
  ...
  if [[ -s /etc/openvpn/server/crl.pem ]]; then chown root:root /etc/openvpn/server/crl.pem; chmod 644 /etc/openvpn/server/crl.pem; fi
