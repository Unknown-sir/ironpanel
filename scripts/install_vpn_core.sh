...
if [[ -x /opt/ironpanel/scripts/repair_ocserv.sh ]]; then
  if ! OCSERV_PORT=${OCSERV_TCP:-8445} OCSERV_TCP=${OCSERV_TCP:-8445} OCSERV_UDP=${OCSERV_UDP:-8445} OCSERV_TRANSPORT=${OCSERV_TRANSPORT:-tcp_udp} IRONPANEL_PUBLIC_HOST="$PUBLIC_HOST" ETC_DIR="$ETC_DIR" bash /opt/ironpanel/scripts/repair_ocserv.sh; then
