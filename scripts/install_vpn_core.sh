...
apt-get install -y \
  ...
  xl2tpd ppp \
  iptables iptables-persistent openssl ca-certificates qrencode curl
...
PostUp = iptables -A FORWARD -i wg0 -j ACCEPT; iptables -A FORWARD -o wg0 -j ACCEPT; iptables -t nat -A POSTROUTING -s 10.66.66.0/24 -o ${WAN_IF:-eth0} -j MASQUERADE
PostDown = iptables -D FORWARD -i wg0 -j ACCEPT; iptables -D FORWARD -o wg0 -j ACCEPT; iptables -t nat -D POSTROUTING -s 10.66.66.0/24 -o ${WAN_IF:-eth0} -j MASQUERADE
...
systemctl restart strongswan-starter || systemctl restart strongswan || systemctl restart ipsec || true
