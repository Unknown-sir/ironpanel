...
OPENVPN_PKI_READY=0
# Fresh installs must not proceed to a restart loop with a partial PKI. Retry the
# idempotent repair once because an interrupted dpkg/Easy-RSA first pass can
# leave a valid CA while the server leaf/DH/runtime files are still absent.
for openvpn_try in 1 2; do
  if APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" bash "$PREREQ_SCRIPT" --ensure-openvpn; then
    OPENVPN_PKI_READY=1
    break
  fi
  echo "[IronPanel] OpenVPN PKI preparation attempt $openvpn_try failed" >&2
  sleep 2
done
if [[ "$OPENVPN_PKI_READY" != "1" ]]; then
...
verb 3
OVPN
openvpn_runtime_preflight(){
  local required cert_hash key_hash
  for required in /etc/openvpn/server/ca.crt /etc/openvpn/server/server.crt \
                  /etc/openvpn/server/server.key /etc/openvpn/server/dh.pem \
                  /etc/openvpn/server/tls-crypt.key; do
    [[ -s "$required" ]] || { echo "[IronPanel] OpenVPN required file missing: $required" >&2; return 1; }
  done
  openssl x509 -in /etc/openvpn/server/ca.crt -noout >/dev/null 2>&1 || return 1
  openssl x509 -in /etc/openvpn/server/server.crt -noout >/dev/null 2>&1 || return 1
  openssl pkey -in /etc/openvpn/server/server.key -noout >/dev/null 2>&1 || return 1
  openssl verify -CAfile /etc/openvpn/server/ca.crt /etc/openvpn/server/server.crt >/dev/null 2>&1 || return 1
  ...
  timeout 10 openssl dhparam -in /etc/openvpn/server/dh.pem -noout >/dev/null 2>&1 || return 1
  grep -q '^-----BEGIN OpenVPN Static key V1-----$' /etc/openvpn/server/tls-crypt.key || return 1
  grep -q '^-----END OpenVPN Static key V1-----$' /etc/openvpn/server/tls-crypt.key || return 1
  [[ -s /etc/openvpn/easy-rsa/pki/ca.crt && -s /etc/openvpn/easy-rsa/pki/private/ca.key ]] || return 1
  cmp -s /etc/openvpn/server/ca.crt /etc/openvpn/easy-rsa/pki/ca.crt || return 1
}
if [[ "$OPENVPN_PKI_READY" != "1" ]] || ! openvpn_runtime_preflight; then
  OPENVPN_PKI_READY=0
  CORE_ERRORS+=(openvpn-runtime-preflight)
  echo '[IronPanel] OpenVPN runtime preflight failed; service will NOT be started with incomplete PKI files.' >&2
fi
...
if [[ "${OPENVPN_PKI_READY:-0}" == "1" ]]; then
  ...
  if ! systemctl is-active --quiet openvpn-server@server; then
    CORE_ERRORS+=(openvpn-inactive)
    journalctl -u openvpn-server@server -n 80 --no-pager >&2 || true
  fi
else
