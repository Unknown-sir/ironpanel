#!/usr/bin/env bash
set -euo pipefail
WIREGUARD_MTU=${IRONPANEL_WIREGUARD_MTU:-${WIREGUARD_MTU:-1360}}
...
for rule in "${OPENVPN_UDP}/udp" "${OPENVPN_TCP}/tcp" "${OCSERV_TCP}/tcp" "${OCSERV_UDP}/udp" "${WIREGUARD_PORT}/udp" "${XRAY_PORT}/tcp" "${PPTP_PORT}/tcp" "${HYSTERIA2_PORT}/udp" "500/udp" "4500/udp" "${L2TP_PORT}/udp"; do
...
if [[ -x /opt/ironpanel/scripts/repair_hysteria2.sh ]]; then HYSTERIA2_PORT=${HYSTERIA2_PORT:-4433} HYSTERIA2_UP="${HYSTERIA2_UP:-100 mbps}" HYSTERIA2_DOWN="${HYSTERIA2_DOWN:-300 mbps}" bash /opt/ironpanel/scripts/repair_hysteria2.sh || true; fi
cat <<INFO
