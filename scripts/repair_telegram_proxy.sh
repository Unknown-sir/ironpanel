...
install_packages(){
  ...
  if command -v apt-get >/dev/null 2>&1; then
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y nodejs npm git ca-certificates curl iproute2 || true
  elif command -v yum >/dev/null 2>&1; then
    yum install -y nodejs npm git ca-certificates curl iproute || true
  fi
