...
_doc=json.load(open(sys.argv[1]))
plan=_doc.get("plan",[])
panel_ports=set(int(x) for x in (_doc.get("panel_ports") or []) if str(x).isdigit())
for r in plan:
  ...
  while read -r NAME PROTO PORT HOST; do
    [[ -z "${NAME:-}" ]] && continue
    if [[ "$NAME" == "SKIPPANEL" ]]; then log "SKIP $PROTO/$PORT because it is panel port"; continue; fi
    IP=$(resolve_host "$HOST")
    [[ -z "$IP" ]] && { log "cannot resolve $HOST for $NAME"; continue; }
    if [[ "$PROTO" == "gre" ]]; then
      iptables -t nat -A "$CHAIN" -m addrtype --dst-type LOCAL -p gre -j DNAT --to-destination "$IP" -m comment --comment "IRONPANEL_NODE_GW_${NAME}_gre" >/dev/null 2>&1 || true
    ...
    if [[ -n "${PANEL_PORT:-}" && "$PROTO" == "tcp" && "$PORT" == "$PANEL_PORT" ]]; then log "SKIP $NAME $PROTO/$PORT because it is panel port"; continue; fi
    iptables -t nat -A "$CHAIN" -m addrtype --dst-type LOCAL -p "$PROTO" --dport "$PORT" -j DNAT --to-destination "$IP:$PORT" -m comment --comment "IRONPANEL_NODE_GW_${NAME}_${PROTO}_${PORT}" >/dev/null 2>&1 || true
  ...
  log "gateway rules applied from dynamic protocol ports; only traffic whose destination is the main server local IP is DNATed to node(s)"
}
case "${1:---apply}" in
  --apply) apply_rules ;;
