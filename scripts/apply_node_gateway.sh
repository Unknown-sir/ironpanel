...
apply_rules(){
  ...
  # OUTPUT hook is only for local tests from the panel server itself and is restricted to LOCAL destinations.
  ...
  while read -r KIND NAME PROTO PORT HOST; do
    ...
    if [[ "$PROTO" == "gre" ]]; then
      iptables -t nat -A "$CHAIN" ! -s "$IP" -p gre -j DNAT --to-destination "$IP" -m comment --comment "IRONPANEL_NODE_GW_${NAME}_gre" >/dev/null 2>&1 || { log "failed NAT $NAME GRE -> $IP"; failed=$((failed+1)); continue; }
    ...
    iptables -t nat -A "$CHAIN" ! -s "$IP" -p "$PROTO" --dport "$PORT" -j DNAT --to-destination "$IP:$PORT" -m comment --comment "IRONPANEL_NODE_GW_${NAME}_${PROTO}_${PORT}" >/dev/null 2>&1 || { log "failed NAT $NAME $PROTO/$PORT -> $IP:$PORT"; failed=$((failed+1)); continue; }
  ...
  log "note: client config still targets main-panel IP/domain; verify node routing by checking public IP through VPN/Xray, not by app server-address label"
