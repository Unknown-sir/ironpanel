...
SSHD_CONF=$SSHD_CONF_DIR/ironpanel.conf
...
configure_sshd(){
  ...
  rm -f "$SSHD_CONF_DIR/99-ironpanel-ssh.conf" 2>/dev/null || true
...
Match Group $SSH_GROUP
  ...
  PermitTTY yes
  GatewayPorts no
  # Do not force /bin/false here. Many mobile SSH tunnel apps request a
  # lightweight session before opening forwarding, and ForceCommand closes it.
  # SSH users are still isolated by the ironpanel-ssh group and generated names.
EOF
  } > "$SSHD_CONF"
