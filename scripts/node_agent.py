...
def protocol_health():
    ...
    for proto, services in PROTOCOL_SERVICES.items():
        ...
        if installed and active_ok and proto == 'ocserv':
            ...
            tcp=_text_port(text, r'^\s*tcp-port\s*=\s*(\d+)', 8445)
            udp=_text_port(text, r'^\s*udp-port\s*=\s*(\d+)', 0)
            try:
            ...
            auth_count=0
            try: auth_count=sum(1 for x in Path('/etc/ironpanel/ocpasswd').read_text(errors='ignore').splitlines() if x.strip() and not x.lstrip().startswith('#') and ':' in x)
            except Exception: pass
            ...
            port=_text_port(text, r'^\s*port\s+(\d+)',1194)
            ...
            runtime_ok=bool(ports and listening)
            detail_parts.append(f'public-ports={ports};listening={listening}')
            ...
            port=_text_port(text, r'^\s*listen\s*:\s*(?:[^\n:]+:)?(\d+)',4433)
            ...
            port=_text_port(text, r'^\s*Port\s+(\d+)',22)
...
def _run_node_core_install(protos):
    ...
    try:
        proc=subprocess.run(['bash', str(script), str(protos or '')], capture_output=True, text=True, timeout=1200)
...
def _normalize_node_direct_ports(protocols, direct_ports):
    changed=[]
    dp={str(k): int(v) for k,v in (direct_ports or {}).items() if str(v).isdigit() or isinstance(v,int)}
    ...
    if 'ocserv' in protocols and dp.get('ocserv'):
        port=str(dp['ocserv']);
        for path in ('/etc/ocserv/ocserv.conf',):
            for x in _normalize_text_port_file(path, [(r'^tcp-port\s*=\s*\d+\s*$', f'tcp-port = {port}'),(r'^udp-port\s*=\s*\d+\s*$', f'udp-port = {port}')]): changed.append(f'ocserv:{x}:port={port}')
    ...
    if 'telegram_proxy' in protocols and dp.get('telegram_proxy'):
        ...
        pats=[(r'^(\s*PORT\s*=\s*)\d+\s*$', r'\g<1>'+port),(r'(--port\s+)\d+', r'\g<1>'+port)]
...
def _rebuild_plain_auth_from_metadata(protocols):
    ...
    if 'ocserv' in selected:
        ...
        target=Path('/etc/ironpanel/ocpasswd'); tmp=Path('/etc/ironpanel/.ocpasswd.node.tmp')
        tmp.unlink(missing_ok=True)
        binary=shutil.which('ocpasswd')
        if oc_rows and not binary:
            errors.append('ocpasswd binary missing on node')
        else:
            for item in oc_rows:
                ...
                try:
                    proc=subprocess.run([binary, '-c', str(tmp), username], input=f'{password}\n{password}\n', capture_output=True, text=True, timeout=20) if binary else None
                    if proc and proc.returncode != 0:
                        errors.append(f'ocpasswd {username}: '+(proc.stderr or proc.stdout or 'failed')[-400:]); break
                except Exception as exc:
                    errors.append(f'ocpasswd {username}: {exc}'); break
            if not oc_rows and not errors:
                tmp.write_text('', encoding='utf-8')
            if not errors and tmp.exists():
                os.chmod(tmp,0o600); tmp.replace(target); details.append(f'ocserv-users={len(oc_rows)}')
            else:
                tmp.unlink(missing_ok=True)
...
def handle_job(job):
    ...
    if action == 'ensure_protocols':
        ...
        rc,out=_run_node_core_install(','.join(protos))
    ...
    if action == 'sync_protocol_configs':
        ...
        pre_rc,before=_run_node_core_install(','.join(protos))
        out=_apply_config_bundle(payload.get('bundle') or {})
        normalized=_normalize_node_protocol_configs(protos, payload.get('bundle') or {}, payload.get('direct_ports') or {})
        auth_ok, auth_detail, auth_errors=_rebuild_plain_auth_from_metadata(protos)
        post_rc,after=_run_node_core_install(','.join(protos))
