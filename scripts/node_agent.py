...
def handle_job(job):
    ...
    if action == 'sync_user':
        ...
        if str(payload.get('protocol') or '') == 'ssh':
    ...
    if action == 'sync_users_bulk':
        ...
        stale_locked=_lock_stale_ssh_accounts(wanted_ssh) if 'ssh' in (payload.get('protocols') or []) else []
        manifest={'count':len(written), 'protocols':payload.get('protocols') or [], 'reason':payload.get('reason') or '', 'written':written[-500:], 'ssh_accounts':sorted(wanted_ssh), 'stale_ssh_locked':stale_locked, 'errors':errors[-50:]}
        ok=not errors
    ...
    if action == 'sync_protocol_configs':
        ...
        ok=(pre_rc == 0 and post_rc == 0 and not missing and not inactive)
        _node_sync_state({'protocols':protos, 'protocol_health':ph, 'sync_ok':ok, 'missing':missing, 'inactive':inactive})
        norm_text='\nnormalized:\n' + ('\n'.join(normalized) if normalized else 'no bind changes needed')
        summary=f'\nverification: ok={ok} pre_exit={pre_rc} post_exit={post_rc} missing={",".join(missing) or "none"} inactive={",".join(inactive) or "none"}'
        detail=('pre-install:\n'+before[-4000:]+'\nconfig-sync:\n'+out+norm_text+
