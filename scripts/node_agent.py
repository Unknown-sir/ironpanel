#!/usr/bin/env python3
import argparse, json, os, subprocess, time, urllib.request, urllib.error, ssl, re, sys, shutil, socket, base64, stat, hashlib, secrets
...
PROTOCOL_SERVICES={
 ...
 'hysteria2':['hysteria-server','hysteria2-server','hysteria-server@server','hysteria','hysteria2'], 'telegram_proxy':['ironpanel-tgproxy','mtproxy'], 'ssh':['ssh','sshd']
}
LAST_ERROR=''
...
def _load_direct_ports_raw(value=None):
    raw = value if value is not None else os.environ.get('IRONPANEL_NODE_DIRECT_PORTS_JSON')
    if isinstance(raw, dict):
        ...
        try:
            ...
            # v19.10.10: tolerate malformed values like {"telegram_proxy":6974}}
            # no matter whether they come from legacy env, queued job payloads or
            # shell quoting. Parse the first valid balanced object, then fall back
            # to key:port extraction.
            candidates=[]
            ...
            for candidate in candidates:
                try:
                    ...
                    continue
            if data is None:
                data = {m.group(1): int(m.group(2)) for m in re.finditer(r"[\'\"]?([A-Za-z0-9_]+)[\'\"]?\s*:\s*([0-9]{1,5})", text)}
    out = {}
    try:
...
def _direct_port_for(proto):
    try:
        data=_load_direct_ports_raw()
        port=int(data.get(str(proto)) or 0)
        return port if 0 < port <= 65535 else 0
    except Exception:
        return 0

def _clean_ssl_domain(value=''):
    value=str(value or '').strip()
    value=re.sub(r'^https?://', '', value, flags=re.I).split('/')[0].strip()
    if ':' in value and not value.startswith('['):
        h, _, p = value.rpartition(':')
        if p.isdigit(): value = h
    return value.strip('[]').lower()

def _payload_ssl_domain(payload=None):
    if isinstance(payload, dict):
        v=_clean_ssl_domain(payload.get('ssl_domain') or payload.get('node_ssl_domain') or '')
        if v: return v
    return _clean_ssl_domain(os.environ.get('IRONPANEL_NODE_SSL_DOMAIN') or '')
...
def _port_listening(port, transport='tcp'):
    try:
        ...
        for _ in range(5):
            out = sh(f"ss -H -ln{flag} 'sport = :{port}' 2>/dev/null || ss -H -ln{flag} 2>/dev/null | awk '{{print $4}}' | grep -E '(^|:){port}$' || true")
            if out:
                return True
            time.sleep(0.4)
        return False
...
def _hysteria_config_port(default=4433):
    text=_config_text(['/etc/hysteria/config.yaml','/etc/hysteria2/config.yaml','/etc/hysteria/server.yaml','/etc/hysteria2/server.yaml'])
    direct=_direct_port_for('hysteria2')
    if direct:
        return direct
    return _text_port(text, r'^\s*(?:listen|addr)\s*:\s*(?:[^\n:]+:)?(\d+)', default)


def _repair_hysteria2_runtime_for_node():
    port=_hysteria_config_port(4433)
    if Path('/etc/systemd/system/hysteria-server.service').exists() or Path('/lib/systemd/system/hysteria-server.service').exists():
        sh('systemctl daemon-reload 2>/dev/null || true', timeout=20)
        sh('systemctl reset-failed hysteria-server 2>/dev/null || true', timeout=15)
        sh('systemctl restart hysteria-server 2>/dev/null || true', timeout=20)
        time.sleep(1)
    return bool(_port_listening(port, 'udp') or is_active('hysteria-server') or is_active('hysteria2-server') or is_active('hysteria'))
...
def protocol_health():
    ...
    for proto, services in PROTOCOL_SERVICES.items():
        ...
        elif proto=='xray': installed=bool(sh('command -v xray || test -x /usr/local/bin/xray && echo xray || test -x /usr/bin/xray && echo xray || test -x /usr/local/xray/xray && echo xray || true'))
        ...
        if installed and active_ok and proto == 'ocserv':
        ...
        elif installed and proto == 'hysteria2':
            port=_hysteria_config_port(4433)
            listen_ok=_port_listening(port,'udp')
            if not active_ok and not listen_ok:
                _repair_hysteria2_runtime_for_node()
                active=[svc for svc in services if is_active(svc)]
                active_ok=bool(active)
                listen_ok=_port_listening(port,'udp')
                detail_parts=[','.join(active) if active else 'installed-not-running']
            if listen_ok and not active_ok:
                active_ok=True
                detail_parts=['udp-listener']
            runtime_ok=bool(listen_ok or active_ok)
            detail_parts.append(f'udp/{port}:{"listen" if listen_ok else "pending-listen"}')
            ...
            # A just-restarted Node.js proxy can take a moment to bind. Treat a
            # healthy active service plus correct configured direct port as pending-ok
            # during sync, but still report listener details for diagnostics.
            service_ok=bool(active_ok)
            runtime_ok=bool(listen_ok or (service_ok and port > 0))
            detail_parts.append(f'tcp/{port}:{"listen" if listen_ok else "pending-listen"}')
...
def _safe_source(value, fallback='runtime'):
    text=re.sub(r'[^A-Za-z0-9_.:-]+','_',str(value or '')).strip('_.:-')
    return text[:48] or fallback


def _meta_for(metas, identity, protocol=''):
    identity=str(identity or '').strip()
    protocol=str(protocol or '').strip().lower()
    uid=0
    mm=re.match(r'ip-(\d+)-', identity)
    if mm:
        uid=int(mm.group(1))
    for row in metas:
        if protocol and str(row.get('protocol') or '').lower() != protocol:
            continue
        if uid and int(row.get('user_id') or 0) == uid:
            return row
        username=str(row.get('username') or '')
        safe=re.sub(r'[^A-Za-z0-9_.-]','_',username)[:64]
        identities=(username, safe, str(row.get('xray_email') or ''), str(row.get('xray_uuid') or ''))
        if identity in identities:
            return row
    return None


def _xray_api_server():
    """Read the real loopback Stats API port from the active Xray config."""
    for raw in ('/usr/local/etc/xray/config.json', '/etc/xray/config.json', '/etc/ironpanel/xray/config.json'):
        try:
            data=json.loads(Path(raw).read_text(encoding='utf-8', errors='ignore'))
        except Exception:
            continue
        for inbound in data.get('inbounds', []) if isinstance(data, dict) else []:
            ...
            if tag == 'api' or ('api' in tag and protocol == 'dokodemo-door'):
                try:
...
def _parse_xray_stats_output(output):
    """Parse quoted/unquoted JSON and legacy text-protobuf Xray stats."""
    text=str(output or '').strip(); stats={}
    if not text: return stats
    decoder=json.JSONDecoder()
    for idx,char in enumerate(text):
        if char not in '{[': continue
        try: payload,_=decoder.raw_decode(text[idx:])
        except Exception: continue
        rows=payload.get('stat',[]) if isinstance(payload,dict) else payload
        if not isinstance(rows,list): continue
        for row in rows:
            if not isinstance(row,dict): continue
            name=str(row.get('name') or '').strip()
            try: value=int(str(row.get('value',0)).strip().strip('"'))
            except Exception: continue
            if name: stats[name]=max(0,value)
        if stats or (isinstance(payload,dict) and 'stat' in payload): return stats
    current=None
    for line in text.splitlines():
        nm=re.search(r'(?:\bname\s*:|"name"\s*:)\s*"([^"]+)"',line)
        if nm: current=nm.group(1)
        vm=re.search(r'(?:\bvalue\s*:|"value"\s*:)\s*"?(\d+)"?',line)
        if vm and current:
            stats[current]=int(vm.group(1)); current=None
    if not stats:
        for block in re.finditer(r'\{[^{}]{0,2000}\}',text,re.S):
            nm=re.search(r'"?name"?\s*:\s*"([^"]+)"',block.group(0))
            vm=re.search(r'"?value"?\s*:\s*"?(\d+)"?',block.group(0))
            if nm and vm: stats[nm.group(1)]=int(vm.group(1))
    return stats


def _query_xray_stats():
    server=_xray_api_server()
    bins=[]
    for candidate in ('/usr/local/bin/xray','/usr/bin/xray'):
        if Path(candidate).is_file(): bins.append(candidate)
    found=sh('command -v xray 2>/dev/null || true',timeout=2)
    if found and found not in bins: bins.append(found)
    for binary in bins or ['xray']:
        out=sh(f'{binary} api statsquery --server={server} 2>&1',timeout=8)
        # Successful empty JSON is meaningful; common connection errors are not.
        if out and not any(x in out.lower() for x in ('failed to dial','connection refused','no such file or directory','not found')):
            return out
    return ''


def _usage_report(meta, protocol, rx, tx, source=None, final=False):
    return {
        'user_id': int(meta.get('user_id') or 0),
        'username': str(meta.get('username') or ''),
        'protocol': str(protocol or meta.get('protocol') or 'runtime'),
        'source': _safe_source(source or protocol or 'runtime'),
        'rx': max(0,int(rx or 0)), 'tx': max(0,int(tx or 0)),
        'final': bool(final),
    }


def _walk_ocserv_rows(obj):
    if isinstance(obj, dict):
        low={str(k).lower().replace('-','_').replace(' ','_'):v for k,v in obj.items()}
        ident=next((low.get(k) for k in ('username','user_name','user','name') if low.get(k) not in (None,'')),None)
        rx=next((low.get(k) for k in ('bytes_in','rx','received','bytes_received','traffic_in','input_bytes') if low.get(k) not in (None,'')),None)
        tx=next((low.get(k) for k in ('bytes_out','tx','sent','bytes_sent','traffic_out','output_bytes') if low.get(k) not in (None,'')),None)
        if ident is not None and (rx is not None or tx is not None):
            sid=next((low.get(k) for k in ('id','session_id','sid','device','vpn_ip','ip') if low.get(k) not in (None,'')),ident)
            try: yield str(ident),str(sid),int(rx or 0),int(tx or 0)
            except Exception: pass
        for value in obj.values(): yield from _walk_ocserv_rows(value)
    elif isinstance(obj,list):
        for value in obj: yield from _walk_ocserv_rows(value)


def _consume_usage_events(metas):
    """Persist hook events so a lost heartbeat cannot lose final counters."""
    event_dir=Path('/var/lib/ironpanel/usage-events')
    ledger_path=STATE/'usage_ledger.json'
    try: ledger=json.loads(ledger_path.read_text(encoding='utf-8'))
    except Exception: ledger={}
    if not isinstance(ledger,dict): ledger={}
    dirty=False
    if event_dir.is_dir():
        for path in sorted(event_dir.glob('*.json'))[:5000]:
            try:
                row=json.loads(path.read_text(encoding='utf-8'))
                protocol=str(row.get('protocol') or '').lower()
                meta=_meta_for(metas,row.get('username'),protocol)
                if not meta: continue
                source=_safe_source(row.get('source'),protocol)
                ledger[f"{int(meta.get('user_id') or 0)}:{source}"]=_usage_report(meta,protocol,int(row.get('rx') or 0),int(row.get('tx') or 0),source,final=True)
                path.unlink(missing_ok=True); dirty=True
            except Exception:
                continue
    if dirty:
    ...
    return [v for v in ledger.values() if isinstance(v,dict)][-5000:]


def _active_ppp_reports(metas):
    reports=[]
    root=Path('/run/ironpanel-ppp')
    if not root.is_dir(): return reports
    for path in root.glob('*.json'):
        try:
            row=json.loads(path.read_text(encoding='utf-8'))
            protocol=str(row.get('protocol') or 'l2tp').lower()
            meta=_meta_for(metas,row.get('username'),protocol)
            if not meta: continue
            iface=_safe_source(row.get('interface') or path.stem,'ppp')
            rx=int((Path('/sys/class/net')/iface/'statistics/rx_bytes').read_text().strip())
            tx=int((Path('/sys/class/net')/iface/'statistics/tx_bytes').read_text().strip())
            reports.append(_usage_report(meta,protocol,rx,tx,row.get('source') or f'ppp_{protocol}_{iface}'))
        except Exception: pass
    return reports


def _ocserv_reports(metas):
    payload=''
    for cmd in ('occtl -j show users 2>/dev/null','occtl --json show users 2>/dev/null'):
        payload=sh(cmd,timeout=4)
        if payload: break
    if not payload: return []
    starts=[i for i in (payload.find('{'),payload.find('[')) if i>=0]
    if starts: payload=payload[min(starts):]
    try: data=json.loads(payload)
    except Exception: return []
    reports=[]
    for ident,sid,rx,tx in _walk_ocserv_rows(data):
        meta=_meta_for(metas,ident,'ocserv')
        if meta: reports.append(_usage_report(meta,'ocserv',rx,tx,'ocserv_'+_safe_source(sid,ident)))
    return reports


def _hysteria_reports(metas):
    try: secret=Path('/etc/ironpanel/hysteria2_stats_secret').read_text().strip()
    ...
    if isinstance(data,dict):
        for ident,row in data.items():
            if not isinstance(row,dict): continue
            meta=_meta_for(metas,ident,'hysteria2')
            if meta: reports.append(_usage_report(meta,'hysteria2',int(row.get('tx') or 0),int(row.get('rx') or 0),'hysteria2'))
    return reports


def _telegram_proxy_reports(metas):
    try: data=json.loads(Path('/opt/ironpanel-telegram-proxy/ironpanel/usage.json').read_text(errors='ignore'))
    except Exception: return []
    ...
    for key,row in users.items():
        ...
        try:
            uid=int(key)
            meta=next((m for m in metas if str(m.get('protocol') or '')=='telegram_proxy' and int(m.get('user_id') or 0)==uid),None)
        except Exception: pass
        if not meta: continue
        reports.append(_usage_report(meta,'telegram_proxy',int(row.get('rx') or 0),int(row.get('tx') or 0),'telegram_proxy'))
    return reports
...
def collect_usage_reports():
    ...
    # Xray Stats API: parse current JSON (quoted int64 values) and old output.
    out=_query_xray_stats()
    xmap={}
    for name,value in _parse_xray_stats_output(out).items():
        parts=name.split('>>>')
        ...
        if direction not in ('uplink','downlink'):
            continue
        meta=_meta_for(metas,email,'xray')
        if meta:
            key=str(meta.get('user_id') or meta.get('username'))
            row=xmap.setdefault(key,_usage_report(meta,'xray',0,0,'xray'))
            if direction=='uplink': row['rx']=max(0,int(value))
            if direction=='downlink': row['tx']=max(0,int(value))
    ...
    for line in sh('wg show wg0 transfer 2>/dev/null || true',timeout=4).splitlines():
        parts=line.split()
        if len(parts)>=3 and parts[0] in wg_keys:
            try: reports.append(_usage_report(wg_keys[parts[0]],'wireguard',int(parts[1]),int(parts[2]),'wireguard'))
            except Exception: pass

    # OpenVPN status counters.
    for raw in ('/var/log/openvpn/status.log','/run/openvpn-server/status-server.log','/etc/openvpn/server/status.log','/var/log/openvpn/openvpn-status.log'):
        try: lines=Path(raw).read_text(errors='ignore').splitlines()
        except Exception: continue
        in_v1=False
        for line in lines:
            if line.startswith('CLIENT_LIST,'):
                parts=line.split(',')
                if len(parts)>=7: cn,rx_s,tx_s=parts[1].strip(),parts[5],parts[6]
                elif len(parts)>=5: cn,rx_s,tx_s=parts[1].strip(),parts[3],parts[4]
                else: continue
                meta=_meta_for(metas,cn,'openvpn')
                if meta:
                    try: reports.append(_usage_report(meta,'openvpn',int(rx_s or 0),int(tx_s or 0),'openvpn'))
                    except Exception: pass
                continue
            if line.startswith('Common Name,Real Address,Bytes Received,Bytes Sent,Connected Since'):
                in_v1=True; continue
            if line.startswith('ROUTING TABLE'): in_v1=False
            if in_v1 and ',' in line and not line.startswith('Updated,'):
                parts=line.split(','); meta=_meta_for(metas,parts[0].strip(),'openvpn') if len(parts)>=4 else None
                if meta:
                    try: reports.append(_usage_report(meta,'openvpn',int(parts[2] or 0),int(parts[3] or 0),'openvpn'))
                    except Exception: pass
        break

    reports.extend(_active_ppp_reports(metas))
    reports.extend(_ocserv_reports(metas))
    reports.extend(_hysteria_reports(metas))
    reports.extend(_telegram_proxy_reports(metas))
    reports.extend(_consume_usage_events(metas))

    # Deduplicate by stable source/user. Live counters win over an older ledger
    # value only when they are greater, preserving monotonic reports.
    dedup={}
    for row in reports:
        key=(int(row.get('user_id') or 0),str(row.get('source') or row.get('protocol') or ''))
        old=dedup.get(key)
        if not old or int(row.get('rx') or 0)+int(row.get('tx') or 0) >= int(old.get('rx') or 0)+int(old.get('tx') or 0):
            dedup[key]=row
    return list(dedup.values())[:5000]
...
def _curl_post_json(base, token, path, data):
    # v19.10.10: urllib can time out on IP/SNI-mismatched TLS on some
    # deployments while curl -k succeeds. Keep curl as a transport fallback
    # for the exact same authenticated JSON API calls.
    if not shutil.which('curl'):
        raise RuntimeError('curl not available')
    url=base.rstrip('/') + path
    payload=json.dumps(data, ensure_ascii=False, separators=(',',':'))
    args=['curl','-fsS','--connect-timeout','8','--max-time','30',
          '-H','Content-Type: application/json', '-H', 'X-NODE-TOKEN: '+str(token or ''),
          '-H','User-Agent: IronPanel-Node-Agent/'+VERSION+'-curl',
          '--data-binary','@-', url]
    if url.lower().startswith('https://') and os.environ.get('IRONPANEL_NODE_INSECURE_TLS','0') == '1':
        args.insert(1, '-k')
    proc=subprocess.run(args, input=payload, capture_output=True, text=True, timeout=35)
    if proc.returncode != 0:
        err=((proc.stderr or '') + (proc.stdout or ''))[-1000:]
        raise RuntimeError('curl rc=%s %s' % (proc.returncode, err.strip()))
...
def post_json(url, token, path, data):
    ...
    for base in _master_candidates(url):
        try:
            ...
            try:
                res=_curl_post_json(base, token, path, data)
                LAST_ERROR=''
                if base.rstrip('/') != url.rstrip('/'):
                    try: (STATE/'master.resolved').write_text(base.rstrip()+'\n', encoding='utf-8')
                    except Exception: pass
                ...
                errors.append(f'{base}: curl-fallback {ce}')
...
def _coerce_direct_ports(direct_ports=None):
    try:
        return _load_direct_ports_raw(direct_ports)
    except Exception:
        return {}


def _direct_port_env(direct_ports=None, ssl_domain=None):
    env={}
    ...
    sd=_clean_ssl_domain(ssl_domain or os.environ.get('IRONPANEL_NODE_SSL_DOMAIN') or '')
    if sd:
        env['IRONPANEL_NODE_SSL_DOMAIN']=sd
        env['NODE_SSL_DOMAIN']=sd
    return env


def _run_node_core_install(protos, direct_ports=None, ssl_domain=None):
    ...
    try:
        env=os.environ.copy(); env.update(_direct_port_env(direct_ports, ssl_domain))
...
SAFE_WRITE_PREFIXES = ('/etc/openvpn','/etc/wireguard','/etc/ocserv','/etc/xl2tpd','/etc/ppp','/etc/ipsec.d','/etc/xray','/usr/local/etc/xray','/etc/hysteria','/etc/hysteria2','/etc/ironpanel','/etc/ssh/sshd_config.d','/etc/systemd/system/ironpanel-tgproxy.service','/opt/ironpanel/scripts','/opt/ironpanel-telegram-proxy/ironpanel')
...
def _sanitize_host(value):
    raw=str(value or '').strip().replace('https://','').replace('http://','').split('/')[0].strip('[]')
    if raw.count(':') == 1:
        raw=raw.split(':',1)[0]
...
def _ensure_node_ikev2_cert(host):
    host=_sanitize_host(host)
    if not host:
        return ''
    cert=Path('/etc/ipsec.d/certs/ironpanel-ikev2-server.crt')
    key=Path('/etc/ipsec.d/private/ironpanel-ikev2-server.key')
    cert.parent.mkdir(parents=True, exist_ok=True); key.parent.mkdir(parents=True, exist_ok=True)
    le_cert=Path(f'/etc/letsencrypt/live/{host}/fullchain.pem')
    le_key=Path(f'/etc/letsencrypt/live/{host}/privkey.pem')
    if le_cert.exists() and le_key.exists() and sh(f'openssl x509 -in {le_cert} -noout >/dev/null 2>&1 && openssl pkey -in {le_key} -noout >/dev/null 2>&1; echo $?', timeout=10) == '0':
        try:
            shutil.copy2(le_cert, cert); shutil.copy2(le_key, key); os.chmod(key,0o600); os.chmod(cert,0o644)
            return 'l2tp:ikev2-cert=letsencrypt'
        except Exception:
            pass
    valid = sh(f'openssl x509 -in {cert} -noout >/dev/null 2>&1 && openssl pkey -in {key} -noout >/dev/null 2>&1; echo $?', timeout=10) == '0'
    if valid:
        return ''
    san=f'DNS:{host}'
    if re.match(r'^\d{1,3}(?:\.\d{1,3}){3}$', host): san=f'IP:{host}'
    rc=sh(f'openssl req -x509 -newkey rsa:3072 -sha256 -nodes -days 825 -keyout {key} -out {cert} -subj /CN={host} -addext subjectAltName={san} -addext keyUsage=digitalSignature,keyEncipherment -addext extendedKeyUsage=serverAuth >/dev/null 2>&1; echo $?', timeout=40)
    try: os.chmod(key,0o600); os.chmod(cert,0o644)
    except Exception: pass
    return 'l2tp:ikev2-cert=self-signed' if rc == '0' else 'l2tp:ikev2-cert=failed'


def _normalize_l2tp_ikev2_node_identity(host):
    host=_sanitize_host(host)
    if not host:
        return []
    out=[]
    c=_ensure_node_ikev2_cert(host)
    if c: out.append(c)
    conf=Path('/etc/ipsec.conf')
    if conf.exists():
        try:
            txt=conf.read_text(encoding='utf-8', errors='ignore')
            new=re.sub(r'^\s*leftid\s*=\s*@?[^\n]+', f'  leftid=@{host}', txt, flags=re.M)
            if 'leftcert=' in new:
                new=re.sub(r'^\s*leftcert\s*=\s*[^\n]+', '  leftcert=/etc/ipsec.d/certs/ironpanel-ikev2-server.crt', new, flags=re.M)
            if new != txt:
                conf.write_text(new, encoding='utf-8')
                CONFIG_CHANGED_PATHS.append(str(conf))
                out.append(f'l2tp:/etc/ipsec.conf:leftid=@{host}')
        except Exception as exc:
            out.append('l2tp:ikev2-leftid-error='+str(exc)[:120])
    return out

def _ensure_node_usage_runtime(protocols):
    """Install safe accounting hooks and private loopback stats endpoints."""
    protos=set(protocols or []); changed=[]
    scripts=Path('/opt/ironpanel-node/scripts')
    compat=Path('/opt/ironpanel/scripts'); compat.mkdir(parents=True,exist_ok=True)
    for name in ('ironpanel_usage_event_hook.py','ocserv_usage_disconnect.sh','install_ppp_usage_hooks.sh'):
        src=scripts/name; dst=compat/name
        try:
            if src.is_file() and (not dst.exists() or src.read_bytes()!=dst.read_bytes()):
                shutil.copy2(src,dst); os.chmod(dst,0o755); changed.append('usage-hook:'+str(dst))
        except Exception: pass
    try:
        Path('/var/lib/ironpanel/usage-events').mkdir(parents=True,exist_ok=True)
        os.chmod('/var/lib/ironpanel/usage-events',0o700)
    except Exception: pass
    if 'ocserv' in protos:
        cfg=Path('/etc/ocserv/ocserv.conf')
        try:
            txt=cfg.read_text(errors='ignore') if cfg.exists() else ''
            txt=re.sub(r'^\s*(?:connect|disconnect)-script\s*=.*ocserv_session_hook\.sh.*\n?','',txt,flags=re.M)
            line='disconnect-script = /opt/ironpanel/scripts/ocserv_usage_disconnect.sh'
            if line not in txt:
                txt=txt.rstrip()+'\n'+line+'\n'; cfg.write_text(txt); changed.append('ocserv:usage-hook')
        except Exception: pass
    if protos.intersection({'l2tp','pptp'}):
        try:
            installer=compat/'install_ppp_usage_hooks.sh'
            if installer.exists(): sh(f'APP_DIR=/opt/ironpanel bash {installer} >/dev/null 2>&1 || true',timeout=15)
        except Exception: pass
        for proto,path in (('l2tp','/etc/ppp/options.xl2tpd'),('pptp','/etc/ppp/pptpd-options')):
            if proto not in protos: continue
            try:
                pp=Path(path); txt=pp.read_text(errors='ignore') if pp.exists() else ''
                if txt and not re.search(r'^\s*ipparam\s+',txt,re.M):
                    pp.write_text(txt.rstrip()+f'\nipparam ironpanel-{proto}\n'); changed.append(proto+':ipparam')
            except Exception: pass
    if 'hysteria2' in protos:
        secret_path=Path('/etc/ironpanel/hysteria2_stats_secret')
        try:
            ...
            for raw in ('/etc/hysteria/config.yaml','/etc/hysteria2/config.yaml','/etc/hysteria/server.yaml','/etc/hysteria2/server.yaml'):
                ...
                block=f'trafficStats:\n  listen: 127.0.0.1:9999\n  secret: {secret}\n'
                if re.search(r'^trafficStats:\s*$',txt,re.M):
                    new=re.sub(r'^trafficStats:\s*\n(?:[ \t]+.*\n)*',block,txt,count=1,flags=re.M)
                else: new=block+txt
                if new!=txt: cfg.write_text(new); changed.append('hysteria2:trafficStats')
                break
        except Exception: pass
    return changed

def _normalize_node_protocol_configs(protocols, bundle=None, direct_ports=None, ssl_domain=None):
    protos=set(protocols or []); changed=[]; main_hosts=_main_host_set_from_bundle(bundle)
    changed.extend(_ensure_node_usage_runtime(protos))
    if 'xray' in protos:
    ...
    if 'l2tp' in protos:
        changed.extend(_normalize_l2tp_ikev2_node_identity(ssl_domain or os.environ.get('IRONPANEL_NODE_SSL_DOMAIN') or os.environ.get('IRONPANEL_CONFIG_DOMAIN') or ''))
    changed.extend(_normalize_node_direct_ports(protos, direct_ports or {}))
    return changed

def _active_connection_count_for_proto(proto):
    ports=[]
    try:
        if proto == 'openvpn':
            text=_config_text(['/etc/openvpn/server/server.conf','/etc/openvpn/server.conf'])
            ports.append(_direct_port_for('openvpn') or _text_port(text, r'^\s*port\s+(\d+)',1194))
        elif proto == 'ocserv':
            text=_config_text(['/etc/ocserv/ocserv.conf'])
            ports.append(_direct_port_for('ocserv') or _text_port(text, r'^\s*tcp-port\s*=\s*(\d+)',8445))
        elif proto == 'xray':
            ports.extend(_xray_public_ports())
        elif proto == 'hysteria2':
            ports.append(_hysteria_config_port(4433))
        elif proto == 'telegram_proxy':
            ports.append(_direct_port_for('telegram_proxy') or _json_config_port(['/opt/ironpanel-telegram-proxy/ironpanel/config.json','/etc/ironpanel/telegram_proxy.json','/etc/ironpanel/tgproxy.json'],6969))
        elif proto == 'wireguard':
            # WireGuard peers are connectionless. Use transfer/state info as a soft signal.
            return int(sh("wg show 2>/dev/null | grep -c '^peer:' || true") or 0)
    except Exception:
        ports=[]
    total=0
    for port in [p for p in ports if int(p or 0) > 0]:
        try:
            total += int(sh(f"ss -Hnt sport = :{int(port)} 2>/dev/null | wc -l || true") or 0)
            total += int(sh(f"ss -Hnt dport = :{int(port)} 2>/dev/null | wc -l || true") or 0)
        except Exception:
            pass
    return total


def _restart_protocols(protocols, *, allow_busy_restart=False):
    sh('systemctl daemon-reload 2>/dev/null || true', timeout=20)
    mapping={
      'openvpn':['openvpn-server@server','openvpn'], 'wireguard':['wg-quick@wg0','wg-quick@wg-ironpanel'],
      'ocserv':['ocserv'], 'l2tp':['strongswan-starter','strongswan','xl2tpd'], 'xray':['xray'], 'pptp':['pptpd'],
      'hysteria2':['hysteria-server','hysteria2-server','hysteria-server@server','hysteria','hysteria2'], 'telegram_proxy':['ironpanel-tgproxy'], 'ssh':['ssh','sshd']}
    touched=[]; skipped=[]
    for proto in protocols or []:
        busy=_active_connection_count_for_proto(proto)
        if busy > 0 and not allow_busy_restart:
            skipped.append(f'{proto}:busy={busy}')
            continue
        for svc in mapping.get(proto,[]):
            sh(f'systemctl try-reload-or-restart {svc} 2>/dev/null || systemctl restart {svc} 2>/dev/null || true', timeout=20)
            touched.append(svc)
    if skipped:
        try:
            (STATE/'pending_protocol_restarts.json').write_text(json.dumps({'updated_at':time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'skipped':skipped}, ensure_ascii=False, indent=2), encoding='utf-8')
        except Exception:
            pass
        touched.extend(['SKIPPED-'+x for x in skipped])
    return touched


def _changed_protocols_from_paths(paths, normalized=None, selected=None):
    selected=set(_selected_protocols(selected or [])) if selected else set(PROTOCOL_SERVICES)
    protos=[]
    def add(proto):
        if proto in selected and proto not in protos: protos.append(proto)
    auth_only={'/etc/ocserv/ocpasswd','/etc/ironpanel/ocpasswd','/etc/ppp/chap-secrets'}
    for raw in list(paths or []):
        path=str(raw or '')
        if path in auth_only or path.endswith('/ocpasswd') or path.endswith('/chap-secrets'):
            continue
        if path.startswith('/etc/openvpn'): add('openvpn')
        elif path.startswith('/etc/wireguard'): add('wireguard')
        elif path.startswith('/etc/ocserv'): add('ocserv')
        elif path.startswith('/etc/xray') or path.startswith('/usr/local/etc/xray'): add('xray')
        elif path.startswith('/etc/hysteria') or path.startswith('/etc/hysteria2'): add('hysteria2')
        elif path.startswith('/etc/xl2tpd') or path.startswith('/etc/ipsec.d') or path in ('/etc/ipsec.conf','/etc/ipsec.secrets') or path.startswith('/etc/strongswan'): add('l2tp')
        elif path == '/etc/pptpd.conf': add('pptp')
        elif path.startswith('/opt/ironpanel-telegram-proxy') or path in ('/etc/ironpanel/telegram_proxy.json','/etc/ironpanel/tgproxy.json','/etc/systemd/system/ironpanel-tgproxy.service'): add('telegram_proxy')
        elif path.startswith('/etc/ssh/sshd_config'): add('ssh')
    for raw in normalized or []:
        proto=str(raw).split(':',1)[0]
        add(proto)
    return protos


def _apply_config_bundle(bundle):
    global CONFIG_CHANGED_PATHS
    CONFIG_CHANGED_PATHS=[]
    ...
    for item in files:
        ...
        try:
            ...
            if target.exists() and target.is_file():
                try: same=(target.read_bytes() == data)
            ...
            written.append(path); CONFIG_CHANGED_PATHS.append(path)
        except Exception as e:
            skipped.append(path+': '+str(e))
    return f'config files written={len(written)} unchanged={len(unchanged)} skipped={len(skipped)} backup={backup}\n' + ('\n'.join(skipped[-30:]) if skipped else '')
...
def _rebuild_plain_auth_from_metadata(protocols):
    ...
    if selected.intersection({'l2tp','pptp'}):
        ...
        # IKEv2-EAP uses /etc/ipsec.secrets, not chap-secrets. Keep EAP users
        # updated during hot user sync so node L2TP/IKEv2 does not require a full
        # core/config restart after every user change. Preserve PSK/RSA lines from
        # the current file and replace only generated EAP entries.
        if 'l2tp' in selected:
            ...
            if not any(': PSK ' in line for line in keep):
                psk=Path('/etc/ironpanel/ipsec.psk')
                secret=(psk.read_text(errors='ignore').strip() if psk.exists() else 'managed-by-ironpanel')
                keep.insert(0, f'%any %any : PSK "{secret}"\n')
            if not any(': RSA ' in line for line in keep):
                keep.append(': RSA /etc/ipsec.d/private/ironpanel-ikev2-server.key\n')
            for item in rows:
                if str(item.get('protocol') or '') != 'l2tp' or not bool(item.get('enabled', True)):
                    continue
                username=_clean_auth(item.get('username'),'user').replace('"','\\"')
                password=_clean_auth(item.get('l2tp_password') or item.get('cisco_password')).replace('"','\\"')
                keep.append(f'"{username}" : EAP "{password}"\n')
            tmpsec=sec.with_name('.ipsec.secrets.ironpanel-node.tmp')
            tmpsec.write_text(''.join(keep), encoding='utf-8'); os.chmod(tmpsec,0o600); tmpsec.replace(sec)
            details.append('ikev2-eap-users=synced')
...
def handle_job(job):
    ...
    if action == 'ensure_protocols':
        ...
        rc,out=_run_node_core_install(','.join(protos), payload.get('direct_ports') or {}, _payload_ssl_domain(payload))
    ...
    if action == 'sync_protocol_configs':
        ...
        ssl_domain=_payload_ssl_domain(payload)
        # v19.10.10: Do not reinstall/restart all protocol cores on every user
        # sync. First ensure only missing binaries/services, then write only changed
        # config files and restart only the protocols whose real runtime config
        # changed. Auth/user files are hot-synced without restarting cores.
        pre_health=protocol_health()
        missing_before=_missing_installed_cores(protos, pre_health)
        if missing_before:
            pre_rc,before=_run_node_core_install(','.join(missing_before), direct_ports, ssl_domain)
        else:
            pre_rc,before=0,'all selected protocol cores already installed; skipped core reinstall'
        out=_apply_config_bundle(payload.get('bundle') or {})
        normalized=_normalize_node_protocol_configs(protos, payload.get('bundle') or {}, direct_ports, ssl_domain or payload.get('config_domain') or '')
        auth_ok, auth_detail, auth_errors=_rebuild_plain_auth_from_metadata(protos)
        changed_protos=_changed_protocols_from_paths(CONFIG_CHANGED_PATHS, normalized, protos)
        if changed_protos:
            restarted=_restart_protocols(changed_protos, allow_busy_restart=bool(payload.get('force_restart') or False))
            if 'telegram_proxy' in changed_protos:
                time.sleep(1.5)
        else:
            restarted=[]
        post_rc,after=0,'core reinstall skipped; no missing cores after pre-check'
        ph=protocol_health(); missing=_missing_installed_cores(protos, ph)
        ...
        _node_sync_state({'protocols':protos, 'protocol_health':ph, 'sync_ok':ok, 'missing':missing, 'inactive':inactive, 'unhealthy':unhealthy, 'changed_protocols':changed_protos, 'restarted':restarted})
        norm_text='\nnormalized:\n' + ('\n'.join(normalized) if normalized else 'no bind changes needed') + (('\nssl-domain: '+ssl_domain) if ssl_domain else '')
        auth_text='\nauth-sync: '+auth_detail + ((' errors='+json.dumps(auth_errors, ensure_ascii=False)) if auth_errors else '')
        summary=f'\nverification: ok={ok} pre_exit={pre_rc} post_exit={post_rc} changed={",".join(changed_protos) or "none"} missing={",".join(missing) or "none"} inactive={",".join(inactive) or "none"} unhealthy={",".join(unhealthy) or "none"}'
        detail=('pre-install:\n'+before[-4000:]+'\nconfig-sync:\n'+out+norm_text+auth_text+
                '\npost-config ports/core:\n'+after[-4000:]+'\nrestarted: ' + (','.join(restarted) if restarted else 'none') + summary)
        return ok, detail, {'protocol_health': ph, 'online_users': int(sh("ss -ntu state established 2>/dev/null | tail -n +2 | wc -l") or 0), 'cpu_percent': _cpu_percent(), 'ram_percent': _ram_percent(), 'disk_percent': _disk_percent()}
