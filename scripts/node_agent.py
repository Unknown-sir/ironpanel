...
def handle_job(job):
    ...
    if action == 'sync_user':
        ...
        return True, f'user metadata synced: {path}', {}
    if action == 'sync_users_bulk':
        ...
        for item in users:
            if isinstance(item, dict):
                try: written.append(_write_user_metadata(item))
                except Exception as exc: written.append('ERR:'+str(exc)[:160])
        manifest={'count':len(written), 'protocols':payload.get('protocols') or [], 'reason':payload.get('reason') or '', 'written':written[-500:]}
        _node_sync_state({'users_count':len(written), 'protocols':payload.get('protocols') or []})
        return True, 'bulk users synced to node metadata store: '+json.dumps(manifest, ensure_ascii=False)[:12000], {'online_users': int(sh("ss -ntu state established 2>/dev/null | tail -n +2 | wc -l") or 0), 'cpu_percent': _cpu_percent(), 'ram_percent': _ram_percent(), 'disk_percent': _disk_percent()}
    ...
    if action == 'sync_protocol_configs':
        ...
        ph=protocol_health()
        _node_sync_state({'protocols':protos, 'protocol_health':ph})
        norm_text='\nnormalized:\n' + ('\n'.join(normalized) if normalized else 'no bind changes needed')
        return True, 'pre-install:\n'+before[-4000:]+'\nconfig-sync:\n'+out+norm_text+'\npost-config ports/core:\n'+after[-4000:]+'\nrestarted: ' + ','.join(restarted), {'protocol_health': ph, 'online_users': int(sh("ss -ntu state established 2>/dev/null | tail -n +2 | wc -l") or 0), 'cpu_percent': _cpu_percent(), 'ram_percent': _ram_percent(), 'disk_percent': _disk_percent()}
