...
def _load_direct_ports_raw(value=None):
    ...
    if isinstance(raw, dict):
        ...
        try:
            ...
            # v19.9.28: tolerate malformed values like {"telegram_proxy":6974}}
...
def _curl_post_json(base, token, path, data):
    # v19.9.28: urllib can time out on IP/SNI-mismatched TLS on some
...
def _changed_protocols_from_paths(paths, normalized=None, selected=None):
    ...
    for raw in list(paths or []):
        ...
        elif path.startswith('/etc/xl2tpd') or path in ('/etc/ipsec.conf','/etc/ipsec.secrets') or path.startswith('/etc/strongswan'): add('l2tp')
...
def handle_job(job):
    ...
    if action == 'sync_protocol_configs':
        ...
        # v19.9.28: Do not reinstall/restart all protocol cores on every user
