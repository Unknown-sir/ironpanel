...
def protocol_health():
    ...
    for proto, services in PROTOCOL_SERVICES.items():
        ...
        if installed and active_ok and proto == 'ocserv':
            ...
            port=_json_config_port(['/opt/ironpanel-telegram-proxy/ironpanel/config.json','/etc/ironpanel/telegram_proxy.json','/etc/ironpanel/tgproxy.json'], 6969)
            ...
            port=_text_port(text, r'(?:--port|PORT|IRONPANEL_TGPROXY_PORT|port)\D+(\d+)', port or 6969)
...
def _master_candidates(raw):
    ...
    if host:
        ...
        if scheme in ('http','https'):
            if port:
                # v19.9.13: custom panel ports may terminate HTTP or HTTPS.
...
def _coerce_direct_ports(direct_ports=None):
    ...
    if not data:
        try:
            ...
            data={}
    try:
        return {str(k): int(v) for k,v in (data or {}).items() if str(v).isdigit() or isinstance(v,int)}
    except Exception:
        return {}
