...
timeout 10 openssl dhparam -in /etc/openvpn/server/dh.pem -noout >/dev/null 2>&1 || { echo '[IronPanel] invalid OpenVPN DH parameters' >&2; exit 22; }
openssl verify -CAfile /etc/openvpn/server/ca.crt /etc/openvpn/server/server.crt >/dev/null 2>&1 || { echo '[IronPanel] OpenVPN server certificate is not signed by active CA' >&2; exit 22; }
CERT_HASH=$(openssl x509 -in /etc/openvpn/server/server.crt -pubkey -noout 2>/dev/null | openssl pkey -pubin -outform DER 2>/dev/null | sha256sum | awk '{print $1}')
KEY_HASH=$(openssl pkey -in /etc/openvpn/server/server.key -pubout -outform DER 2>/dev/null | sha256sum | awk '{print $1}')
[[ -n "$CERT_HASH" && "$CERT_HASH" == "$KEY_HASH" ]] || { echo '[IronPanel] OpenVPN server certificate/private-key mismatch' >&2; exit 22; }
grep -q '^-----BEGIN OpenVPN Static key V1-----$' /etc/openvpn/server/tls-crypt.key || { echo '[IronPanel] invalid OpenVPN tls-crypt key header' >&2; exit 22; }
grep -q '^-----END OpenVPN Static key V1-----$' /etc/openvpn/server/tls-crypt.key || { echo '[IronPanel] invalid OpenVPN tls-crypt key footer' >&2; exit 22; }
cmp -s /etc/openvpn/server/ca.crt /etc/openvpn/easy-rsa/pki/ca.crt || { echo '[IronPanel] OpenVPN runtime CA differs from canonical Easy-RSA CA' >&2; exit 22; }
