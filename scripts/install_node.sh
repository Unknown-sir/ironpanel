...
# v19.8.11: Generate master URL candidates in the same order the node can really use.
# Important: for raw IP + non-443 custom panel ports, prefer plain HTTP before HTTPS.
# Older installers accepted https://IP:8001 just because / or /ping appeared reachable;
# this version verifies the real heartbeat endpoint with the node token before saving.
...
if host:
    ...
    if scheme == 'https':
        if port:
            # Raw IP on custom port is normally IronPanel HTTP, not valid TLS.
            ...
            # https://IP without port from old panels: try common HTTP panel ports first.
            if ipish:
...
payload={
  'version':'19.8.11','agent_version':'19.8.11','public_ip':os.environ.get('IRONPANEL_PROBE_HOST') or '',
  'protocols':os.environ.get('IRONPANEL_PROBE_PROTOCOLS') or '', 'probe': True,
  'protocol_health':{}, 'cpu_percent':0, 'ram_percent':0, 'disk_percent':0,
...
def ctx_for(url):
    ...
    return None
try:
    req=urllib.request.Request(base + '/api/v2/node/heartbeat', data=json.dumps(payload).encode(), headers={'Content-Type':'application/json','X-NODE-TOKEN':token,'User-Agent':'IronPanel-Node-Installer/19.8.11'})
    ctx=ctx_for(base)
...
# Older master fallback: explicit node_api ping, using Python urllib too.
try:
    req=urllib.request.Request(base + '/api/v2/node/ping', headers={'User-Agent':'IronPanel-Node-Installer/19.8.11'})
    ctx=ctx_for(base)
    r=urllib.request.urlopen(req, timeout=8, context=ctx) if ctx else urllib.request.urlopen(req, timeout=8)
    body=r.read().decode(errors='ignore')[:1000]
    print('ping', r.status, body[:300])
    if 'node_api' in body and 'ironpanel-master' in body:
        sys.exit(0)
except Exception as e:
    print('ping-' + type(e).__name__ + ': ' + str(e))
sys.exit(1)
PYPROBE
  rc=$?
  if [[ $rc -eq 0 ]]; then
...
if [[ -z "$RESOLVED_MASTER" ]]; then
  ...
  log "No verified master endpoint found. Service will still be installed with provided URL, but node will stay offline until master URL/firewall/SSL is fixed: $RESOLVED_MASTER"
fi
...
systemctl restart ironpanel-node.service 2>/dev/null || systemctl start ironpanel-node.service
