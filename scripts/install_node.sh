...
payload={
  'version':'19.8.19','agent_version':'19.8.19','public_ip':os.environ.get('IRONPANEL_PROBE_HOST') or '',
...
def ctx_for(url):
    ...
    return None
try:
    req=urllib.request.Request(base + '/api/v2/node/heartbeat', data=json.dumps(payload).encode(), headers={'Content-Type':'application/json','X-NODE-TOKEN':token,'User-Agent':'IronPanel-Node-Installer/19.8.19'})
    ctx=ctx_for(base)
...
try:
    req=urllib.request.Request(base + '/api/v2/node/ping', headers={'User-Agent':'IronPanel-Node-Installer/19.8.19'})
    ctx=ctx_for(base)
    r=urllib.request.urlopen(req, timeout=8, context=ctx) if ctx else urllib.request.urlopen(req, timeout=8)
...
if [[ "$INSTALL_CORES" == "1" ]]; then
  ...
  if [[ -x "$APP_DIR/scripts/install_node_cores.sh" ]]; then
    bash "$APP_DIR/scripts/install_node_cores.sh" "$PROTOCOLS" || log "Core installation finished with warnings. Check /var/log/ironpanel-node-core-install.log"
  else
    log "install_node_cores.sh not found; skipping core installation"
  fi
fi
...
if [[ $rc -ne 0 && $rc2 -ne 0 ]]; then
  log "One-shot heartbeat/sync failed. Check: journalctl -u ironpanel-node -n 100 --no-pager"
else
  log "One-shot heartbeat completed and initial core/config/user sync jobs were requested. The node should become Online in the main panel."
fi
