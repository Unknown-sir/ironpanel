...
if host:
    ...
    if scheme in ('http','https'):
        if port:
            # v19.9.13: explicit custom panel ports may be either HTTP or HTTPS.
...
payload={
  'version':'19.9.13','agent_version':'19.9.13','public_ip':os.environ.get('IRONPANEL_PROBE_HOST') or '',
...
def ctx_for(url):
    ...
    if insecure or re.match(r'^https://\d+\.\d+\.\d+\.\d+(:\d+)?(/|$)', url, re.I):
        return ssl._create_unverified_context()
    return None
try:
    req=urllib.request.Request(base + '/api/v2/node/heartbeat', data=json.dumps(payload).encode(), headers={'Content-Type':'application/json','X-NODE-TOKEN':token,'User-Agent':'IronPanel-Node-Installer/19.9.13'})
    ctx=ctx_for(base)
...
for candidate in "${MASTER_CANDIDATES[@]}"; do
  ...
  if probe_master "$candidate" 0; then
    ...
    break
  elif grep -Eqi 'CERTIFICATE_VERIFY_FAILED|certificate verify failed|self[- ]signed certificate' /tmp/ironpanel-node-probe.out /tmp/ironpanel-node-probe.err 2>/dev/null && probe_master "$candidate" 1; then
    RESOLVED_MASTER="$candidate"
    INSECURE_TLS="1"
    log "Master heartbeat verified with a self-signed/untrusted certificate; node TLS fallback enabled for this URL."
    break
  else
