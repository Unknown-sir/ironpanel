#!/usr/bin/env bash
set -euo pipefail
MASTER=""; TOKEN=""; HOST=""; PROTOCOLS=""; NODE_NAME=""; SSL_DOMAIN=""; DIRECT_PORTS_JSON="{}"; INSECURE_TLS="0"; INSTALL_CORES="1"
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ssl-domain|--node-ssl-domain) SSL_DOMAIN="${2:-}"; shift 2;;
...
normalize_ssl_domain(){
  python3 - "$1" "$2" <<'PYSSLHOST'
import re, sys
raw=(sys.argv[1] or sys.argv[2] or '').strip()
...
SSL_DOMAIN="$(normalize_ssl_domain "$SSL_DOMAIN" "$HOST")"
normalize_direct_ports_json(){
  python3 - "$1" <<'PYDIRECTJSON'
import json, re, sys
raw = (sys.argv[1] if len(sys.argv) > 1 else '{}') or '{}'
raw = str(raw).strip()
# v19.10.10: tolerate bad legacy payloads like {"telegram_proxy":6974}}}
# but always store/pass a compact valid JSON object.
def try_load(s):
    try:
        data = json.loads(s)
...
if data is None:
    ...
    while s and s.count('}') > s.count('{'):
        ...
        if data is not None:
            break
if data is None:
    data = {m.group(1): int(m.group(2)) for m in re.finditer(r"[\'\"]?([A-Za-z0-9_]+)[\'\"]?\s*:\s*([0-9]{1,5})", raw)}
out = {}
...
DIRECT_PORTS_JSON="$(normalize_direct_ports_json "$DIRECT_PORTS_JSON")"
APP_DIR=/opt/ironpanel-node
STATE_DIR=/etc/ironpanel-node

preclean_existing_node_runtime(){
  if [[ "${IRONPANEL_NODE_SKIP_PRECLEAN:-0}" == "1" ]]; then
    log "Skipping existing node cleanup because IRONPANEL_NODE_SKIP_PRECLEAN=1"
    return 0
  fi
  log "Stopping and removing previous IronPanel node/panel runtime services if present"
  local units=(
    ironpanel-node.service ironpanel-node-agent.service ironpanel-node-gateway.service
    ironpanel.service ironpanel-web.service ironpanel-sales-bot.service ironpanel-usage-sync.service
    openvpn-server@server.service openvpn.service ocserv.service wg-quick@wg0.service wg-quick@wg-ironpanel.service
    xray.service hysteria-server.service hysteria2-server.service hysteria-server@server.service ironpanel-tgproxy.service
    strongswan.service strongswan-starter.service xl2tpd.service pptpd.service
  )
  local u
  for u in "${units[@]}"; do
  ...
  pkill -f '/opt/ironpanel-node/node_agent.py' >/dev/null 2>&1 || true
  pkill -f 'ironpanel_mtproxy.js' >/dev/null 2>&1 || true
  rm -f /etc/systemd/system/ironpanel-node.service /etc/systemd/system/ironpanel-node-agent.service /etc/systemd/system/ironpanel-node-gateway.service
  rm -f /etc/systemd/system/hysteria-server.service /etc/systemd/system/hysteria2-server.service /etc/systemd/system/hysteria-server@server.service
...
payload={
  'version':'19.10.16','agent_version':'19.10.16','public_ip':os.environ.get('IRONPANEL_PROBE_HOST') or '',
...
def ctx_for(url):
    ...
    return None
try:
    req=urllib.request.Request(base + '/api/v2/node/heartbeat', data=json.dumps(payload).encode(), headers={'Content-Type':'application/json','X-NODE-TOKEN':token,'User-Agent':'IronPanel-Node-Installer/19.10.16'})
    ctx=ctx_for(base)
...
PYPROBE
  ...
  # v19.10.10: Some hardened panel TLS/proxy stacks allow the authenticated
  # package download with curl -k but Python urllib times out during the same
  # TLS handshake. Use curl as an equivalent heartbeat probe before rejecting
  # the endpoint, so node installation does not fail after a valid package pull.
  if command -v curl >/dev/null 2>&1; then
    local curl_tls=() code payload
    if [[ "$insecure" == "1" && "$base" =~ ^https:// ]]; then curl_tls=(-k); fi
    payload=$(IRONPANEL_PROBE_HOST="$HOST" IRONPANEL_PROBE_PROTOCOLS="$PROTOCOLS" python3 - <<'PYCURLPAYLOAD'
import json, os
print(json.dumps({
  'version':'19.10.16','agent_version':'19.10.16','public_ip':os.environ.get('IRONPANEL_PROBE_HOST') or '',
  'protocols':os.environ.get('IRONPANEL_PROBE_PROTOCOLS') or '', 'probe': True,
...
PYCURLPAYLOAD
    ...
    code=$(curl -sS "${curl_tls[@]}" --connect-timeout 8 --max-time 25 \
      -o /tmp/ironpanel-node-probe.out -w '%{http_code}' \
      -H 'Content-Type: application/json' -H "X-NODE-TOKEN: $TOKEN" \
      -H 'User-Agent: IronPanel-Node-Installer/19.10.16-curl' \
      --data-binary "$payload" "${base%/}/api/v2/node/heartbeat" 2>/tmp/ironpanel-node-probe.err || true)
    if [[ "$code" == "200" ]]; then
      echo "heartbeat-ok-curl $code" >>/tmp/ironpanel-node-probe.out
      return 0
    fi
    if [[ "$code" == "401" || "$code" == "403" ]]; then
      echo "heartbeat-http-$code $(cat /tmp/ironpanel-node-probe.out 2>/dev/null | head -c 300)" > /tmp/ironpanel-node-probe.auth
      return 1
    fi
    echo "curl-heartbeat-http-$code $(cat /tmp/ironpanel-node-probe.err 2>/dev/null | tail -n 1)" >>/tmp/ironpanel-node-probe.out
  fi
  return 1
}
RESOLVED_MASTER=""
...
export IRONPANEL_NODE_SSL_DOMAIN="$SSL_DOMAIN"
...
items={
 ...
 'IRONPANEL_NODE_SSL_DOMAIN': os.environ.get('IRONPANEL_NODE_SSL_DOMAIN',''),
...
if [[ "$INSTALL_CORES" == "1" ]]; then
  ...
  if [[ -x "$APP_DIR/scripts/install_node_cores.sh" ]]; then
    if ! IRONPANEL_NODE_DIRECT_PORTS_JSON="$DIRECT_PORTS_JSON" IRONPANEL_NODE_SSL_DOMAIN="$SSL_DOMAIN" bash "$APP_DIR/scripts/install_node_cores.sh" "$PROTOCOLS"; then
