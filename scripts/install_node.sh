...
raw = str(raw).strip()
# v19.9.30: tolerate bad legacy payloads like {"telegram_proxy":6974}}}
...
payload={
  'version':'19.9.30','agent_version':'19.9.30','public_ip':os.environ.get('IRONPANEL_PROBE_HOST') or '',
...
def ctx_for(url):
    ...
    return None
try:
    req=urllib.request.Request(base + '/api/v2/node/heartbeat', data=json.dumps(payload).encode(), headers={'Content-Type':'application/json','X-NODE-TOKEN':token,'User-Agent':'IronPanel-Node-Installer/19.9.30'})
    ctx=ctx_for(base)
...
PYPROBE
  ...
  # v19.9.30: Some hardened panel TLS/proxy stacks allow the authenticated
...
PYCURLPAYLOAD
    ...
    code=$(curl -sS "${curl_tls[@]}" --connect-timeout 8 --max-time 25 \
      ...
      -H 'User-Agent: IronPanel-Node-Installer/19.9.30-curl' \
