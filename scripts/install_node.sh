...
raw = str(raw).strip()
# v19.9.18: tolerate bad legacy payloads like {"telegram_proxy":6974}}}
...
payload={
  'version':'19.9.18','agent_version':'19.9.18','public_ip':os.environ.get('IRONPANEL_PROBE_HOST') or '',
...
def ctx_for(url):
    ...
    return None
try:
    req=urllib.request.Request(base + '/api/v2/node/heartbeat', data=json.dumps(payload).encode(), headers={'Content-Type':'application/json','X-NODE-TOKEN':token,'User-Agent':'IronPanel-Node-Installer/19.9.18'})
    ctx=ctx_for(base)
