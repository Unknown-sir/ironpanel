...
payload={
  'version':'19.8.16','agent_version':'19.8.16','public_ip':os.environ.get('IRONPANEL_PROBE_HOST') or '',
...
def ctx_for(url):
    ...
    return None
try:
    req=urllib.request.Request(base + '/api/v2/node/heartbeat', data=json.dumps(payload).encode(), headers={'Content-Type':'application/json','X-NODE-TOKEN':token,'User-Agent':'IronPanel-Node-Installer/19.8.16'})
    ctx=ctx_for(base)
...
try:
    req=urllib.request.Request(base + '/api/v2/node/ping', headers={'User-Agent':'IronPanel-Node-Installer/19.8.16'})
    ctx=ctx_for(base)
    r=urllib.request.urlopen(req, timeout=8, context=ctx) if ctx else urllib.request.urlopen(req, timeout=8)
