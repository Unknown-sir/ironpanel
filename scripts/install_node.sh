...
payload={
  'version':'19.10.15','agent_version':'19.10.15','public_ip':os.environ.get('IRONPANEL_PROBE_HOST') or '',
...
def ctx_for(url):
    ...
    return None
try:
    req=urllib.request.Request(base + '/api/v2/node/heartbeat', data=json.dumps(payload).encode(), headers={'Content-Type':'application/json','X-NODE-TOKEN':token,'User-Agent':'IronPanel-Node-Installer/19.10.15'})
    ctx=ctx_for(base)
...
PYCURLPAYLOAD
    ...
    code=$(curl -sS "${curl_tls[@]}" --connect-timeout 8 --max-time 25 \
      ...
      -H 'User-Agent: IronPanel-Node-Installer/19.10.15-curl' \
