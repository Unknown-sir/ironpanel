...
elif ! exists_nonempty /etc/openvpn/easy-rsa/pki/issued/server.crt || ! exists_nonempty /etc/openvpn/easy-rsa/pki/private/server.key || ! exists_nonempty /etc/openvpn/easy-rsa/pki/dh.pem; then
  add openvpn failed 'canonical Easy-RSA server identity or DH parameters are incomplete'
elif ! openssl verify -CAfile /etc/openvpn/easy-rsa/pki/ca.crt /etc/openvpn/easy-rsa/pki/issued/server.crt >/dev/null 2>&1; then
  add openvpn failed 'canonical Easy-RSA server certificate is not signed by canonical CA'
