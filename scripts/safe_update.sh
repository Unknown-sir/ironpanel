...
mkdir -p "$(dirname "$LOG")" "$(dirname "$STATE")" /var/backups/ironpanel
exec > >(tee -a "$LOG") 2>&1
...
progress 15 "Creating full backup"
...
bash "$APP_DIR/scripts/update_from_github.sh"

progress 82 "Database migration and service reconciliation"
cd "$APP_DIR"
if [[ -x "$APP_DIR/.venv/bin/flask" ]]; then
  "$APP_DIR/.venv/bin/flask" --app run.py upgrade-db || true
fi
systemctl daemon-reload || true

progress 90 "Running Health Doctor repair"
if [[ -x "$APP_DIR/scripts/ironpanel_doctor.sh" ]]; then
  bash "$APP_DIR/scripts/ironpanel_doctor.sh" --repair || true
fi
if [[ -x "$APP_DIR/scripts/apply_speed_limits.sh" ]]; then bash "$APP_DIR/scripts/apply_speed_limits.sh" --apply || true; fi
if [[ -x "$APP_DIR/scripts/apply_node_gateway.sh" ]]; then bash "$APP_DIR/scripts/apply_node_gateway.sh" --apply || true; fi
