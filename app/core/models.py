...
class Admin(UserMixin, db.Model):
    ...
    # v19.10.17: protocols a reseller is allowed to assign to owned users/plans.
    reseller_protocols = db.Column(db.String(160), default='openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2,telegram_proxy,ssh')
    ...
    # v19.10.23: why a reseller is currently disabled. Empty means enabled/no suspension.
    # manual, traffic_quota, user_limit are the managed values.
    disabled_reason = db.Column(db.String(40), default='')
    ...
    def allowed_protocol_list(self):
        raw = self.reseller_protocols or ''
...
class VpnUser(db.Model):
    ...
    enabled = db.Column(db.Boolean, default=True)
    # v19.10.23: durable state reason so reseller suspension is reversible without
    # accidentally re-enabling expired, over-quota or manually disabled accounts.
    disabled_reason = db.Column(db.String(48), default='')
    ...
    # v19.10.21: one user-wide Mbps cap. 0 = no user-wide override.
    # When set, it takes precedence over protocol defaults/overrides for this user.
    speed_limit_mbps = db.Column(db.Integer, default=0)
...
class Node(db.Model):
    ...
    # v19.9.8: rebuilt node model aliases for simpler Direct Location workflow.
    server_name = db.Column(db.String(160), default='')
    config_domain = db.Column(db.String(255), default='')
    # v19.9.24: certificate/ACME domain that must resolve directly to the node IP.
    ssl_domain = db.Column(db.String(255), default='')
