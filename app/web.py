...
def nodes():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('web.nodes'))
    return render_template('nodes.html', nodes=Node.query.order_by(Node.name).all(), summary=node_sync_status_summary())
...
def node_sync(node_id):
    ...
    for u in VpnUser.query.filter_by(enabled=True).all():
        for p in [x for x in (u.protocols or '').split(',') if x and x in (node.protocols or '')]:
            queue_node_job(node.id, 'sync_user', {'user_id':u.id,'username':u.username,'protocol':p,'enabled':bool(u.enabled),'data_limit_mb':int(u.data_limit_mb or 0)})
            count += 1
    flash(f'{count} job برای sync کاربران روی {node.name} ساخته شد')
    return redirect(url_for('web.nodes'))
...
def node_ensure(node_id):
    if current_user.role != 'main_admin' or not feature_allowed('nodes'):
        return redirect(url_for('web.dashboard'))
    node = Node.query.get_or_404(node_id)
    queue_node_job(node.id, 'ensure_protocols', {'protocols': node.protocols})
    queue_node_health_check(node.id)
    flash('Job نصب/Repair هسته‌ها و Health Check برای نود ساخته شد')
    return redirect(url_for('web.nodes'))
