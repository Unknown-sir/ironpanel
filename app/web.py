...
def dashboard_quick_upgrade():
    ...
    if not latest.get('update_available'):
        ...
        return redirect(url_for('web.dashboard'))
    flash('نسخه جدید موجود است. برای آپدیت مرحله‌ای با درصد پیشرفت، از صفحه Update Manager استفاده کن.')
...
def updates():
    ...
    if request.method=='POST':
        ...
        if action == 'process_jobs':
            ...
            flash('برای آپدیت جدید، دکمه شروع آپدیت مرحله‌ای را بزنید تا درصد پیشرفت در همین صفحه نمایش داده شود.')
        else:
