...
def settings():
    ...
    if request.method == 'POST':
        ...
        if section == 'wireguard':
            raw_mtu = request.form.get('wireguard_mtu', '1360')
            raw_keepalive = request.form.get('wireguard_persistent_keepalive', '25')
            try:
                mtu = max(576, min(int(raw_mtu or 1360), 1500))
            except Exception:
                mtu = 1360
            try:
            ...
            try:
                ...
                return redirect(url_for('web.settings'))
            log(current_user.username, 'update_wireguard_client_mtu', str(mtu), f'keepalive={keepalive}')
            flash(f'WireGuard MTU saved: {mtu}. User configs will now include this MTU.')
            return redirect(url_for('web.settings'))
...
def subscription(token):
    ...
    collect_usage_from_runtime()
    configs = user_config_payload(u)
    return render_template('subscription.html', user=u, host=get_public_host(), subscription_base_url=get_subscription_base_url(), subscription_url=subscription_url_for_user(u), configs=configs, usage=user_usage_summary(u), theme=subscription_theme_settings())
