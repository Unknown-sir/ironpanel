...
from .services.provisioning import sync_user, delete_user, log, get_setting, set_setting, get_port, get_public_host, active_protocols, user_config_payload, apply_runtime_configs, user_access_status, sync_all_users
...
def users():
    if request.method == 'POST':
        ...
        u = VpnUser(username=username, l2tp_password=request.form.get('l2tp_password') or password, cisco_password=request.form.get('cisco_password') or password, data_limit_mb=int(request.form.get('data_limit_mb') or 0), connection_limit=int(request.form.get('connection_limit') or 1), protocols=','.join(protocols), expires_at=expires_at, owner_id=current_user.id if current_user.role=='sub_admin' else None)
...
def settings():
    ...
    if request.method == 'POST':
        ...
        for key in ['public_host','tunnel_host','active_protocols','openvpn_transport','ocserv_transport','wireguard_transport','l2tp_transport']:
