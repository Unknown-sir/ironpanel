...
from .services.license import check_license, save_license_key, clear_license_key, license_key, license_server_url, license_remaining_days, feature_allowed, feature_label, current_license_type, current_license_features, paid_license_active, is_free_edition, filter_protocols_for_license, allowed_protocols_for_license
...
from .services.speed_limit import speed_limit_rows, save_speed_limits, apply_speed_limits_runtime, speed_limit_status, PROTOCOL_LABELS, PROTOCOL_ICONS, speed_limit_user_matrix, save_user_speed_limits
from .services.i18n import t, current_language, current_theme, language_dir, LANGUAGES, THEMES, save_appearance
from .services.maintenance import doctor_status, doctor_repair, backup_catalog, create_safe_backup, restore_safe_backup, safe_updater_terminal_command
from .services.firewall_manager import apply_firewall_rules, create_ip_ban, firewall_summary, normalize_ip_target, ip_matches_target
from .services.node_auto_installer import node_auto_installer_allowed, node_has_saved_ssh_credentials, save_node_ssh_credentials, clear_node_ssh_credentials, run_auto_node_install

web_bp = Blueprint('web', __name__)


# v19.4 owner-aware reseller sales bot helpers
SALES_BOT_SETTING_KEYS = ['sales_bot_enabled','sales_bot_token','sales_bot_admin_ids','sales_bot_support_url','sales_bot_welcome_text','sales_bot_payment_text','sales_bot_rules_text','sales_bot_connection_guide','sales_bot_qr_enabled','sales_bot_trial_enabled','sales_bot_trial_days','sales_bot_trial_traffic_gb','sales_bot_currency']
...
def enforce_license_features():
    ...
    feature_paths = {
        'nodes': ['/nodes', '/cluster', '/v17/nodes', '/node-gateway'],
...
def _allowed_form_protocols(values=None):
    return filter_protocols_for_license(values or active_protocols())
...
def quick_create_user():
    if request.method == 'POST':
        ...
        if preset == 'xray_plus':
            ...
            protocols = _allowed_form_protocols(request.form.getlist('protocols') or active)
        else:
            protocols = active
        ...
        sync_user(u, changed_protocols=protocols); log(current_user.username,'quick_create_user',u.username, ','.join(protocols))
...
def users():
    if request.method == 'POST':
        ...
        password = request.form.get('password') or secrets.token_urlsafe(10)
        protocols = _allowed_form_protocols(request.form.getlist('protocols') or active_protocols())
        ...
        u.set_password(password); db.session.add(u); db.session.commit(); set_user_ip_limit(u, request.form.get('ip_limit','0')); sync_user(u, changed_protocols=protocols); log(current_user.username,'create_user',u.username)
    ...
    return render_template('users.html', users=q.order_by(VpnUser.id.desc()).all(), user_status=user_access_status, usage_summary=user_usage_summary, get_user_ip_limit=get_user_ip_limit, search=search, nodes=Node.query.order_by(Node.name).all())
...
def user_toggle(user_id):
    ...
    if current_user.role == 'sub_admin' and u.owner_id != current_user.id:
        ...
        return redirect(url_for('web.users'))
    affected_protocols = (u.allowed_protocol_list() or u.protocol_list() or active_protocols())
    u.enabled = not u.enabled; db.session.commit(); sync_user(u, changed_protocols=affected_protocols); log(current_user.username,'toggle_user',u.username,str(u.enabled))
    return redirect(url_for('web.users'))
...
def user_edit(user_id):
    ...
    if request.method == 'POST':
        ...
        allowed_protocols = _allowed_form_protocols(request.form.getlist('protocols') or active_protocols())
        ...
        db.session.commit(); set_user_ip_limit(u, request.form.get('ip_limit','0')); sync_user(u, restart=bool(affected_protocols), changed_protocols=affected_protocols); log(current_user.username,'edit_user',u.username)
        flash('کاربر ویرایش شد و سرویس‌های VPN همگام‌سازی شدند')
        return redirect(url_for('web.user_configs', user_id=u.id))
    return render_template('user_edit.html', user=u, active=active_protocols(), plans=ServicePlan.query.filter_by(active=True).all(), get_user_ip_limit=get_user_ip_limit, nodes=Node.query.order_by(Node.name).all())
...
def health():
    ...
    if request.method == 'POST':
        ...
        ok, repair_output = doctor_repair(target)
        log(current_user.username, 'health_doctor_repair', target, repair_output[-800:])
        flash('Repair اجرا شد' if ok else 'Repair با هشدار/خطا تمام شد')
    summary = doctor_status()
    return render_template('health.html', summary=summary, repair_output=repair_output)
...
def nodes():
    ...
    if not feature_allowed('nodes'):
        ...
        return redirect(url_for('web.upgrade'))
    if request.method == 'POST':
        protocols = ','.join(_allowed_form_protocols(request.form.getlist('protocols') or active_protocols()))
        n = Node(
            ...
            gateway_enabled=bool(request.form.get('gateway_enabled')),
            delivery_mode=(request.form.get('delivery_mode') or 'relay'),
            subscription_enabled=bool(request.form.get('subscription_enabled')),
            subscription_host=(request.form.get('subscription_host') or request.form.get('host') or '').strip(),
            subscription_label=(request.form.get('subscription_label') or request.form.get('location') or request.form.get('name') or '').strip(),
        ...
        if not n.name or not n.host:
            flash('Name و Host الزامی است.')
            return redirect(url_for('web.nodes'))
        db.session.add(n); db.session.commit()
        try:
            queue_protocol_config_sync(n.id, [x for x in protocols.split(',') if x])
        except Exception:
            db.session.rollback()
        log(current_user.username,'node_add',n.host); flash('سرور اضافه شد؛ Sync اولیه کانفیگ و نصب هسته‌ها برای نود صف شد.')
        return redirect(url_for('web.nodes'))
    return render_template('nodes.html', nodes=Node.query.order_by(Node.name).all(), summary=node_sync_status_summary(), node_subscription_ports=_node_subscription_ports)
...
def node_edit(node_id):
    ...
    if request.method == 'POST':
        node.name = (request.form.get('name') or '').strip() or node.name
        node.host = (request.form.get('host') or '').strip() or node.host
        node.location = (request.form.get('location') or '').strip()
        node.protocols = ','.join(_allowed_form_protocols(request.form.getlist('protocols') or active_protocols()))
        ...
        node.gateway_enabled = bool(request.form.get('gateway_enabled'))
        node.delivery_mode = request.form.get('delivery_mode') or 'relay'
        node.subscription_enabled = bool(request.form.get('subscription_enabled'))
        node.subscription_host = (request.form.get('subscription_host') or node.host or '').strip()
        node.subscription_label = (request.form.get('subscription_label') or node.location or node.name or '').strip()
        ...
        try:
            queue_full_node_sync(node.id, [x for x in (node.protocols or '').split(',') if x], reason='direct-location-edit', force=False)
        except Exception:
            db.session.rollback()
        db.session.commit()
        log(current_user.username, 'node_edit', node.host)
        flash('نود به‌روزرسانی شد')
        return redirect(url_for('web.nodes'))
...
def node_delete(node_id):
    if current_user.role != 'main_admin' or not feature_allowed('nodes'):
        return redirect(url_for('web.dashboard'))
    node = Node.query.get_or_404(node_id)
    from .core.models import Port, ProtocolOutboundMap
    host = node.host
...
def settings():
    ...
    if request.method == 'POST':
        ...
        sanitized_active_protocols = ','.join(_allowed_form_protocols(request.form.get('active_protocols','').split(','))) if 'active_protocols' in request.form else ''
...
def cluster():
    ...
    if request.method=='POST':
        ...
        if action in ('restart_panel','restart_vpn','repair','backup','update'):
            ...
            n=Node(name=request.form['name'], host=request.form['host'], protocols=','.join(_allowed_form_protocols(request.form.getlist('protocols') or active_protocols())), health='pending')
...
def finance_plans():
    if current_user.role != 'main_admin': return redirect(url_for('web.dashboard'))
    if request.method == 'POST':
        p=ServicePlan(name=request.form['name'], days=int(request.form.get('days') or 0), traffic_gb=int(request.form.get('traffic_gb') or 0), price=float(request.form.get('price') or 0), currency=request.form.get('currency','USD'), protocols=','.join(_allowed_form_protocols(request.form.getlist('protocols') or active_protocols())), active=bool(request.form.get('active')))
...
def sales_bot():
    ...
    return render_template('sales_bot.html', settings=settings, plans=plans, orders=orders, customers=customers, active=active_protocols(), sales_bot_owner_id=owner_id)
...
def node_auto_install(node_id):
    ...
    try:
        ...
        if action == 'save_credentials':
            ...
            return redirect(url_for('web.nodes'))
        result = run_auto_node_install(node, request.url_root)
        log(current_user.username, 'node_auto_install', node.host, result.get('message', ''))
        flash(result.get('message', 'Auto install finished'))
    except Exception as exc:
        db.session.rollback()
...
from .services.node_gateway import node_gateway_settings, save_node_gateway_settings, node_gateway_status, apply_node_gateway_runtime, queue_user_sync, queue_all_user_sync, queue_node_job, queue_node_health_check, rebalance_users, assign_user_node, node_sync_status_summary, force_protocols_to_node, reset_node_gateway_to_local, queue_protocol_config_sync, queue_full_node_sync
