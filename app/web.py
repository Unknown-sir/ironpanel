...
def quick_create_user():
    if request.method == 'POST':
        ...
        u = VpnUser(username=username, l2tp_password=password, cisco_password=password, data_limit_mb=data_limit_mb, connection_limit=int(request.form.get('connection_limit') or 1), protocols=','.join(protocols), protocol_permissions=','.join(protocols), allowed_devices=0, expires_at=expires_at, owner_id=current_user.id if current_user.role=='sub_admin' else None, node_mode=request.form.get('node_mode','auto'), preferred_node_id=int(request.form.get('preferred_node_id') or 0) or None)
...
def users():
    if request.method == 'POST':
        ...
        if not ok:
            ...
            return redirect(url_for('web.users'))
        expires_at = _parse_unlimited_days(request.form.get('days'), 30)
        u = VpnUser(username=username, l2tp_password=request.form.get('l2tp_password') or password, cisco_password=request.form.get('cisco_password') or password, data_limit_mb=data_limit_mb, connection_limit=int(request.form.get('connection_limit') or 1), protocols=','.join(protocols), protocol_permissions=','.join(protocols), allowed_devices=int(request.form.get('allowed_devices') or 0), expires_at=expires_at, owner_id=current_user.id if current_user.role=='sub_admin' else None, node_mode=request.form.get('node_mode','auto'), preferred_node_id=int(request.form.get('preferred_node_id') or 0) or None)
