...
def users():
    ...
    user_rows = q.order_by(VpnUser.id.desc()).all()
    auto_disabled_cleanup_count = len(auto_disabled_cleanup_users(user_rows))
    return render_template('users.html', users=user_rows, user_status=user_access_status, usage_summary=user_usage_summary, get_user_ip_limit=get_user_ip_limit, search=search, nodes=Node.query.order_by(Node.name).all(), available=available_protocols_for_current_user(), auto_password_length=auto_password_length, auto_password_mode=auto_password_mode, auto_disabled_cleanup_count=auto_disabled_cleanup_count)
...
def users_bulk_create():
    ...
    lines = [
        'IronPanel v19.10.19 bulk users',
