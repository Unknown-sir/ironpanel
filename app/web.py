...
def _allowed_form_protocols(values=None, *, allow_default=False):
    ...
    return filter_protocols_for_license(normalize_user_protocols(values or [], allow_default=allow_default))
...
def settings():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('تنظیمات ذخیره و روی هسته‌ها اعمال شد. برای اعمال پورت پنل: sudo bash /opt/ironpanel/upgrade.sh --restart-only یا systemctl restart ironpanel')
        except Exception as exc:
