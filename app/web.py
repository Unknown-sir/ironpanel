...
from .services.license import check_license, save_license_key, license_key, license_server_url, license_remaining_days, feature_allowed, feature_label, current_license_type, current_license_features
...
from .services.v13 import latest_release, create_release, process_local_jobs, current_version, github_latest_version, run_github_update
from .services.xray import XRAY_PROFILE_TYPES, xray_settings, update_xray_settings, write_xray_config, ensure_reality_keys, xray_runtime_status, xray_link
...
def inject_globals():
    return dict(panel_host=get_public_host, active_protocols=active_protocols, feature_allowed=feature_allowed, license_type=current_license_type, license_features=current_license_features)
...
def enforce_license():
    ...
    feature_paths={
        'nodes': ['/nodes','/cluster'],
        'network': ['/firewall','/dns','/domains','/ssl'],
        'billing': ['/billing','/plans','/wallet','/invoices'],
        'sales_bot': ['/sales-bot'],
    }
    for feature, prefixes in feature_paths.items():
        if any(path.startswith(p) for p in prefixes) and not feature_allowed(feature):
            flash(f'این بخش در نوع لایسنس فعلی شما فعال نیست: {feature_label(feature)}')
...
def system_metrics():
    ...
    try:
        ...
        data.update(license_days_remaining=license_remaining_days(lres), license_valid=bool(lres.get('valid')), license_expires_at=lres.get('expires_at',''))
    except Exception:
        data.update(license_days_remaining=None, license_valid=False, license_expires_at='')
    return jsonify(**data)
...
def settings():
    ...
    if request.method == 'POST':
        ...
        for key in ['public_host','tunnel_host','active_protocols','openvpn_transport','ocserv_transport','wireguard_transport','l2tp_transport','pptp_enabled','hysteria2_enabled','hysteria2_obfs_password','hysteria2_tls_cert_file','hysteria2_tls_key_file','telegram_bot_token','telegram_chat_id','notify_login','notify_expiry','ha_enabled','load_balancer_enabled','auto_failover_enabled','auto_backup_enabled','auto_backup_time','backup_remote_type','backup_remote_path','theme_mode','language','security_2fa_enabled','security_ip_whitelist','security_captcha_enabled','fail2ban_enabled','release_channel']:
            set_setting(key, request.form.get(key, ''))
        set_setting('license_server_url', 'http://license.skyshield.space:8002')
        for key in ['panel','openvpn_udp','openvpn_tcp','ocserv_tcp','ocserv_udp','l2tp_udp','ipsec_ike','ipsec_nat','wireguard_udp','xray_tcp','xray_api','pptp_tcp','hysteria2_udp']:
...
def xray_core():
    ...
    return render_template('xray.html', settings=xray_settings(), profile_types=XRAY_PROFILE_TYPES, runtime=xray_runtime_status(), users=valid_users)
...
def sales_bot():
    ...
    if request.method == 'POST':
        ...
        if action == 'settings':
            keys = ['sales_bot_enabled','sales_bot_token','sales_bot_admin_ids','sales_bot_support_url','sales_bot_payment_text','sales_bot_trial_enabled','sales_bot_trial_days','sales_bot_trial_traffic_gb','sales_bot_currency']
            for k in keys:
                if k in ('sales_bot_enabled','sales_bot_trial_enabled'):
...
def updates():
    ...
    if request.method=='POST':
        ...
        if action == 'process_jobs':
            ...
            ok,out=run_github_update(); flash(('آپدیت از GitHub اجرا شد. ' if ok else 'آپدیت با خطا مواجه شد. ')+out[-800:])
        else:
    ...
    return render_template('updates.html', releases=UpdateRelease.query.order_by(UpdateRelease.id.desc()).all(), latest=latest_release(), github=github_latest_version(), current_version=current_version(), jobs=RemoteJob.query.order_by(RemoteJob.id.desc()).limit(50).all())
...
@web_bp.route('/subscription-manager')
@login_required
def subscription_manager():
    return render_template('subscription_manager.html', users=VpnUser.query.order_by(VpnUser.id.desc()).all(), formats=['raw','clash','singbox','hiddify'])
