...
from .services.provisioning import sync_user, delete_user, log, get_setting, set_setting, get_port, get_public_host, active_protocols, user_config_payload, apply_runtime_configs, user_access_status, sync_all_users, service_status, service_status_detailed, service_error_detail, backup_now, restore_backup, service_health_repair, telegram_notify, run_cmd, collect_usage_from_runtime, user_usage_summary, traffic_multiplier_settings, set_traffic_multiplier, enforce_usage_limits, ip_limit_settings, set_ip_limit_settings, get_user_ip_limit, set_user_ip_limit, enforce_ip_limits, subscription_theme_settings, set_subscription_theme
...
from .services.v13 import latest_release, create_release, process_local_jobs, current_version, github_latest_version, run_github_update, github_update_log_tail
...
from .services.admin_bot import admin_bot_settings, save_admin_bot_settings, admin_bot_report_text, send_test_admin_report
...
def settings():
    ...
    if request.method == 'POST':
        ...
        for key in ['public_host','tunnel_host','active_protocols','openvpn_transport','ocserv_transport','wireguard_transport','l2tp_transport','pptp_enabled','hysteria2_enabled','hysteria2_obfs_password','hysteria2_tls_cert_file','hysteria2_tls_key_file','hysteria2_up_mbps','hysteria2_down_mbps','telegram_bot_token','telegram_chat_id','notify_login','notify_expiry','ha_enabled','load_balancer_enabled','auto_failover_enabled','auto_backup_enabled','auto_backup_time','backup_remote_type','backup_remote_path','theme_mode','language','security_2fa_enabled','security_ip_whitelist','security_captcha_enabled','fail2ban_enabled','release_channel','ui_mode']:
...
def subscription(token):
    ...
    collect_usage_from_runtime()
    configs = user_config_payload(u)
    return render_template('subscription.html', user=u, host=get_public_host(), configs=configs, usage=user_usage_summary(u), theme=subscription_theme_settings())
...
def sales_bot():
    ...
    if request.method == 'POST':
        ...
        if action == 'settings':
            keys = ['sales_bot_enabled','sales_bot_token','sales_bot_admin_ids','sales_bot_support_url','sales_bot_payment_text','sales_bot_rules_text','sales_bot_connection_guide','sales_bot_qr_enabled','sales_bot_trial_enabled','sales_bot_trial_days','sales_bot_trial_traffic_gb','sales_bot_currency']
            for k in keys:
...
def dashboard_quick_upgrade():
    ...
    if not latest.get('update_available'):
        ...
        return redirect(url_for('web.dashboard'))
    ok, out = run_github_update(background=True)
    log(current_user.username, 'github_quick_upgrade', latest.get('latest', ''), out[-1000:])
    flash(('آپگرید سریع شروع شد. ' if ok else 'شروع آپگرید سریع ناموفق بود. ') + out[-800:])
    return redirect(url_for('web.dashboard'))
...
def updates():
    ...
    if request.method=='POST':
        ...
        if action == 'process_jobs':
            ...
            ok,out=run_github_update(background=True); flash(('آپدیت از GitHub در پس‌زمینه شروع شد. ' if ok else 'شروع آپدیت با خطا مواجه شد. ')+out[-800:])
        else:
    ...
    return render_template('updates.html', releases=UpdateRelease.query.order_by(UpdateRelease.id.desc()).all(), latest=latest_release(), github=github_latest_version(force=True), current_version=current_version(), jobs=RemoteJob.query.order_by(RemoteJob.id.desc()).limit(50).all(), github_log=github_update_log_tail())
...
def subscription_manager():
    if request.method == 'POST':
        ...
        flash('قالب و تنظیمات صفحه Subscription ذخیره شد')
        return redirect(url_for('web.subscription_manager'))
    return render_template('subscription_manager.html', users=VpnUser.query.order_by(VpnUser.id.desc()).all(), formats=['raw','clash','singbox','hiddify'], theme=subscription_theme_settings())
