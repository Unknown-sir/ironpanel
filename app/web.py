...
def users():
    ...
    return render_template('users.html', users=q.order_by(VpnUser.id.desc()).all(), user_status=user_access_status, usage_summary=user_usage_summary, get_user_ip_limit=get_user_ip_limit, search=search, nodes=Node.query.order_by(Node.name).all(), available=available_protocols_for_current_user(), auto_password_length=auto_password_length, auto_password_mode=auto_password_mode)
...
def users_bulk_create():
    ...
    lines = [
        'IronPanel v19.10.18 bulk users',
