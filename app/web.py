...
from .services.i18n import t, ui, current_language, current_theme, language_dir, LANGUAGES, THEMES, save_appearance, localize_hardcoded_text
from .services.maintenance import doctor_status, doctor_repair, repair_status, queue_doctor_repair, backup_catalog, create_safe_backup, restore_safe_backup, safe_updater_terminal_command
...
def _normalize_reseller_protocols(values=None, *, allow_default=False):
    from .services.provisioning import normalize_user_protocols
    selected = filter_protocols_for_license(normalize_user_protocols(values or [], allow_default=allow_default))
...
def _allowed_form_protocols(values=None, *, allow_default=False):
    from .services.provisioning import normalize_user_protocols
    requested = filter_protocols_for_license(normalize_user_protocols(values or [], allow_default=allow_default))
...
def quick_create_user():
    if request.method == 'POST':
        ...
        if preset == 'xray_plus':
            ...
            protocols = _allowed_form_protocols(active, allow_default=True)
        if not protocols:
            flash('این Preset با پروتکل‌های مجاز نماینده سازگار نیست؛ انتخاب دستی را باز کنید.')
            return redirect(url_for('web.quick_create_user'))
        ...
        sync_user(u, restart=False, changed_protocols=protocols); log(current_user.username,'quick_create_user',u.username, ','.join(protocols))
...
def users():
    if request.method == 'POST':
        ...
        u.set_password(password); db.session.add(u); db.session.commit(); set_user_ip_limit(u, request.form.get('ip_limit','0')); sync_user(u, restart=False, changed_protocols=protocols); log(current_user.username,'create_user',u.username)
    ...
    return render_template('users.html', users=q.order_by(VpnUser.id.desc()).all(), user_status=user_access_status, usage_summary=user_usage_summary, get_user_ip_limit=get_user_ip_limit, search=search, nodes=Node.query.order_by(Node.name).all(), available=available_protocols_for_current_user())


# v19.10.12 bulk username-range user creator

def _bulk_password(length, mode):
    import string
    try:
        length = int(length)
    except Exception:
    ...
    if mode in {'number','numbers','num','digit','digits','عدد','numeric'}:
        alphabet = string.digits
    elif mode in {'letter','letters','alpha','alphabet','حرف'}:
...
def users_bulk_create():
    ...
    if count < 1 or count > 1000:
        ...
        return redirect(url_for('web.users'))
    try:
        password_length = int(request.form.get('bulk_password_length') or 8)
    except Exception:
        password_length = 8
    if password_length < 3:
        flash('طول رمز باید حداقل 3 کاراکتر باشد.')
        return redirect(url_for('web.users'))
    password_mode = request.form.get('bulk_password_mode') or 'both'
    try:
    ...
    lines = [
        'IronPanel v19.10.16 bulk users',
...
def resellers():
    ...
    if changed:
        db.session.commit()
    return render_template('resellers.html', resellers=rows, reseller_stats=_reseller_stats, reseller_panel_url=reseller_panel_url, reseller_protocol_choices=_normalize_reseller_protocols(active_protocols() or [], allow_default=True))
...
def reseller_update(reseller_id):
    ...
    log(current_user.username, 'update_reseller', r.username, f'{r.panel_path}; protocols={r.reseller_protocols}; users_restricted={len(changed_users)}')
    flash(f'نماینده ویرایش شد. پروتکل‌های مجاز: {r.reseller_protocols}. کاربران اصلاح‌شده: {len(changed_users)}. آدرس پنل: {reseller_panel_url(r)}')
    return redirect(url_for('web.resellers'))
