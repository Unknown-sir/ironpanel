// IronPanel v19.10.2 stable UI runtime: single menu controller, responsive helpers, safe copy/metrics.
(function(){
  'use strict';
  ...
  function initMenu(){
    ...
    qsa('.nav-group').forEach(function(details){
      ...
      var key = 'ironpanel.v19102.nav.' + (details.dataset.menu || 'group');
        ...
        if(details.open){
          qsa('.nav-group', details.closest('.app-sidebar') || document).forEach(function(other){
            if(other !== details && !other.querySelector('a.active')){
              other.open = false;
              localStorage.setItem('ironpanel.v19102.nav.' + (other.dataset.menu || 'group'), '0');
            }
          });
        }
      });
