...
async function refreshMetrics(){
  try{
    ...
    if(ld){let d=m.license_days_remaining; const unknown=(d===null||d===undefined||d===''); ld.innerText=unknown?'--':d; setText('license_sub',unknown?'نامشخص':'روز باقی‌مانده'); setRing('license',unknown?0:Math.min(100,Math.max(0,Number(d)||0))); const lr=document.querySelector('[data-metric="license"] .ring'); if(lr) lr.classList.toggle('warning',!unknown && Number(d)<=7);}
