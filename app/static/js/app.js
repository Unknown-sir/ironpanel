...
// v18.6.3 - robust mobile sidebar toggle
(function(){
  ...
  shell.addEventListener('click',(e)=>{if(document.body.classList.contains('menu-open') && e.target===shell) closeMenu();});
  document.addEventListener('keydown',(e)=>{if(e.key==='Escape') closeMenu();});
  document.querySelectorAll('.sidebar a').forEach(a=>a.addEventListener('click',()=>{if(window.matchMedia('(max-width:920px)').matches) closeMenu();}));
})();
// Close mobile menu when tapping outside the sidebar.
(function(){
  document.addEventListener('click',(e)=>{
    if(!document.body.classList.contains('menu-open')) return;
    if(e.target.closest('.sidebar') || e.target.closest('.mobile-menu-toggle')) return;
    document.body.classList.remove('menu-open');
    const btn=document.querySelector('.mobile-menu-toggle'); if(btn) btn.setAttribute('aria-expanded','false');
  });
})();


// v19.8.1 - keep More Settings/Details stable inside mobile sidebar.
(function(){
  document.querySelectorAll('.menu-section[data-menu] summary').forEach(function(summary){
    summary.addEventListener('click', function(e){
      // Prevent accidental double toggle caused by nested icons/text on touch devices.
      const details = summary.parentElement;
      if(!details) return;
      details.classList.add('touched-menu-section');
      setTimeout(function(){details.classList.remove('touched-menu-section')}, 180);
    }, {passive:true});
  });
})();


// v19.8.2 - robust sidebar details toggle; prevents More Settings collapsing/overlapping on touch devices.
(function(){
  document.querySelectorAll('.side-nav details.menu-section').forEach(function(details){
    ...
    summary.addEventListener('click', function(e){
      ...
      requestAnimationFrame(function(){
        ...
        if(items){items.style.position = 'static'; items.style.maxWidth = '100%';}
      });
    });
  });
})();

// v19.9.27 - modern UI helpers: responsive tables and safer drawer backdrop.
(function(){
