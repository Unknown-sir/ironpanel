// IronPanel v19.10.23 stable UI runtime: single menu controller, strict i18n, responsive helpers, safe copy/metrics.
(function(){
  'use strict';
  ...
  function legacyCopyText(text){
    ...
    ta.style.top = '0';
    ta.style.opacity = '0';
    ta.style.fontSize = '16px'; // prevents iOS Safari input zoom
    document.body.appendChild(ta);
    try{ ta.focus({preventScroll:true}); }catch(err){ try{ ta.focus(); }catch(e2){} }
    ta.select();
    try{ ta.setSelectionRange(0, ta.value.length); }catch(err){}
    var ok = false;
  ...
  async function copyTextCompat(text){
    ...
    // iOS Safari is most reliable when execCommand runs synchronously inside the tap gesture.
    var isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent || '') ||
      (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
    if(isIOS && legacyCopyText(text)) return true;
    if(window.isSecureContext && navigator.clipboard && navigator.clipboard.writeText){
      try{ await navigator.clipboard.writeText(text); return true; }catch(err){}
    }
    return legacyCopyText(text);
  }
  window.ironpanelCopyText = copyTextCompat;
  ...
  function initCopy(){
    document.addEventListener('click', function(e){
      ...
      if(!text) return;
      copyTextCompat(text).then(function(ok){
        if(!ok) return;
