...
def _get_port(protocol: str) -> int:
    ...
    if protocol == 'openvpn':
        transport = _setting('openvpn_transport', 'udp').strip().lower()
        key = 'port_openvpn_tcp' if transport == 'tcp' else 'port_openvpn_udp'
    try:
...
def node_ssl_domain(node: Node) -> str:
    # v19.9.24: ACME/TLS domain for the physical node. In tunnel scenarios
    # this may be different from the config/subscription host.
    return _clean_host(getattr(node, 'ssl_domain', '') or getattr(node, 'config_domain', '') or getattr(node, 'subscription_host', '') or getattr(node, 'host', '') or getattr(node, 'public_ip', ''))
...
def _replace_text_endpoint(body: str, host: str, port: int | None = None) -> str:
    ...
    if re.search(r'^Server:\s*', body, flags=re.M | re.I):
        body = re.sub(r'^Server:\s*[^\n]+', f'Server: {endpoint}', body, count=1, flags=re.M | re.I)
        if re.search(r'^(Remote ID|Server identity)\s*:', body, flags=re.M | re.I):
            body = re.sub(r'^(Remote ID|Server identity)\s*:[^\n]+', lambda m: f'{m.group(1)}: {host}', body, count=1, flags=re.M | re.I)
        return body
...
def apply_node_usage_reports(node: Node, reports: list) -> int:
    ...
    if not isinstance(reports, list):
        return 0
    try:
        from .provisioning import _account_runtime_counter, _account_final_runtime_counter, enforce_usage_limits
    except Exception:
        return 0
    changed = 0
    for r in reports:
        ...
        try:
            ...
            source = re.sub(r'[^a-z0-9_.:-]+','_', str(r.get('source') or proto).lower())[:36]
        ...
        if not user:
            continue
        account = _account_final_runtime_counter if bool(r.get('final')) else _account_runtime_counter
        if account(user, f'node{node.id}_{source}', rx, tx):
            changed += 1
    if changed:
        try:
