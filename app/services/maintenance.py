...
CRITICAL_SERVICES = [
    ...
    {"id": "l2tp", "label": "L2TP / xl2tpd", "unit": "xl2tpd.service", "category": "protocol", "proto": "l2tp", "repair": "repair_l2tp.sh"},
...
def doctor_status() -> Dict:
    ...
    for svc in CRITICAL_SERVICES:
        ...
        if proto and proto not in protocols:
            rows.append(_status_row(svc["id"], svc["label"], svc["category"], True, "disabled", "Protocol is disabled in Active Protocols", svc.get("repair", "")))
            continue
        unit = svc["unit"]
        ...
        rows.append(_status_row(svc["id"], svc["label"], svc["category"], ok, state, detail, svc.get("repair", "restart:" + unit) if svc.get("category") == "protocol" or not ok else ""))
...
def _status_file() -> Path:
    ...
    return root / "health_repair_status.json"
...
def repair_status() -> Dict:
    path = _status_file()
    data = {"running": False, "status": "idle", "target": "", "pid": 0, "started_at": "", "finished_at": "", "ok": None, "output_tail": ""}
    try:
        if path.exists():
            data.update(json.loads(path.read_text(encoding="utf-8") or "{}"))
    except Exception:
        pass
    if data.get("running") and not _pid_alive(data.get("pid")):
        data["running"] = False
        data["status"] = "finished-or-stale"
    try:
        logp = _log_file()
        if logp.exists():
            data["output_tail"] = logp.read_text(errors="ignore")[-12000:]
    except Exception:
        pass
    return data


def _write_repair_status(**kwargs) -> Dict:
    data = repair_status()
    data.update(kwargs)
    try:
...
def _repair_script_command(script_dir: Path, name: str, timeout_sec: int = 240) -> str:
    return f"timeout {int(timeout_sec)} bash {script_dir / name} || true"


def queue_doctor_repair(target: str = "all", actor: str = "web") -> tuple[bool, str, Dict]:
    """Start a Health Doctor repair outside the Flask request.

    Long repair scripts used to run synchronously from /health. On small VPSs
    that could exhaust the web workers and make the panel look down. This queues
    the same repair in a detached Python process and lets the page return
    immediately.
    """
    target = (target or "all").strip()
    current = repair_status()
    if current.get("running") and _pid_alive(current.get("pid")):
        return False, "repair_already_running", current
    ...
    code = (
        ...
        "    from app.services.maintenance import _run_queued_doctor_repair\n"
        "    _run_queued_doctor_repair(os.environ.get('IRONPANEL_REPAIR_TARGET','all'), os.environ.get('IRONPANEL_REPAIR_ACTOR','web'))\n"
    )
    env = os.environ.copy()
    env["IRONPANEL_REPAIR_TARGET"] = target
    env["IRONPANEL_REPAIR_ACTOR"] = actor
    logp = _log_file()
    try:
        logp.parent.mkdir(parents=True, exist_ok=True)
        lf = open(logp, "a", encoding="utf-8")
        proc = subprocess.Popen([py, "-c", code], cwd="/opt/ironpanel", env=env, stdout=lf, stderr=lf, start_new_session=True)
        data = _write_repair_status(running=True, status="running", target=target, pid=proc.pid, actor=actor, started_at=_now(), finished_at="", ok=None)
        return True, "queued", data
    except Exception as exc:
        data = _write_repair_status(running=False, status="failed-to-start", target=target, pid=0, actor=actor, started_at=_now(), finished_at=_now(), ok=False, output_tail=str(exc))
        return False, str(exc), data


def _run_queued_doctor_repair(target: str, actor: str = "web") -> None:
    _write_repair_status(running=True, status="running", target=target, pid=os.getpid(), actor=actor, started_at=_now(), finished_at="", ok=None)
    ok = False
    out = ""
    try:
        ok, out = doctor_repair(target)
    except Exception as exc:
        out = f"Health repair crashed: {exc}"
        ok = False
    try:
        _log_file().write_text((out or '')[-60000:], encoding="utf-8")
    except Exception:
        pass
    _write_repair_status(running=False, status="done" if ok else "failed", target=target, pid=os.getpid(), actor=actor, finished_at=_now(), ok=bool(ok), output_tail=(out or '')[-12000:])
...
def doctor_repair(target: str = "all") -> tuple[bool, str]:
    ...
    cmds = []
    if target == "all":
        # Lean full repair: do not run install_vpn_core.sh by default because it
        # performs apt operations and restarts every core. Individual repair
        # scripts install their missing package when needed and are capped by
        # timeout to protect the web panel.
        cmds.extend([
            "systemctl daemon-reload || true",
            _repair_script_command(script_dir, "repair_db.sh", 90),
            _repair_script_command(script_dir, "repair_openvpn.sh", 180),
            _repair_script_command(script_dir, "repair_wireguard.sh", 180),
            _repair_script_command(script_dir, "repair_ocserv.sh", 180),
            _repair_script_command(script_dir, "repair_xray.sh", 220),
            _repair_script_command(script_dir, "repair_hysteria2.sh", 220),
            _repair_script_command(script_dir, "repair_pptp.sh", 180),
            _repair_script_command(script_dir, "repair_l2tp.sh", 220),
            f"timeout 220 bash {script_dir}/repair_telegram_proxy.sh --sync || true",
            f"timeout 180 bash {script_dir}/repair_ssh.sh --sync || true",
            f"timeout 90 bash {script_dir}/apply_speed_limits.sh --apply || true",
            f"timeout 90 bash {script_dir}/apply_node_gateway.sh --apply || true",
            "systemctl restart ironpanel-usage-sync.timer >/dev/null 2>&1 || true",
        ])
    elif target == "db":
        cmds.append(_repair_script_command(script_dir, "repair_db.sh", 90))
    elif target == "ssl":
        cmds.append(_repair_script_command(script_dir, "repair_certbot.sh", 240))
    elif target.startswith("restart:"):
        unit = target.split(":", 1)[1]
        cmds.append(f"timeout 90 systemctl restart {unit} || true")
    else:
        safe_target = os.path.basename(target)
        cmds.append(f"timeout 240 bash {script_dir / safe_target} || true")
    cmd = "\n".join(cmds) + "\n"
    p = run_cmd(["bash", "-lc", cmd], timeout=900)
    out = ((p.stdout or "") + (p.stderr or ""))[-12000:]
    try:
        apply_runtime_configs()
        # Do not restart every core after Health Doctor. Repair scripts have
        # already restarted only the services they changed; user/password sync is
        # safe without service restarts and avoids disconnect spikes.
...
def backup_catalog() -> List[Dict]:
    ...
    for p in sorted(_backup_dir().glob("*.tar.gz"), key=lambda x: x.stat().st_mtime, reverse=True):
        ...
        rows.append({
            ...
            "path": str(p),
            "migration_capable": bool(manifest.get("migration_capable")),
            "format_version": int(manifest.get("format_version") or 0),
            "counts": manifest.get("counts") or manifest.get("db_counts") or {},
        })
    return rows
...
def create_safe_backup(note: str = "manual", include_source: bool = False) -> Path:
    """Create a migration-grade backup.

    v19.10.25 keeps the public function name for all existing UI/CLI/bot callers,
    but the implementation now includes the transactional database plus every
    protocol identity required for a no-reissue server migration.
    """
    from .migration_backup import create_migration_backup
    return create_migration_backup(note=note, include_source=include_source)


def _validate_archive(path: Path) -> None:
    from .migration_backup import validate_backup_archive
    validate_backup_archive(Path(path))


def restore_safe_backup(path: Path, restore_source: bool = False) -> tuple[bool, str]:
    """Restore a migration backup while preserving existing client identities."""
    from .migration_backup import restore_migration_backup
    return restore_migration_backup(Path(path), restore_source=restore_source)
