...
PROTOCOL_HELP = {
    ...
    'xray':'برای هر کاربر Xray جداگانه و بر اساس IP عمومی نشست واقعی او؛ شناسایی از access.log با email هر کاربر انجام می‌شود (خودکار روی loglevel=info می‌رود).',
    'pptp':'PPTP روی یک پورت مشترک است؛ با یک کاربرِ محدود، کل پورت محدود می‌شود (ترافیک GRE خودش از این قاعده بیرون است).',
    'hysteria2':'برای هر کاربر Hysteria2 تلاش می‌شود IP از ژورنال سرویس خوانده شود؛ در غیر این صورت اگر فقط یک کاربر محدود باشد کل پورت محدود می‌شود.',
    'telegram_proxy':'هر کاربر Telegram Proxy پورت اختصاصی خودش را دارد؛ محدودیت همیشه دقیق و مستقیم روی همان پورت اعمال می‌شود.',
    'ssh':'برای هر کاربر SSH جداگانه؛ اگر Linux UID موجود باشد دقیق‌تر اعمال می‌شود.',
}
ALL_PROTOCOLS = list(PROTOCOL_LABELS.keys())

# v19.10.29: these protocols share ONE listening port between all users, so a
# destination-IP tc rule can only be built when exactly one enabled user
# actually carries a limit on that protocol. In every other case the row stays
# "pending" instead of silently doing nothing.
SHARED_PORT_PROTOCOLS = {'xray', 'hysteria2', 'pptp', 'telegram_proxy'}


def _limited_enabled_user_count(protocol: str) -> int:
    """How many enabled users effectively carry a limit on this protocol."""
    n = 0
    for u in VpnUser.query.filter_by(enabled=True).all():
        if protocol not in (u.protocols or '').split(','):
            continue
        try:
            if user_protocol_limit(u, protocol) > 0:
                n += 1
        except Exception:
            continue
    return n
...
def write_speed_limit_runtime_config() -> Path:
    ...
    lines=['# Generated by IronPanel v19.10.29', '# username user_id protocol proto port mbps match_type match_value group_key']
    ...
    for u in VpnUser.query.filter_by(enabled=True).order_by(VpnUser.id).all():
        ...
        for protocol in [p for p in (u.protocols or '').split(',') if p]:
            ...
            # v19.10.29: shared-port protocols (Xray/Hysteria2/PPTP listen on one
            # port for everyone) previously stayed "pending" forever, which made
            # per-user limits look broken. Telegram Proxy instances are per-user
            # ports so they are always safe to shape by port alone; the other
            # shared ports are shaped when exactly one enabled user carries a
            # limit on that protocol.
            if mtype == 'pending' and protocol in SHARED_PORT_PROTOCOLS:
                if protocol == 'telegram_proxy':
                    mtype,mval='port_only','-'
                else:
                    if protocol not in shared_port_cache:
                        shared_port_cache[protocol]=_limited_enabled_user_count(protocol)
                    if shared_port_cache[protocol] == 1:
                        mtype,mval='port_only','-'
                    else:
                        key=f'{protocol}:{u.username}'
                        if key not in pending_notes:
    ...
    if pending_notes:
        lines.append('# NOTE pending (shared port used by several limited users; per-user tc matching is not possible):')
        for note in pending_notes:
            lines.append(f'# NOTE pending {note}')
    path.write_text('\n'.join(lines)+'\n', encoding='utf-8')
    return path

def _ensure_xray_access_logging():
    """v19.10.29: per-user Xray tc rules need access.log emails, which only
    exist at loglevel info. Flip it on once when limits are configured."""
    try:
        if get_setting('xray_loglevel', 'warning') == 'info':
            return
        set_setting('xray_loglevel', 'info')
        from .provisioning import _valid_users
        from .xray import write_xray_config
        write_xray_config(_valid_users(), restart=True)
        try:
            Path('/etc/logrotate.d/ironpanel-xray').write_text(
...
def speed_limit_status():
    ...
    head=''
    try:
        conf=Path('/etc/ironpanel/speed_limits.conf')
        if conf.exists():
            notes=[l.replace('# NOTE pending ','') for l in conf.read_text(encoding='utf-8').splitlines() if l.startswith('# NOTE pending ')]
            if notes:
                head='--- pending (no per-user match possible)\n' + '\n'.join(notes[:40]) + '\n\n'
    except Exception:
        pass
    return head + ((p.stdout or '')+(p.stderr or '')).strip()[-10000:]
