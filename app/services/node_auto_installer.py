    ...
    while True:
        ...
        if time.time() - start > timeout:
            try:
                ...
                pass
            partial=''.join(out_chunks)[-12000:]
            raise RuntimeError('SSH command timeout after %ss%s' % (timeout, ('; partial output:\n' + partial) if partial else ''))
        time.sleep(0.25)
...
def run_auto_node_install(node, request_base_url='', *, save_log=True, job=None):
    ...
    try:
        ...
        log_lines.append('[auto-install] launching runtime bootstrap on remote node')
        checkpoint()
        ...
        # v19.9.15: Direct-location nodes no longer require the legacy Node Gateway relay.
        # Do not auto-apply gateway DNAT/relay rules after SSH install.
        ok = True
