...
def _map_for_lang(lang: str) -> dict[str, str]:
    ...
    if lang.startswith('ar'):
        merged = dict(_HARDCODED_FA_TO_EN)
...
def localize_hardcoded_text(text: str, lang: str | None = None) -> str:
    ...
    if _ARABIC_RE.search(out):
        for src in _LEGACY_FA_UI_PHRASES:
            if src in out:
                out = out.replace(src, mapping.get(src) or _guess_legacy_phrase_en(src))
    return out
...
def ui(fa_text: str, en_text: str | None = None) -> str:
    ...
    if lang.startswith('fa'):
        return fa_text
    base = en_text or _HARDCODED_FA_TO_EN.get(fa_text, fa_text)
    if lang.startswith('ar'):
        return _HARDCODED_FA_TO_AR.get(fa_text, base)
    if lang.startswith('ru'):
        return _HARDCODED_FA_TO_RU.get(fa_text, base)
    return base
