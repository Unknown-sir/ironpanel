...
_HARDCODED_FA_TO_EN = {
    ...
    # v19.10.28: first-connection validity + delete-all-users section
    'شروع اعتبار از اولین اتصال': 'Start validity on first connection',
    'اولین اتصال': 'First connection',
    'در انتظار اولین اتصال': 'Waiting for first connection',
    'اعتبار': 'Validity',
    # v19.10.29: shared-port speed-limit help texts
    'برای هر کاربر Xray جداگانه و بر اساس IP عمومی نشست واقعی او؛ شناسایی از access.log با email هر کاربر انجام می‌شود (خودکار روی loglevel=info می‌رود).': 'Per-user Xray limits use the real public IP of each session; users are identified from access.log emails (loglevel is switched to info automatically).',
    'PPTP روی یک پورت مشترک است؛ با یک کاربرِ محدود، کل پورت محدود می‌شود (ترافیک GRE خودش از این قاعده بیرون است).': 'PPTP shares one port; with a single limited user the whole port is capped (GRE payload traffic itself stays outside this rule).',
    'برای هر کاربر Hysteria2 تلاش می‌شود IP از ژورنال سرویس خوانده شود؛ در غیر این صورت اگر فقط یک کاربر محدود باشد کل پورت محدود می‌شود.': "Per-user Hysteria2 IPs are read from the service journal when available; otherwise, with a single limited user the whole port is capped.",
    ...
    # v19.10.30 compact user cards + details modal
    'نمایش اطلاعات و ویرایش': 'View details & edit',
    'ویرایش در همین کادر': 'Edit right here',
    'ذخیره = همگام‌سازی': 'Save = sync',
    'صفحه کامل ویرایش': 'Full edit page',
    ...
    'اگر فعال باشد، روزهای اعتبار از اولین اتصال کاربر حساب می‌شود': "When enabled, validity days are counted from the user's first connection",
    'تمام کاربران به‌همراه کانفیگ‌ها، گواهی‌های OpenVPN و هویت‌های WireGuardشان برای همیشه حذف می‌شوند. این عملیات قابل بازگشت نیست؛ قبل از انجام، بکاپ بگیرید.': 'All users will be permanently deleted along with their configs, OpenVPN certificates and WireGuard identities. This cannot be undone — take a backup first.',
    'زمانی که اعتبار این کاربر از آن شروع شده است': "The moment this user's validity started",
    # global actions
...
# ---------------------------------------------------------------------------
# v19.10.10: strict language/theme policy
