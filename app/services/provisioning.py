...
def _normalize_public_base_url(raw: str | None, default_scheme: str = 'https') -> str:
    ...
    Admins may enter only a domain/subdomain (sub.example.com), a full URL
    (https://sub.example.com), or host:port. The subscription layer needs a
    stable absolute URL so Telegram bots, QR codes and API responses all match.
    """
...
''')
    ...
    if protocol_enabled_for_user(user, 'hysteria2'):
        ...
        hy_uri = f'hysteria2://{safe_user}@{host}:{hy_port}/?sni={host}&insecure={hy_insecure}#{user.username}-IronPanel-Hysteria2'
