...
PROTOCOLS = ['openvpn', 'ocserv', 'l2tp', 'wireguard', 'xray', 'pptp', 'hysteria2', 'telegram_proxy']
...
def active_protocols():
    raw = get_setting('active_protocols', 'openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2,telegram_proxy')
...
def telegram_proxy_port_for(user: VpnUser) -> int:
    uid = int(getattr(user, 'id', 0) or 0)
    port = telegram_proxy_base_port() + uid
    if port > 65535:
        port = 20000 + (uid % 30000)
    return port
...
def telegram_proxy_link_for(user: VpnUser) -> str:
    ...
    port = telegram_proxy_port_for(user)
    secret = telegram_proxy_secret_for(user)
    return f'tg://proxy?server={quote(str(host), safe="")}&port={port}&secret={secret}'


def _telegram_proxy_service_name(user: VpnUser) -> str:
    return 'ironpanel-tgproxy-' + re.sub(r'[^A-Za-z0-9_.-]+', '-', user.username).strip('-_.')[:40]
...
def telegram_proxy_core_status():
    ...
    source = base / 'JSMTProxy'
    js = source / 'mtproxy.js'
    node = shutil.which('node') or shutil.which('nodejs') or ''
    p = run_cmd(['bash', '-lc', "systemctl list-units 'ironpanel-tgproxy-*.service' --all --no-legend --no-pager 2>/dev/null | awk '{print $1, $3, $4}'"])
    units = []
...
def telegram_proxy_user_rows(users=None):
    ...
    for u in list(users if users is not None else VpnUser.query.order_by(VpnUser.id.desc()).all()):
        enabled_for_user = ('telegram_proxy' in active_protocols()) and protocol_enabled_for_user(u, 'telegram_proxy')
        used = user_usage_summary(u) if u.id else {}
        svc = _telegram_proxy_service_name(u) + '.service'
...
def _write_telegram_proxy_instances(users=None, restart=True):
    """Provision per-user Telegram MTProto proxy instances using JSMTProxy."""
    # Respect the global Active Protocols setting. If disabled, stop all per-user
    # proxy units so admins can completely remove Telegram Proxy access.
    ...
    js_source = source / 'mtproxy.js'
    if not js_source.exists():
        _put_setting_raw('telegram_proxy_last_error', 'JSMTProxy source not installed. Run scripts/repair_telegram_proxy.sh')
        return False
    ...
    for u in users:
        if not (('telegram_proxy' in active_protocols()) and protocol_enabled_for_user(u, 'telegram_proxy')):
            continue
        safe = re.sub(r'[^A-Za-z0-9_.-]+', '_', u.username).strip('._-') or f'user{u.id}'
        inst = inst_root / safe
        inst.mkdir(parents=True, exist_ok=True)
        (inst / 'config.json').write_text('{ "port": %d, "secret":"%s" }\n' % (telegram_proxy_port_for(u), telegram_proxy_secret_for(u)))
        target_js = inst / 'mtproxy.js'
        if not target_js.exists():
            try:
                target_js.symlink_to(js_source)
            except Exception:
                shutil.copy2(js_source, target_js)
        svc = _telegram_proxy_service_name(u) + '.service'
        active_services.add(svc)
        service_path = Path('/etc/systemd/system') / svc
        service_text = """[Unit]
Description=IronPanel Telegram MTProto proxy for {username}
...
ExecStart=/usr/bin/node {inst}/mtproxy.js
Restart=always
RestartSec=3
LimitNOFILE=81920
...
""".format(username=u.username, inst=inst)
        service_path.write_text(service_text)
        port = telegram_proxy_port_for(u)
        run_cmd(['bash','-lc', f'ufw allow {port}/tcp >/dev/null 2>&1 || true; iptables -C INPUT -p tcp --dport {port} -m comment --comment ironpanel-tgproxy-{u.id} -j ACCEPT 2>/dev/null || iptables -I INPUT -p tcp --dport {port} -m comment --comment ironpanel-tgproxy-{u.id} -j ACCEPT || true; iptables -C OUTPUT -p tcp --sport {port} -m comment --comment ironpanel-tgproxy-{u.id} -j ACCEPT 2>/dev/null || iptables -I OUTPUT -p tcp --sport {port} -m comment --comment ironpanel-tgproxy-{u.id} -j ACCEPT || true'])
        if restart:
            run_cmd(['systemctl','enable','--now',svc])
        else:
            run_cmd(['systemctl','enable',svc])
    try:
        out = run_cmd(['bash','-lc', "systemctl list-unit-files 'ironpanel-tgproxy-*.service' --no-legend --no-pager | awk '{print $1}'"]).stdout.splitlines()
        for svc in out:
            if svc and svc not in active_services:
                run_cmd(['systemctl','disable','--now',svc])
                Path('/etc/systemd/system', svc).unlink(missing_ok=True)
...
def sync_user(user: VpnUser, restart=True):
    ...
    _write_telegram_proxy_instances(_valid_users(), restart=False)
...
''')
    ...
    if ('telegram_proxy' in active_protocols()) and protocol_enabled_for_user(user, 'telegram_proxy'):
        ...
        root.joinpath('telegram_proxy.txt').write_text(f'''Telegram MTProto Proxy
Server: {host}
Port: {telegram_proxy_port_for(user)}
Secret: {telegram_proxy_secret_for(user)}
Link: {tg_link}

Open this link on a device with Telegram installed:
{tg_link}
''')
    else:
...
def _collect_telegram_proxy_usage():
    """Collect per-user Telegram proxy traffic from iptables counters by port."""
    changed = 0
    for u in _valid_users():
        if not (('telegram_proxy' in active_protocols()) and protocol_enabled_for_user(u, 'telegram_proxy')):
            continue
        try:
            # iptables output columns vary between distros; the comment is stable.
            # Column 2 is byte counter in `iptables -nvx -L`. Keep INPUT and OUTPUT
            # separate so upload/download totals are still meaningful.
            raw = run_cmd(['bash','-lc', f"iptables -nvx -L INPUT 2>/dev/null | grep 'ironpanel-tgproxy-{u.id}' | awk '{{s+=$2}} END{{print s+0}}'; iptables -nvx -L OUTPUT 2>/dev/null | grep 'ironpanel-tgproxy-{u.id}' | awk '{{s+=$2}} END{{print s+0}}'"]).stdout.splitlines()
            vals = [int(x.strip() or 0) for x in raw[:2]]
            rx = vals[0] if len(vals) > 0 else 0
            tx = vals[1] if len(vals) > 1 else 0
