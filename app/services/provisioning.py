...
def _write_telegram_proxy_instances(users=None, restart=True):
    ...
    if not js_source.exists():
        run_cmd(['bash', '-lc', '/opt/ironpanel/scripts/repair_telegram_proxy.sh --install-only >/dev/null 2>&1 || true'], timeout=120)
    js_source = source / 'mtproxy.js'
    if not js_source.exists():
    ...
    active_services = set()
    for u in users:
        if not protocol_enabled_for_user(u, 'telegram_proxy'):
            continue
...
''')
    ...
    if protocol_enabled_for_user(user, 'telegram_proxy'):
...
def _collect_telegram_proxy_usage():
    ...
    for u in _valid_users():
        if not protocol_enabled_for_user(u, 'telegram_proxy'):
            continue
        try:
            raw = run_cmd(['bash','-lc', f"iptables -nvx -L INPUT 2>/dev/null | awk '$11==\"ironpanel-tgproxy-{u.id}\"{{print $2; exit}}'; iptables -nvx -L OUTPUT 2>/dev/null | awk '$11==\"ironpanel-tgproxy-{u.id}\"{{print $2; exit}}'"]).stdout.splitlines()
            vals = [int(x.strip()) for x in raw if x.strip().isdigit()]
            total = sum(vals)
            if _account_runtime_counter(u, 'telegram_proxy', total, 0):
                changed += 1
        except Exception:
            continue
    return changed
