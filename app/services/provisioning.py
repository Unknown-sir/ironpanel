...
''')
    ...
    if protocol_enabled_for_user(user, 'hysteria2'):
        ...
        hy_cert = get_setting('hysteria2_tls_cert_file', '')
        hy_secure = get_setting('ssl_enabled', '0') == '1' and bool(hy_cert) and Path(hy_cert).exists()
        hy_insecure = '0' if hy_secure else '1'
        hy_yaml_insecure = 'false' if hy_secure else 'true'
        hy_uri = f'hysteria2://{hy_pass}@{host}:{hy_port}/?sni={host}&insecure={hy_insecure}#{user.username}-IronPanel-Hysteria2'
...
tls:
  cert: {get_setting('hysteria2_tls_cert_file','/etc/hysteria/server.crt')}
  key: {get_setting('hysteria2_tls_key_file','/etc/hysteria/server.key')}
auth:
  type: command
