...
def automatic_disable_reason(user: VpnUser) -> str:
    """Return the automatic quota/expiry reason for a user, independent of manual enabled state.

    This is shared by the usage enforcer and the UI cleanup action so only users
    disabled because their time or traffic allowance is exhausted are selected.
    """
    if user.expired:
        return 'expired'
    limit = _traffic_limit_bytes(user)
...
def auto_disabled_cleanup_users(users):
    ...
    for u in rows:
        ...
        if reason and entry and entry.action == 'auto_disable_user':
            out.append(u)
    return out
...
''')
    ...
    if oc.parent.exists():
        ...
        oc.write_text(f'''# Managed by IronPanel 19.10.19
