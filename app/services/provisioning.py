...
''')
    ...
    if ('ssh' in active_protocols()) and protocol_enabled_for_user(user, 'ssh'):
        ...
        root.joinpath('ssh.txt').write_text(f"""SSH Tunnel / SSH Proxy
Server: {host}
Port: {ssh_port()}
...
Example:
ssh -p {ssh_port()} {ssh_account}@{host}

For tunnel clients such as HTTP Injector/NapsternetV, use the Server, Port, Username and Password above.
""")
    else:
