...
''')
    ...
    if protocol_enabled_for_user(user, 'hysteria2'):
        ...
        hy_uri = f'hysteria2://{hy_pass}@{host}:{hy_port}/?sni={host}&insecure=1#{user.username}-IronPanel-Hysteria2'
...
tls:
  cert: {get_setting('hysteria2_tls_cert_file','/etc/letsencrypt/live/YOUR_DOMAIN/fullchain.pem')}
  key: {get_setting('hysteria2_tls_key_file','/etc/letsencrypt/live/YOUR_DOMAIN/privkey.pem')}
auth:
  type: command
