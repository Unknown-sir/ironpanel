...
def _write_plain_passwords(root: Path):
    ...
    A failed/missing ``ocpasswd`` binary must never replace a working password
    database with an empty file. This was the root cause of valid Cisco users
    being rejected after repair/update runs.
    """
    users = _valid_users()
    ...
    oc_tmp = root / '.ocpasswd.ironpanel.tmp'
    oc_tmp.unlink(missing_ok=True)
    errors = []
    oc_users = [u for u in users if protocol_enabled_for_user(u, 'ocserv')]
    if shutil.which('ocpasswd'):
        for u in oc_users:
            username = clean_field(u.username, 'user').replace(':', '_')
            password = clean_field(u.cisco_password or u.l2tp_password)
            result = run_cmd(['ocpasswd', '-c', str(oc_tmp), username], input_text=f'{password}\n{password}\n', timeout=20)
            if result.returncode != 0:
                errors.append(f'{username}: {(result.stderr or result.stdout or "ocpasswd failed")[-300:]}')
                break
    elif oc_users:
        errors.append('ocpasswd binary is not installed')

    if not oc_users:
        oc_tmp.write_text('', encoding='utf-8')
    if not errors and oc_tmp.exists():
        oc_tmp.chmod(0o600)
        oc_tmp.replace(ocpasswd)
        ...
        # Preserve the previous known-good file instead of breaking every login.
        if not ocpasswd.exists():
            ocpasswd.touch(mode=0o600)
...
def sync_user(user: VpnUser, restart=True, changed_protocols=None, previous_protocols=None):
    ...
    # Keep node credentials/configs in lockstep with the main server. Previously
    # user edits only updated local files, leaving node ocpasswd/chap-secrets stale.
    try:
        from .node_gateway import queue_user_sync
        queue_user_sync(user)
        # Direct-location nodes may not be selected by gateway routing rules.
        # Queue their full protocol/user bundle as well so a new password or
        # permission change is immediately usable on every advertised node.
        _queue_full_node_runtime_sync(affected, reason='user-change')
...
''')
    ...
    if oc.parent.exists():
        hook_lines = ''
        if Path('/opt/ironpanel/scripts/ocserv_session_hook.sh').exists():
            hook_lines = 'connect-script = /opt/ironpanel/scripts/ocserv_session_hook.sh connect\ndisconnect-script = /opt/ironpanel/scripts/ocserv_session_hook.sh disconnect\n'
        oc.write_text(f'''# Managed by IronPanel 19.9.10
...
def _ocserv_runtime_diagnostics(item):
    ...
    auth_path = Path(auth_match.group(1)) if auth_match else current_app.config['CONFIG_ROOT'] / 'ocpasswd'
    try:
