...
from ..core.models import Admin, VpnUser, ServicePlan, WalletTransaction, Invoice, PaymentRecord, LoginHistory, TwoFactorSecret, RecoveryCode, TelegramCommandLog, Coupon
...
def apply_plan(user: VpnUser, plan: ServicePlan):
    ...
    if plan.protocols:
        selected = [p.strip().lower() for p in plan.protocols.split(',') if p.strip()]
        if user.owner_id:
            reseller = Admin.query.filter_by(id=user.owner_id, role='sub_admin').first()
            if reseller:
                allowed = set(reseller.allowed_protocol_list())
                selected = [p for p in selected if p in allowed]
        if not selected:
            raise ValueError('این پلن هیچ پروتکل مجازی برای این نماینده ندارد.')
        user.protocols = ','.join(selected)
        user.protocol_permissions = ','.join(selected)
    db.session.commit(); sync_user(user, changed_protocols=(user.allowed_protocol_list() or user.protocol_list() or []))
    return user
...
def handle_telegram_command(text, chat_id=''):
    ...
    if text == '/status':
        ...
        u=VpnUser.query.filter_by(username=parts[1]).first()
        if u:
            from .provisioning import reset_user_usage_preserving_reseller; reset_user_usage_preserving_reseller(u); db.session.commit(); sync_user(u, restart=False, changed_protocols=[]); result='Traffic reset'
        else: result='User not found'
