...
from .provisioning import sync_user, telegram_notify, user_access_status
...
def handle_telegram_command(text, chat_id=''):
    ...
    if text == '/status':
        ...
        if u:
            ...
            result=f'{u.username}: {reason} | used={u.used_total_mb}MB | limit={u.data_limit_mb or "unlimited"}'
        else: result='User not found'
    elif parts[:1] == ['/reset'] and len(parts) >= 2:
        u=VpnUser.query.filter_by(username=parts[1]).first()
        if u:
            u.used_upload_mb=0; u.used_download_mb=0; db.session.commit(); sync_user(u); result='Traffic reset'
        else: result='User not found'
