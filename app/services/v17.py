...
'''.strip()
    return "\n".join([
        ...
        f'$SUDO bash scripts/install_node.sh --master "$RESOLVED_MASTER" --token {token_q} --host {host_q} --protocols {protocols_q} --name {name_q} --direct-ports {direct_ports_json_q} --install-cores $([ "${{IRONPANEL_NODE_INSECURE_TLS:-0}}" = "1" ] && printf %s --insecure-tls)',
    ])
