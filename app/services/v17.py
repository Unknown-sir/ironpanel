...
def node_install_command(node):
    ...
    return "\n".join([
        'apt update && apt install -y git curl python3 python3-venv iproute2 iptables',
        f'git clone {repo} ironpanel-node || true',
        'cd ironpanel-node',
        f'sudo bash scripts/install_node.sh --master "{master}" --token "{node.api_key}" --host "{node.host}" --protocols "{protocols}" --name "{node.name}"',
    ])
