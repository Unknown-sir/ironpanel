...
def _node_master_url_candidates(request_base_url: str = '') -> list[str]:
    ...
    v19.9.13: auto-install previously handed the node a single URL. If the panel
...
def node_install_command(node, request_base_url=''):
    ...
    direct_ports_json_q = shlex.quote(json.dumps(direct_ports, ensure_ascii=False))
...
'''.strip()
    return "\n".join([
        ...
        f'$SUDO bash scripts/install_node.sh --master "$RESOLVED_MASTER" --token {token_q} --host {host_q} --protocols {protocols_q} --name {name_q} --direct-ports {direct_ports_json_q} --install-cores',
    ])
