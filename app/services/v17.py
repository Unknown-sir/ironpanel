...
def _node_master_url(request_base_url: str = '') -> str:
    ...
    if explicit_url:
        ...
        # A URL like https://1.2.3.4 with a non-443 panel port is almost always
        # wrong on non-SSL/IP installs. Keep HTTPS only when the admin specified a
        # custom port or the panel port is 443.
        if host_is_ip and not explicit_port and panel_port and panel_port != 443:
            scheme = 'http'
    else:
...
def node_install_command(node, request_base_url=''):
    ...
    return "\n".join([
        ...
        f'sudo bash scripts/install_node.sh --master {master_q} --token {token_q} --host {host_q} --protocols {protocols_q} --name {name_q}',
    ])
