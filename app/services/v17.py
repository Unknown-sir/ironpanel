...
def node_install_command(node, request_base_url=''):
    ...
    return "\n".join([
        ...
        '$SUDO apt-get update && $SUDO apt-get install -y curl ca-certificates python3 python3-venv iproute2 iptables tar',
        ...
        f'if ! curl -fsSL --retry 2 --connect-timeout 15 --max-time 180 -H {header_q} {package_url_q} -o "$NODE_TMP/node-runtime.tar.gz"; then echo "Validated TLS download failed; retrying for a self-signed panel certificate" >&2; curl -fksSL --retry 2 --connect-timeout 15 --max-time 180 -H {header_q} {package_url_q} -o "$NODE_TMP/node-runtime.tar.gz"; fi',
        'tar -xzf "$NODE_TMP/node-runtime.tar.gz" -C "$NODE_TMP"',
        'test -s "$NODE_TMP/scripts/install_node.sh" && test -s "$NODE_TMP/scripts/node_agent.py" && test -s "$NODE_TMP/scripts/install_node_cores.sh"',
        'cd "$NODE_TMP"',
