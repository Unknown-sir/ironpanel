...
CLIENT_FORMATS = ['auto', 'raw', 'clash', 'singbox', 'hiddify']
...
def node_install_command(node, request_base_url=''):
    ...
    # v19.10.10: keep node direct-port payload compact and transport it via base64
    ...
    try:
        from .direct_locations import node_ssl_domain
        ssl_domain_value = node_ssl_domain(node)
    except Exception:
        ssl_domain_value = getattr(node, 'ssl_domain', '') or getattr(node, 'config_domain', '') or ''
    ssl_domain_q = shlex.quote(ssl_domain_value or '')
...
'''.strip()
    ...
    return "\n".join([
        ...
        f'$SUDO bash scripts/install_node.sh --master "$RESOLVED_MASTER" --token {token_q} --host {host_q} --protocols {protocols_q} --name {name_q} --ssl-domain {ssl_domain_q} --direct-ports "$DIRECT_PORTS_JSON" --install-cores $([ "${{IRONPANEL_NODE_INSECURE_TLS:-0}}" = "1" ] && printf %s --insecure-tls)',
    ])
...
def _detect_subscription_client(request=None, explicit='auto'):
    requested = (explicit or 'auto').lower().strip()
    if requested and requested not in ('auto', 'smart'):
        return requested
    ua = ''
    try:
        ua = (request.headers.get('User-Agent') or request.headers.get('user-agent') or '').lower() if request else ''
    except Exception:
        ua = ''
    query_format = ''
    try:
        query_format = (request.args.get('format') or request.args.get('client') or '').lower().strip() if request else ''
    except Exception:
        query_format = ''
    if query_format in ('raw','base64','v2ray','v2rayn','v2rayng'):
        return 'raw'
    if query_format in ('clash','mihomo','openclash','meta'):
        return 'clash'
    if query_format in ('singbox','sing-box','sfa','sfm','sfi'):
        return 'singbox'
    if query_format == 'hiddify':
        return 'hiddify'
    if any(x in ua for x in ('clash','mihomo','openclash','stash')):
        return 'clash'
    if any(x in ua for x in ('sing-box','singbox','sfa','sfm','sfi')):
        return 'singbox'
    if 'hiddify' in ua:
        return 'hiddify'
    if any(x in ua for x in ('v2ray','v2rayn','v2rayng','nekobox','shadowrocket','fair')):
        return 'raw'
    return 'raw'
...
def subscription_for_client(user, client_type='raw', request=None):
    ...
    client_type=_detect_subscription_client(request=request, explicit=client_type)
...
def run_full_backup_v17():
    # Scheduled v17 backups now use the same migration-grade engine as the panel,
    # CLI and admin bot. Source code is intentionally omitted to keep recurring
    # backups small; all user/settings/protocol identity state is always included.
    from .maintenance import create_safe_backup
    return create_safe_backup(note='scheduled full migration backup', include_source=False)
