...
# ---------------- v18.6.6: inline step-by-step GitHub updater ----------------
...
def github_update_step_status():
    try:
        ...
        return {'running': False, 'step': 0, 'progress': 0, 'message': 'Ready', 'ok': True}

def github_update_step(step: int):
    if step <= 0:
        _STEP_DIR.mkdir(parents=True, exist_ok=True)
        _append_step_log('\n==== inline update started %s ====' % datetime.utcnow().isoformat(timespec='seconds'))
        _write_step_state(running=True, step=0, progress=3, message='Preparing inline GitHub update', ok=True)
...
""".replace('STAMP', stamp)
        ...
        _write_step_state(running=True, step=1, progress=25, message='Backups created' if ok else 'Backup step failed', ok=ok)
...
"""
        ...
        _write_step_state(running=True, step=2, progress=45, message='Latest source downloaded' if ok else 'Source download failed', ok=ok)
    ...
    if step == 3:
        cmd = """set -e
cd /tmp/ironpanel-inline-update/source
IRONPANEL_FULL_SERVICE_SYNC=1 IRONPANEL_DEFER_RESTART=1 bash upgrade.sh --github-full-sync
"""
        ok, out = _run_step_shell(cmd, timeout=2400)
        _write_step_state(running=True, step=3, progress=82, message='Files, dependencies and database upgraded' if ok else 'Upgrade step failed', ok=ok)
        return {'ok': ok, 'step': 4, 'next_step': 4, 'progress': 82, 'message': 'Files, dependencies and database upgraded' if ok else 'Upgrade step failed', 'log': out[-5000:], 'done': False}
    if step == 4:
        cmd = """set -e
systemctl daemon-reload || true
for unit in ironpanel ironpanel-sales-bot ironpanel-admin-bot ironpanel-usage-sync.timer ironpanel-job-worker.timer ironpanel-sales-reminders.timer ironpanel-admin-report.timer ironpanel-backup-v17.timer; do systemctl enable --now "$unit" >/dev/null 2>&1 || true; done
...
"""
        ...
        _write_step_state(running=False, step=4, progress=100, message=f'Upgrade completed. Installed version: {version}', ok=ok, done=True)
        return {'ok': ok, 'step': 5, 'next_step': 5, 'progress': 100, 'message': f'Upgrade completed. Installed version: {version}', 'log': out[-5000:], 'done': True, 'restart_required': True}
...
def github_update_schedule_restart():
    cmd = "(sleep 1; systemctl restart ironpanel >/dev/null 2>&1 || true; systemctl restart ironpanel-sales-bot >/dev/null 2>&1 || true; systemctl restart ironpanel-admin-bot >/dev/null 2>&1 || true) >/dev/null 2>&1 &"
    ok, out = _run_step_shell(cmd, timeout=5)
    return {'ok': ok, 'message': 'Panel restart scheduled. Refresh the page in a few seconds.', 'log': out[-1000:]}
