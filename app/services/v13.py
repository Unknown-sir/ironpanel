...
echo '[phase] running upgrade.sh without restarting panel during fetch polling'
IRONPANEL_FULL_SERVICE_SYNC=1 IRONPANEL_DEFER_RESTART=1 bash upgrade.sh --github-full-sync
rc=$?
if [ $rc -eq 0 ]; then echo 'IRONPANEL_UPDATE_COMPLETE'; fi
...
"""
    ...
    if step == 4:
        ...
        # The log is truncated at step 0, so completion tokens are from this run only.
        if 'IRONPANEL_UPDATE_COMPLETE' in tail_now or 'Safe update completed successfully' in tail_now or 'IronPanel GitHub updater finished' in tail_now or 'Upgrade completed. Installed version' in tail_now:
            version = current_version()
            _write_step_state(running=False, step=5, progress=100, message=f'Upgrade completed. Installed version: {version}', ok=True, done=True, upgrade_poll_count=elapsed)
        ...
        if exit_file.exists():
            ...
            # v19.1.0: do not leave the UI at 80/82%. Once the tracked upgrade task exits successfully,
            # report 100% immediately and schedule final repair/restart work through the restart endpoint.
            version = current_version()
            _write_step_state(running=False, step=5, progress=100, message=f'Upgrade completed. Installed version: {version}', ok=True, done=True, upgrade_poll_count=elapsed)
            return {'ok': True, 'step': 6, 'next_step': 6, 'progress': 100, 'message': f'Upgrade completed. Installed version: {version}', 'log': _tail_update_log(), 'done': True, 'restart_required': True}
        if _pid_running(pid) or running_file.exists():
            progress = min(96, 50 + elapsed * 3)
            _write_step_state(running=True, step=4, progress=progress, message='Upgrade is running; do not close this page', ok=True, done=False, upgrade_poll_count=elapsed)
            return {'ok': True, 'step': 4, 'next_step': 4, 'progress': progress, 'message': 'Upgrade is running; do not close this page', 'log': _tail_update_log(50), 'done': False, 'poll_delay_ms': 3000}
...
for unit in ironpanel ironpanel-sales-bot ironpanel-admin-bot ironpanel-usage-sync.timer ironpanel-license-heartbeat.timer ironpanel-job-worker.timer ironpanel-sales-reminders.timer ironpanel-admin-report.timer ironpanel-backup-v17.timer ironpanel-tgproxy.service; do systemctl enable --now "$unit" >/dev/null 2>&1 || true; done
if [ -x /opt/ironpanel/scripts/repair_telegram_proxy.sh ]; then /opt/ironpanel/scripts/repair_telegram_proxy.sh --sync >/dev/null 2>&1 || true; fi
if [ -x /opt/ironpanel/scripts/ironpanel_doctor.sh ]; then bash /opt/ironpanel/scripts/ironpanel_doctor.sh --repair >/dev/null 2>&1 || true; fi
cat /opt/ironpanel/VERSION 2>/dev/null || true
"""
        ok, out = _run_step_shell(cmd, timeout=1800)
        version = current_version()
...
def github_update_schedule_restart():
    cmd = "(sleep 1; systemctl daemon-reload >/dev/null 2>&1 || true; systemctl enable --now ironpanel-speed-limits.service >/dev/null 2>&1 || true; /opt/ironpanel/scripts/apply_speed_limits.sh --apply >/dev/null 2>&1 || true; systemctl restart ironpanel >/dev/null 2>&1 || true; systemctl restart ironpanel-sales-bot >/dev/null 2>&1 || true; systemctl restart ironpanel-admin-bot >/dev/null 2>&1 || true) >/dev/null 2>&1 &"
