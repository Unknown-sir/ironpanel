...
LOCAL_WORKER_ACTIONS = ('auto_ssh_install', 'restart_panel', 'restart_vpn', 'repair', 'backup', 'update')
...
def run_local_remote_action(action):
    ...
    command = cmds.get(action)
    if not command:
        return False, f'unsupported local worker action: {action}'
    p=run_cmd(['bash','-lc',command])
    return p.returncode==0, ((p.stdout or '')+(p.stderr or ''))[-5000:]
...
def process_local_jobs():
    # Only consume jobs that truly belong to the master server. Node-agent jobs
    # (ensure_protocols, sync_protocol_configs, sync_users_bulk, health_check,
    # sync_user, ...) must remain queued until the remote node heartbeat fetches
    # them. The old unfiltered worker silently marked those jobs done on master.
    jobs=(RemoteJob.query
          .filter(RemoteJob.status=='queued', RemoteJob.action.in_(LOCAL_WORKER_ACTIONS))
          .order_by(RemoteJob.id.asc())
          .limit(10).all())
    for j in jobs:
        if j.action == 'auto_ssh_install':
            try:
                from .node_auto_installer import run_auto_node_install_for_job
                run_auto_node_install_for_job(j)
            except Exception as exc:
                j.status='failed'; j.output=('auto ssh install job failed: '+str(exc))[-5000:]; db.session.commit()
            continue
...
def github_latest_version(force: bool = False):
    ...
    try:
        r = requests.get(url, timeout=1.2, headers={'User-Agent': 'IronPanel-Updater/19.10.18'})
...
def github_update_step(step: int):
    ...
    if step == 1:
        # Large /etc/ironpanel and source archives made normal updates appear hung.
        # Keep backup opt-in, consistent with safe_update.sh/update_from_github.sh.
        backup_enabled = str(os.environ.get('IRONPANEL_UPDATE_BACKUP') or get_setting('update_backup_enabled', '0')).lower() in ('1','true','yes','on')
        if not backup_enabled:
            message = 'Automatic backup skipped for faster update'
            _append_step_log(message + '; set IRONPANEL_UPDATE_BACKUP=1 to enable it')
            _write_step_state(running=True, step=2, progress=25, message=message, ok=True, done=False)
            return {'ok': True, 'step': 2, 'next_step': 2, 'progress': 25, 'message': message, 'log': message, 'done': False}
...
""".replace('STAMP', stamp)
        ...
        _write_step_state(running=True, step=2, progress=25, message='Optional backups created' if ok else 'Backup step failed', ok=ok, done=False)
        return {'ok': ok, 'step': 2, 'next_step': 2, 'progress': 25, 'message': 'Optional backups created' if ok else 'Backup step failed', 'log': out[-3000:], 'done': False}
    if step == 2:
