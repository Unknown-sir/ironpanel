...
def run_cmd(args, input_text=None, timeout=None):
    """Run an Xray command without allowing collector failures to crash sync.

    The previous build called this helper with ``timeout=`` from the usage collector,
    but the helper did not accept that keyword. Every Xray accounting cycle
    therefore failed before the Stats API command was executed.
    """
    try:
...
def generate_reality_keypair() -> tuple[str, str, str]:
    ...
    for bin_path in ('/usr/local/bin/xray', '/usr/bin/xray', 'xray'):
        ...
        for line in out.splitlines():
            ...
            if re.match(r'^(Private\s*key|PrivateKey)\s*:', normalized, re.I):
                priv = normalized.split(':', 1)[1].strip()
            if re.match(r'^(Public\s*key|PublicKey|Password\s*\(PublicKey\))\s*:', normalized, re.I):
                pub = normalized.split(':', 1)[1].strip()
...
def xray_link(user: VpnUser, settings: Dict[str, str] | None = None) -> str:
    ...
    remark = quote(str(user.username or 'user'))
    network = profile['transport']
    if network == 'tcp,udp':
        network = 'tcp'
    security = profile['security']
    ...
    if protocol == 'vmess':
        data = {
            'v': '2', 'ps': str(user.username or 'user'), 'add': host, 'port': str(port),
...
def _parse_xray_stats_output(output: str) -> Dict[str, int]:
    """Return ``stat name -> byte value`` for old and new Xray CLI output.

    Current Xray releases emit JSON where protobuf int64 values are quoted,
    for example ``"value": "2040"``. Older builds may emit an unquoted JSON
    number or text-protobuf lines. Supporting all three forms keeps upgrades
    compatible with the Xray binary already installed on the server.
    """
    ...
    # Xray can print a warning before the JSON object. Try every plausible JSON
    # start and accept the first object containing a stat array.
    ...
    for idx, char in enumerate(text):
        ...
        for row in rows:
            ...
            try:
                value = int(str(row.get('value', 0)).strip().strip('"'))
            except Exception:
                continue
            if name:
                stats[name] = max(0, value)
        if stats or (isinstance(payload, dict) and 'stat' in payload):
            return stats

    # Compatibility fallback for text-protobuf and partially formatted output.
    current = None
    for line in text.splitlines():
        name_match = re.search(r'(?:\bname\s*:|"name"\s*:)\s*"([^"]+)"', line)
        if name_match:
            current = name_match.group(1)
        value_match = re.search(r'(?:\bvalue\s*:|"value"\s*:)\s*"?(\d+)"?', line)
        if value_match and current:
            stats[current] = int(value_match.group(1))
            current = None

    # A compact object can contain name and value on one physical line.
    if not stats:
        for block in re.finditer(r'\{[^{}]{0,2000}\}', text, re.S):
            name_match = re.search(r'"?name"?\s*:\s*"([^"]+)"', block.group(0))
            value_match = re.search(r'"?value"?\s*:\s*"?(\d+)"?', block.group(0))
            if name_match and value_match:
                stats[name_match.group(1)] = int(value_match.group(1))
    return stats


def collect_xray_usage(account_counter) -> int:
    """Collect cumulative per-user Xray counters through the local Stats API."""
    settings = xray_settings()
    server = f"127.0.0.1:{settings.get('xray_api_port') or '10085'}"
    binary = _xray_bin()
    # Query all counters as documented by Project X and filter user counters in
    # Python. This avoids flag-compatibility differences between Xray releases.
    p = run_cmd([binary, 'api', 'statsquery', f'--server={server}'], timeout=8)
    output = (p.stdout or '') + ('\n' + p.stderr if p.stderr else '')
    if p.returncode != 0:
        detail = output.strip()[-1200:] or f'Xray statsquery exited with {p.returncode}'
        raise RuntimeError(detail)

    parsed = _parse_xray_stats_output(output)
    ...
    for name, value in parsed.items():
        ...
        if direction not in ('uplink', 'downlink'):
            continue
        totals.setdefault(email, {'uplink': 0, 'downlink': 0})[direction] = max(0, int(value))

    changed = 0
    users = {_safe_email(user): user for user in VpnUser.query.all()}
    for email, counters in totals.items():
        user = users.get(email)
        if user and account_counter(user, 'xray', counters['uplink'], counters['downlink']):
            changed += 1
    return changed
