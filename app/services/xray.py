...
def collect_xray_usage(account_counter) -> int:
    """Collect all Xray user counters with one Stats API query.

    Older builds executed two subprocesses for every user, which made a web
    refresh slow enough to time out on large installations.
    """
    settings = xray_settings()
    server = f"127.0.0.1:{settings.get('xray_api_port') or '10085'}"
    cmd = (
        f'(xray api statsquery --server={server} -pattern "user>>>" 2>/dev/null '
        f'|| /usr/local/bin/xray api statsquery --server={server} -pattern "user>>>" 2>/dev/null || true)'
    )
    p = run_cmd(['bash', '-lc', cmd], timeout=6)
    output = (p.stdout or '') + (p.stderr or '')
    totals = {}
    current = None
    for line in output.splitlines():
        match = re.search(r'name:\s*"([^"]+)"', line) or re.search(r'"name"\s*:\s*"([^"]+)"', line)
        if match:
            current = match.group(1)
            continue
        match = re.search(r'value:\s*(\d+)', line) or re.search(r'"value"\s*:\s*(\d+)', line)
        ...
        if len(parts) >= 4 and parts[0] == 'user':
            email = parts[1]
            direction = parts[-1]
            row = totals.setdefault(email, {'uplink': 0, 'downlink': 0})
            if direction in row:
                row[direction] = int(match.group(1))
        current = None
    changed = 0
