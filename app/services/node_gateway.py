...
def _bulk_user_payload(protocols, reason='auto-sync'):
    ...
    for u in VpnUser.query.all():
        ...
        for p in [x for x in allowed if x in selected]:
            ...
            try:
                from .xray import xray_user_uuid
                item['xray_uuid']=xray_user_uuid(u); item['xray_email']=f"ip-{u.id}-{u.username}"
            except Exception: pass
            if p == 'ocserv':
        ...
        for raw in raw_list:
            ...
            if base.is_file():
                ...
                try:
                    ...
                    pass
    return {'version':'19.10.13','protocols':selected,'main_server_hosts':gateway_main_server_identity(),'files':files,'total_bytes':total[0]}
