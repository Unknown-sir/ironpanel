...
def apply_node_gateway_runtime():
    ...
    if _setting('node_gateway_enabled','0')=='1':
        ...
        if p.returncode==0:
            msg='Node Gateway plan applied; real forwarding is active. The script now uses deterministic SNAT/MASQUERADE, FORWARD rules in both directions, node-port probes, and per-interface rp_filter disable. If the client app still shows the main-panel address, that is normal in gateway mode; verify real routing by rule counters and public IP after connect.' + (f' and {queued} config/core sync job(s) queued' if queued else '')
        else:
            log_tail=''
            try:
        ...
        for raw in raw_list:
            ...
            if base.is_file():
                ...
                try:
                    ...
                    pass
    return {'version':'19.8.20','protocols':selected,'main_server_hosts':gateway_main_server_identity(),'files':files,'total_bytes':total[0]}
