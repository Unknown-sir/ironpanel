...
def save_node_gateway_settings(form):
    ...
    for p in PROTOCOLS:
        ...
        rules[p]={'enabled':form.get(f'enabled_{p}')=='1' and mode!='local','mode':mode,'node_id':node_id,'strategy':form.get(f'strategy_{p}') or form.get('node_gateway_strategy') or 'balanced'}
