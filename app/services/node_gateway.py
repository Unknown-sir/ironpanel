...
def queue_full_node_sync(node_id, protocols=None, reason='manual', force=False):
    ...
    try:
        ...
        direct_ports = {}
        config_domain = ''
    queue_node_job(node.id, 'ensure_protocols', {'protocols': ','.join(selected), 'reason': reason})
...
CONFIG_PATHS = {
    ...
    # Copy only the compact ocserv password database for backward compatibility
    # with already-installed v19.9.9 Agents; never copy the whole panel config tree.
...
def protocol_config_bundle(protocols):
    ...
    for proto in selected:
        ...
        if proto == 'ocserv':
            # Respect non-default CONFIG_ROOT while keeping the default path for
            # old Agents. This file contains hashes only and is required so an
            # existing node can recover Cisco logins before its Agent is reinstalled.
        ...
        for raw in raw_list:
            ...
            if base.is_file():
                ...
                try:
                    ...
                    pass
    return {'version':'19.9.10','protocols':selected,'main_server_hosts':gateway_main_server_identity(),'files':files,'total_bytes':total[0]}
