...
def _bulk_user_payload(protocols, reason='auto-sync'):
    ...
    for u in VpnUser.query.filter_by(enabled=True).all():
        for p in [x for x in (u.protocols or '').split(',') if x in selected]:
            rows.append({'user_id':u.id,'username':u.username,'protocol':p,'enabled':bool(u.enabled),'data_limit_mb':int(u.data_limit_mb or 0),'expires_at':u.expires_at.isoformat() if u.expires_at else None,'reason':reason})
    return rows
