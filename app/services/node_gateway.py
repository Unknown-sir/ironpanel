...
def _bulk_user_payload(protocols, reason='auto-sync'):
    ...
    for u in VpnUser.query.filter_by(enabled=True).all():
        for p in [x for x in (u.protocols or '').split(',') if x in selected]:
            item={'user_id':u.id,'username':u.username,'protocol':p,'enabled':bool(u.enabled),'data_limit_mb':int(u.data_limit_mb or 0),'used_upload_bytes':int(u.used_upload_bytes or 0),'used_download_bytes':int(u.used_download_bytes or 0),'expires_at':u.expires_at.isoformat() if u.expires_at else None,'reason':reason}
        ...
        for raw in raw_list:
            ...
            if base.is_file():
                ...
                try:
                    ...
                    pass
    return {'version':'19.8.21','protocols':selected,'main_server_hosts':gateway_main_server_identity(),'files':files,'total_bytes':total[0]}
