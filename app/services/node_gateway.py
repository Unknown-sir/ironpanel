...
def queue_full_node_sync(node_id, protocols=None, reason='manual', force=False):
    ...
    config_domain = ''
    try:
        from .direct_locations import node_direct_port, node_direct_host
        direct_ports = {p: node_direct_port(node, p) for p in selected if node_direct_port(node, p)}
        config_domain = node_direct_host(node)
    except Exception:
        direct_ports = {}
        config_domain = ''
    queue_node_job(node.id, 'ensure_protocols', {'protocols': ','.join(selected), 'direct_ports': direct_ports, 'config_domain': config_domain, 'reason': reason})
    ...
    queue_node_job(node.id, 'sync_users_bulk', {'protocols': selected, 'users': users, 'direct_ports': direct_ports, 'config_domain': config_domain, 'reason': reason})
    queue_node_job(node.id, 'sync_protocol_configs', {'protocols': selected, 'bundle': bundle, 'direct_ports': direct_ports, 'config_domain': config_domain, 'reason': reason})
    try:
...
def queue_user_sync(user):
    ...
    for protocol in (user.allowed_protocol_list() or user.protocol_list()):
        ...
        if node:
            ...
            # Also queue a full config sync for protocols such as Xray where the
            # actual runtime user list lives inside the core configuration file.
            queue_full_node_sync(node.id, [protocol], reason='user-change', force=False)
    try:
...
def _auto_queue_node_sync_if_due(node):
    ...
    if not due:
        return 0
    return queue_full_node_sync(node.id, None, reason='heartbeat-auto-sync', force=False)
        ...
        for raw in raw_list:
            ...
            if base.is_file():
                ...
                try:
                    ...
                    pass
    return {'version':'19.9.15','protocols':selected,'main_server_hosts':gateway_main_server_identity(),'files':files,'total_bytes':total[0]}
