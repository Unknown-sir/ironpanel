...
def apply_node_gateway_runtime():
    ...
    if _setting('node_gateway_enabled','0')=='1':
        ...
        if p.returncode==0:
            msg='Node Gateway plan applied; real port forwarding is active. If app still displays the main-panel server address, verify by checking real public IP through the tunnel; DNAT keeps the client config target as the main panel while egress should leave from the node.' + (f' and {queued} config/core sync job(s) queued' if queued else '')
        else:
            log_tail=''
            try:
        ...
        for raw in raw_list:
            ...
            if base.is_file():
                ...
                try:
                    ...
                    pass
    return {'version':'19.8.19','protocols':selected,'main_server_hosts':gateway_main_server_identity(),'files':files,'total_bytes':total[0]}
...
def force_protocols_to_node(node_id, protocols):
    ...
    v19.8.18: direct PREROUTING DNAT plus node egress normalization; v19.8.17: filters internal Xray/API ports and normalizes synced node binds; v19.8.16: pipefail-safe real forward apply plus dynamic port detection; v19.8.14 fixed Internal Server Error from the web route, queues full
