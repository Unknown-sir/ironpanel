...
def queue_node_users_sync(node_id, protocols=None, reason='user-sync', force=False):
    ...
    v19.9.24 separates user synchronization from core/config synchronization so
...
def queue_user_sync(user):
    ...
    for protocol in (user.allowed_protocol_list() or user.protocol_list()):
        ...
        if node:
            ...
            # v19.9.24: User changes are hot-synced as metadata/auth only.
...
SAFE_PREFIXES = ('/etc/openvpn','/etc/wireguard','/etc/ocserv','/etc/xl2tpd','/etc/ppp','/etc/xray','/usr/local/etc/xray','/etc/hysteria','/etc/hysteria2','/etc/ironpanel','/etc/ssh/sshd_config.d','/etc/systemd/system/ironpanel-tgproxy.service','/opt/ironpanel/scripts','/opt/ironpanel-telegram-proxy/ironpanel')
        ...
        for raw in raw_list:
            ...
            if base.is_file():
                ...
                try:
                    ...
                    pass
    return {'version':'19.9.24','protocols':selected,'main_server_hosts':gateway_main_server_identity(),'files':files,'total_bytes':total[0]}
