...
def queue_full_node_sync(node_id, protocols=None, reason='manual', force=False):
    ...
    queue_node_job(node.id, 'sync_protocol_configs', {'protocols': selected, 'bundle': bundle, 'reason': reason})
    queue_node_job(node.id, 'sync_users_bulk', {'protocols': selected, 'users': users, 'reason': reason})
    try:
