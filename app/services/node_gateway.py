...
NODE_AGENT_ACTIONS = ('health_check','sync_user','sync_users_bulk','delete_user','ensure_protocols','sync_protocol_configs')
...
PORT_DEFAULTS = {'openvpn_udp':1194,'openvpn_tcp':1195,'wireguard_udp':51820,'ocserv_tcp':8445,'ocserv_udp':8445,'l2tp_udp':1701,'ipsec_ike':500,'ipsec_nat':4500,'xray_tcp':443,'pptp_tcp':1723,'hysteria2_udp':4433,'telegram_proxy_base':6969,'ssh_tcp':422}
...
def _port_value(key):
    """Return the real runtime port for a protocol setting.

    v19.8.15: Node Gateway must not use hard-coded defaults. Some pages store
    ports in dedicated protocol keys (xray_port, hysteria2_port) while the main
    settings page stores port_<name>. We check every legacy/current key and then
    fall back to config-file discovery.
    """
    aliases = {
        'panel': ['port_panel', 'panel_port', 'web_port'],
        'openvpn_udp': ['port_openvpn_udp', 'openvpn_udp_port'],
        'openvpn_tcp': ['port_openvpn_tcp', 'openvpn_tcp_port'],
        'ocserv_tcp': ['port_ocserv_tcp', 'ocserv_tcp_port', 'ocserv_port'],
        'ocserv_udp': ['port_ocserv_udp', 'ocserv_udp_port', 'ocserv_port'],
        'wireguard_udp': ['port_wireguard_udp', 'wireguard_port', 'wireguard_udp_port'],
        'l2tp_udp': ['port_l2tp_udp', 'l2tp_port'],
        'ipsec_ike': ['port_ipsec_ike', 'ipsec_ike_port'],
        'ipsec_nat': ['port_ipsec_nat', 'ipsec_nat_port'],
        'xray_tcp': ['port_xray_tcp', 'xray_port', 'xray_tcp_port'],
        'xray_api': ['port_xray_api', 'xray_api_port'],
        'pptp_tcp': ['port_pptp_tcp', 'pptp_port'],
        'hysteria2_udp': ['port_hysteria2_udp', 'hysteria2_port', 'hysteria2_udp_port', 'hy2_port'],
        'telegram_proxy_base': ['port_telegram_proxy_base', 'telegram_proxy_base_port', 'telegram_proxy_port', 'tgproxy_port', 'mtproto_port'],
        'ssh_tcp': ['port_ssh_tcp', 'ssh_port'],
    }
    val = _setting_any(aliases.get(key, [f'port_{key}']), '')
    if val not in (None, ''):
        port = _as_int(val, 0)
        if 0 < port <= 65535:
            return port
    try:
        return int(get_port(key) or PORT_DEFAULTS.get(key, 0) or 0)
    except Exception:
        return int(PORT_DEFAULTS.get(key, 0) or 0)

def _dedupe_entries(entries):
    seen=set(); out=[]
    for e in entries or []:
        proto=str(e.get('proto') or '').lower().strip()
        ...
        key=(proto, port)
        if key not in seen:
            seen.add(key); out.append({'proto': proto, 'port': port})
    return out

def _json_ports_from_file(path):
    ports=[]
    try:
...
def _xray_public_inbound_ports_from_file(path):
    """Return only public/client Xray inbound ports from config.json.

    Do not include Xray local API/stats ports such as 10085. Only top-level
    public inbounds are valid for gateway forwarding.
    """
    try:
        data=json.loads(Path(path).read_text(encoding='utf-8'))
    except Exception:
        return []
    internal_ports={10085,10086}
    try:
        api_port=_port_value('xray_api')
        if api_port: internal_ports.add(int(api_port))
    except Exception:
        pass
    ports=[]
    for ib in data.get('inbounds') or []:
        if not isinstance(ib, dict):
            continue
        tag=str(ib.get('tag') or '').lower()
        proto=str(ib.get('protocol') or '').lower()
        listen=str(ib.get('listen') or '').strip().lower()
        if 'api' in tag or 'stat' in tag or proto in ('api','dokodemo-door'):
            continue
        try: p=int(ib.get('port') or 0)
        except Exception: p=0
        if not (1 <= p <= 65535):
            continue
        if p in internal_ports:
            continue
        if listen in ('127.0.0.1','localhost','::1','[::1]') or listen.startswith('127.'):
            continue
        ports.append(p)
    return ports

def _xray_public_setting_ports():
    ports=set()
    p=_port_value('xray_tcp')
    if p and p not in (10085,10086):
        ports.add(p)
    try:
        from .xray import xray_settings, xray_builder_enabled, xray_builder_inbounds
        cfg=xray_settings()
        p=_as_int(cfg.get('xray_port'), 0)
        if p and p not in (10085,10086):
            ports.add(p)
        if xray_builder_enabled():
            for row in xray_builder_inbounds():
                if not row.get('enabled'):
                    continue
                tag=str(row.get('tag') or row.get('name') or '').lower()
                proto=str(row.get('protocol') or '').lower()
                if 'api' in tag or 'stat' in tag or proto in ('api','dokodemo-door'):
                    continue
                rp=_as_int(row.get('port'), 0)
                if rp and rp not in (10085,10086):
                    ports.add(rp)
    except Exception:
        pass
    return ports

def _ports_from_text_files(paths, patterns):
    ports=[]
    for raw in paths:
        p=Path(raw)
        ...
        for pat in patterns:
            for m in re.finditer(pat, text, re.I|re.M):
                try:
                    port=int(m.group(1))
                    if 0 < port <= 65535: ports.append(port)
                except Exception: pass
    return ports

def _xray_ports():
    # v19.8.17: public/client inbounds only. Do not forward Xray API/stats
    # ports such as 10085; those are local management listeners.
    ports=set(_xray_public_setting_ports())
    for path in ('/usr/local/etc/xray/config.json','/etc/xray/config.json','/etc/ironpanel/xray/config.json'):
        for p in _xray_public_inbound_ports_from_file(path):
            ports.add(p)
    ports={p for p in ports if p not in (10085,10086)}
    return sorted(ports) or [PORT_DEFAULTS['xray_tcp']]

def _hysteria2_ports():
    ports=set()
    p=_port_value('hysteria2_udp')
    if p: ports.add(p)
    ports.update(_ports_from_text_files(
        ['/etc/hysteria/config.yaml','/etc/hysteria2/config.yaml','/etc/hysteria/server.yaml','/etc/hysteria2/server.yaml'],
        [r'listen\s*:\s*(?:[0-9.\[\]:a-fA-F]*:)?([0-9]{2,5})', r':([0-9]{2,5})\s*(?:#.*)?$']
    ))
    return sorted(ports) or [PORT_DEFAULTS['hysteria2_udp']]

def _telegram_proxy_ports():
    ports=set()
    p=_port_value('telegram_proxy_base')
    if p: ports.add(p)
    for path in ('/etc/ironpanel/telegram_proxy.json','/etc/ironpanel/tgproxy.json'):
        for p in _json_ports_from_file(path):
            ports.add(p)
    ports.update(_ports_from_text_files(
        ['/etc/systemd/system/ironpanel-tgproxy.service','/etc/ironpanel/telegram_proxy.env','/etc/ironpanel/tgproxy.env'],
        [r'(?:--port|PORT|port)\D+([0-9]{2,5})']
    ))
    return sorted(ports) or [PORT_DEFAULTS['telegram_proxy_base']]

def protocol_port_entries(protocol):
    """Return every real port that must be forwarded for a protocol.

    v19.8.15: all rows in Node Gateway are calculated from the active IronPanel
    settings and protocol config files, not from fixed defaults. This fixes cases
    such as Xray on 444, Hysteria2 on 4433, or Telegram Proxy on 6967.
    """
    if protocol == 'openvpn':
        entries=[]
    ...
    if protocol == 'wireguard': return _dedupe_entries([{'proto':'udp','port':_port_value('wireguard_udp')}])
    if protocol == 'ocserv': return _dedupe_entries([{'proto':'tcp','port':_port_value('ocserv_tcp')},{'proto':'udp','port':_port_value('ocserv_udp')}])
    if protocol == 'l2tp': return _dedupe_entries([{'proto':'udp','port':_port_value('ipsec_ike')},{'proto':'udp','port':_port_value('ipsec_nat')},{'proto':'udp','port':_port_value('l2tp_udp')}])
    if protocol == 'xray': return _dedupe_entries([{'proto':'tcp','port':p} for p in _xray_ports()])
    if protocol == 'pptp': return _dedupe_entries([{'proto':'tcp','port':_port_value('pptp_tcp')},{'proto':'gre','port':0}])
    if protocol == 'hysteria2': return _dedupe_entries([{'proto':'udp','port':p} for p in _hysteria2_ports()])
    if protocol == 'telegram_proxy': return _dedupe_entries([{'proto':'tcp','port':p} for p in _telegram_proxy_ports()])
    if protocol == 'ssh': return _dedupe_entries([{'proto':'tcp','port':_port_value('ssh_tcp')}])
    return []
...
def apply_node_gateway_runtime():
    ...
    if _setting('node_gateway_enabled','0')=='1':
        ...
        if p.returncode==0:
            msg='Node Gateway plan applied; transparent relay is active. Client configs keep the main-panel IP/domain. Traffic path is Client/Iran -> Main panel -> local relay -> Node -> local relay -> Main panel -> Client/Iran. Verify with /var/log/ironpanel-node-gateway-relay.log and public IP after connect.' + (f' and {queued} config/core sync job(s) queued' if queued else '')
        else:
            log_tail=''
            try:
                log_tail=Path('/var/log/ironpanel-node-gateway.log').read_text(encoding='utf-8', errors='ignore')[-1200:]
            except Exception:
                pass
            msg='Node Gateway apply error: '+((out or log_tail or 'apply script exited without output; check /var/log/ironpanel-node-gateway.log')[-500:])
        return {'ok':p.returncode==0,'message':msg,'plan':plan,'log':(out or '')[-4000:]}
...
def node_gateway_status():
    # v19.8.15: refresh the runtime plan on every status view so the table never
    # shows stale default ports after admins change Xray/Hysteria2/Telegram ports.
    try:
        write_node_gateway_runtime()
    except Exception:
        pass
...
def _pending_node_jobs(node_id, actions=None):
    q=RemoteJob.query.filter(RemoteJob.node_id==node_id, RemoteJob.status.in_(['queued','running']))
    if actions:
        q=q.filter(RemoteJob.action.in_(list(actions)))
    try: return q.count()
    except Exception: return 0


def _protocols_for_node_auto_sync(node):
    protos=[]
    try:
        rules=_json_setting('node_gateway_rules', {})
        if isinstance(rules, dict):
            for proto, rule in rules.items():
                try: nid=int((rule or {}).get('node_id') or 0)
                except Exception: nid=0
                if proto in PROTOCOLS and (rule or {}).get('enabled') and nid == node.id and (rule or {}).get('mode') in ('fixed','fixed_only','auto'):
                    protos.append(proto)
    except Exception:
        pass
    if not protos:
        try:
            from .direct_locations import node_direct_enabled
            if node_direct_enabled(node):
                protos=[x.strip() for x in (node.protocols or '').split(',') if x.strip() in PROTOCOLS]
        except Exception:
            pass
    if not protos:
        protos=[x.strip() for x in (node.protocols or '').split(',') if x.strip() in PROTOCOLS]
...
def _bulk_user_payload(protocols, reason='auto-sync'):
    selected=set(protocols or [])
    rows=[]
    from .provisioning import user_access_status, _user_effective_used_bytes
    for u in VpnUser.query.all():
        allowed = u.allowed_protocol_list() or u.protocol_list()
        access_ok = bool(user_access_status(u)[0])
        for p in [x for x in allowed if x in selected]:
            item={'user_id':u.id,'username':u.username,'protocol':p,'enabled':access_ok,'data_limit_mb':int(u.data_limit_mb or 0),'used_upload_bytes':int(u.used_upload_bytes or 0),'used_download_bytes':int(u.used_download_bytes or 0),'quota_used_bytes':int(_user_effective_used_bytes(u) or 0),'expires_at':u.expires_at.isoformat() if u.expires_at else None,'reason':reason}
            if getattr(u, 'wg_public_key', None): item['wg_public_key']=u.wg_public_key
            try:
                from .xray import xray_user_uuid, _safe_email
                item['xray_uuid']=xray_user_uuid(u); item['xray_email']=_safe_email(u)
            except Exception: pass
            if p == 'ocserv':
                item['cisco_password'] = u.cisco_password or u.l2tp_password or 'managed-by-panel'
            if p in ('l2tp', 'pptp'):
                item['l2tp_password'] = u.l2tp_password or u.cisco_password or 'managed-by-panel'
            if p == 'hysteria2':
                try:
                    from .provisioning import hysteria2_password_for
                    item['hysteria2_password'] = hysteria2_password_for(u)
                except Exception: pass
            if p == 'ssh':
                try:
                    from .provisioning import _ssh_account_name, _ssh_password_for
                    item['ssh_account'] = _ssh_account_name(u)
                    item['ssh_password'] = _ssh_password_for(u)
                except Exception: pass
            rows.append(item)
    return rows


def queue_node_users_sync(node_id, protocols=None, reason='user-sync', force=False):
    """Queue only user/auth metadata for a node without touching protocol cores.

    v19.10.10 separates user synchronization from core/config synchronization so
    periodic user updates do not restart OpenVPN/Xray/Ocserv/Hysteria/Telegram
    proxy and do not cause short client disconnects.
    """
    node=Node.query.get(int(node_id or 0))
    if not node: return 0
    selected=[p for p in (protocols or _protocols_for_node_auto_sync(node)) if p in PROTOCOLS]
    if not selected: return 0
    if not force and _pending_node_jobs(node.id, ['sync_users_bulk']):
        return 0
    users=_bulk_user_payload(selected, reason=reason)
    direct_ports = {}
    config_domain = ''
    ssl_domain = ''
    try:
        from .direct_locations import node_direct_port, node_direct_host, node_ssl_domain
        direct_ports = {p: node_direct_port(node, p) for p in selected if node_direct_port(node, p)}
        config_domain = node_direct_host(node)
        ssl_domain = node_ssl_domain(node)
    except Exception:
        direct_ports = {}
        config_domain = ''
        ssl_domain = ''
    queue_node_job(node.id, 'sync_users_bulk', {'protocols': selected, 'users': users, 'direct_ports': direct_ports, 'config_domain': config_domain, 'ssl_domain': ssl_domain, 'reason': reason, 'no_core_restart': True})
    try:
...
def queue_full_node_sync(node_id, protocols=None, reason='manual', force=False):
    node=Node.query.get(int(node_id or 0))
    if not node: return 0
    selected=[p for p in (protocols or _protocols_for_node_auto_sync(node)) if p in PROTOCOLS]
    if not selected: return 0
    if not force and _pending_node_jobs(node.id, ['ensure_protocols','sync_protocol_configs','sync_users_bulk']):
        return 0
    ...
    try:
        ...
        config_domain = ''
        ssl_domain = ''
    queue_node_job(node.id, 'ensure_protocols', {'protocols': ','.join(selected), 'direct_ports': direct_ports, 'config_domain': config_domain, 'ssl_domain': ssl_domain, 'reason': reason})
    # User metadata/credentials must exist before protocol services restart,
    # especially for OpenVPN, Hysteria2 and SSH node-side authentication.
    queue_node_job(node.id, 'sync_users_bulk', {'protocols': selected, 'users': users, 'direct_ports': direct_ports, 'config_domain': config_domain, 'ssl_domain': ssl_domain, 'reason': reason})
    queue_node_job(node.id, 'sync_protocol_configs', {'protocols': selected, 'bundle': bundle, 'direct_ports': direct_ports, 'config_domain': config_domain, 'ssl_domain': ssl_domain, 'reason': reason})
    try:
        node.sync_status='queued'
        node.last_sync_at=datetime.utcnow()
        db.session.commit()
...
def queue_user_sync(user):
    queued=[]
    from .provisioning import user_access_status, _user_effective_used_bytes
    access_ok = bool(user_access_status(user)[0])
    for protocol in (user.allowed_protocol_list() or user.protocol_list()):
        node,reason=select_node_for_user(user, protocol)
        if node:
            payload={'user_id':user.id,'username':user.username,'protocol':protocol,'enabled':access_ok,'data_limit_mb':int(user.data_limit_mb or 0),'used_upload_bytes':int(user.used_upload_bytes or 0),'used_download_bytes':int(user.used_download_bytes or 0),'quota_used_bytes':int(_user_effective_used_bytes(user) or 0),'expires_at':user.expires_at.isoformat() if user.expires_at else None,'reason':reason}
            if protocol == 'xray':
                try:
                    from .xray import xray_user_uuid, _safe_email
                    payload['xray_uuid'] = xray_user_uuid(user)
                    payload['xray_email'] = _safe_email(user)
                except Exception:
                    pass
            if protocol == 'ocserv':
                payload['cisco_password'] = user.cisco_password or user.l2tp_password or 'managed-by-panel'
            if protocol in ('l2tp', 'pptp'):
                payload['l2tp_password'] = user.l2tp_password or user.cisco_password or 'managed-by-panel'
            if protocol == 'hysteria2':
                try:
                    from .provisioning import hysteria2_password_for
                    payload['hysteria2_password'] = hysteria2_password_for(user)
                except Exception: pass
            if protocol == 'ssh':
                try:
                    from .provisioning import _ssh_account_name, _ssh_password_for
                    payload['ssh_account'] = _ssh_account_name(user)
                    payload['ssh_password'] = _ssh_password_for(user)
                except Exception: pass
            queued.append(queue_node_job(node.id, 'sync_user', payload))
            # v19.10.10: User changes are hot-synced as metadata/auth only.
            # Do not queue a full protocol config sync here because it restarts
            # node cores and briefly disconnects online users. Full config/core
            # sync is reserved for Install/Repair and protocol/port/domain changes.
    try:
...
def _auto_queue_node_sync_if_due(node):
    if _setting('node_auto_sync_enabled','1') != '1':
        return 0
    try: interval=max(60, int(_setting('node_auto_sync_interval_sec','180') or 180))
    except Exception: interval=180
    now=datetime.utcnow()
    last=getattr(node, 'last_sync_at', None)
    due=(not last) or ((now-last).total_seconds() >= interval)
    if not due:
        return 0
    return queue_node_users_sync(node.id, None, reason='heartbeat-user-sync', force=False)
...
def heartbeat_jobs_for_node(node, limit=10):
    _auto_queue_node_sync_if_due(node)
    # Never expose master-only jobs such as auto_ssh_install to the remote agent.
    # The agent should fetch only actions it knows how to execute on the node.
    jobs=(RemoteJob.query
          .filter(RemoteJob.node_id==node.id,
                  RemoteJob.status.in_(['queued','running']),
                  RemoteJob.action.in_(NODE_AGENT_ACTIONS))
          .order_by(RemoteJob.id.asc()).limit(limit).all())
    out=[]
    for j in jobs:
        j.status='running'
...
def complete_node_job(token, job_id, ok=True, output='', metrics=None):
    ...
    try:
        ...
        apply_node_usage_reports(node, (metrics or {}).get('usage_reports') or [])
    except Exception:
        pass
    try:
        node.sync_status='ok' if ok else 'failed'
        if ok and job.action in ('sync_protocol_configs','sync_users_bulk','ensure_protocols'):
            node.last_sync_at=datetime.utcnow()
    except Exception:
        pass
...
CONFIG_PATHS = {
    ...
    # Copy canonical ocserv config/auth plus the compact legacy hash mirror for
    # older Agents; never copy the whole panel config tree.
    'ocserv':['/etc/ocserv','/etc/ironpanel/ssl','/etc/ironpanel/ocpasswd'],
    'l2tp':['/etc/xl2tpd','/etc/ipsec.conf','/etc/ipsec.secrets','/etc/ipsec.d','/etc/ppp','/etc/strongswan.conf','/etc/strongswan.d'],
    ...
    'telegram_proxy':['/etc/ironpanel/telegram_proxy.json','/etc/ironpanel/tgproxy.json','/etc/systemd/system/ironpanel-tgproxy.service','/opt/ironpanel-telegram-proxy/ironpanel'],
    'ssh':['/etc/ssh/sshd_config.d/ironpanel.conf'],
    # Never copy the whole /etc/ironpanel tree to a node: it can contain the live
    # database, environment secrets, license data, backups and stale auth files.
    # Protocol-specific SSL/config paths above plus runtime scripts are sufficient.
    '_common':['/opt/ironpanel/scripts'],
}
SAFE_PREFIXES = ('/etc/openvpn','/etc/wireguard','/etc/ocserv','/etc/xl2tpd','/etc/ppp','/etc/ipsec.d','/etc/xray','/usr/local/etc/xray','/etc/hysteria','/etc/hysteria2','/etc/ironpanel','/etc/ssh/sshd_config.d','/etc/systemd/system/ironpanel-tgproxy.service','/opt/ironpanel/scripts','/opt/ironpanel-telegram-proxy/ironpanel')
...
def _clean_endpoint_host(value):
    value = str(value or '').strip().rstrip('/')
    if not value:
        return ''
    value = value.replace('https://','').replace('http://','').split('/')[0]
    # Strip port from host:port but keep bare IPv6 reasonably safe by trimming brackets.
...
def gateway_forced_endpoint(protocol, preferred_port=None):
    """Compatibility shim.

    v19.8.22 uses transparent relay mode, so generated client configs must not
    switch to the node endpoint. Keep this function for old imports but always
    return None.
    """
    return None

def gateway_main_server_identity():
    """Return main-panel IP/domain hints used by node-side config normalization."""
    vals=[]
    for key in ('public_host','subscription_domain','panel_domain','ssl_domain','server_ip','public_ip'):
        try:
            v=str(_setting(key,'') or '').strip()
            if v: vals.append(v)
        except Exception:
            pass
    try:
        ...
        pass
    try:
        out=subprocess.check_output(['bash','-lc',"hostname -I 2>/dev/null; ip route get 1.1.1.1 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i==\"src\") print $(i+1)}'"], text=True, timeout=3)
    ...
    cleaned=[]
    for v in vals:
        v=v.replace('https://','').replace('http://','').split('/')[0].split(':')[0].strip()
...
def protocol_config_bundle(protocols):
    ...
    for proto in selected:
        ...
        if proto == 'ocserv':
            # Keep both canonical and legacy compact auth files in node bundles.
            # The service reads /etc/ocserv/ocpasswd; /etc/ironpanel/ocpasswd is
            # only a compatibility mirror for older Agents.
            raw_list.append('/etc/ocserv/ocpasswd')
            raw_list.append(str(current_app.config['CONFIG_ROOT'] / 'ocpasswd'))
        for raw in raw_list:
            base=Path(raw)
            ...
            if base.is_file():
                ...
                try:
                    ...
                    pass
    return {'version':'19.10.18','protocols':selected,'main_server_hosts':gateway_main_server_identity(),'files':files,'total_bytes':total[0]}

def queue_protocol_config_sync(node_id, protocols):
    return queue_full_node_sync(node_id, protocols, reason='protocol-config-sync', force=True)
...
def force_protocols_to_node(node_id, protocols):
    ...
    v19.8.20: deterministic SNAT/FORWARD return path and node port probes; v19.8.18: direct PREROUTING DNAT plus node egress normalization; v19.8.17: filters internal Xray/API ports and normalizes synced node binds; v19.8.16: pipefail-safe real forward apply plus dynamic port detection; v19.8.14 fixed Internal Server Error from the web route, queues full
    ...
    try:
        ...
        sync_jobs=queue_full_node_sync(node.id, selected, reason='force-protocols', force=True)
        ...
        for u in VpnUser.query.filter_by(enabled=True).all():
            ...
            for p in ups:
                payload={'user_id':u.id,'username':u.username,'protocol':p,'enabled':bool(u.enabled),'data_limit_mb':int(u.data_limit_mb or 0),'expires_at':u.expires_at.isoformat() if u.expires_at else None,'reason':'forced-protocol-route'}
                if p == 'ocserv':
                    payload['cisco_password'] = u.cisco_password or u.l2tp_password or 'managed-by-panel'
                if p in ('l2tp', 'pptp'):
                    payload['l2tp_password'] = u.l2tp_password or u.cisco_password or 'managed-by-panel'
                queue_node_job(node.id, 'sync_user', payload)
                queued += 1
        ok=bool(result.get('ok', True))
        msg=f'{len(selected)} protocol(s) forced to {node.name}; DNAT/FORWARD rules applied; {sync_jobs} full sync job(s) and {queued} user sync job(s) queued.'
        if warnings:
