...
def protocol_port_entries(protocol):
    ...
    v19.8.13: fixed-only node routing must be a real gateway. Older builds
...
SAFE_PREFIXES = ('/etc/openvpn','/etc/wireguard','/etc/ocserv','/etc/xl2tpd','/etc/ppp','/etc/xray','/usr/local/etc/xray','/etc/hysteria','/etc/hysteria2','/etc/ironpanel/ssl','/etc/ssh/sshd_config.d','/etc/systemd/system/ironpanel-tgproxy.service')
...
def protocol_config_bundle(protocols):
    ...
    for proto in selected:
        for raw in CONFIG_PATHS.get(proto, []):
            base=Path(raw)
            ...
            if base.is_file():
                ...
                try:
                    ...
                    pass
    return {'version':'19.8.13','protocols':selected,'files':files,'total_bytes':total[0]}
...
def force_protocols_to_node(node_id, protocols):
    ...
    for u in VpnUser.query.filter_by(enabled=True).all():
        ...
        for p in ups:
            ...
            queued += 1
    return {'ok':result.get('ok', True), 'message':f'{len(selected)} protocol(s) forced to {node.name}; forwarding applied; {sync_jobs} core/config job(s) and {queued} user sync job(s) queued', 'count':len(selected), 'queued':queued, 'config_jobs':sync_jobs}
