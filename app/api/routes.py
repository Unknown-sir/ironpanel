...
from ..services.provisioning import sync_user, delete_user, log, service_status, user_access_status
...
def create_user():
    ...
    u = VpnUser(username=username,
                ...
                protocols=','.join(data.get('protocols') or ['openvpn','ocserv','l2tp','wireguard']),
...
def serialize_user(u):
    ...
    return {'id':u.id,'username':u.username,'enabled':u.enabled,'access_ok':ok,'access_reason':reason,'protocols':u.protocols.split(','),'data_limit_mb':u.data_limit_mb,'used_total_mb':u.used_total_mb,'connection_limit':u.connection_limit,'expires_at':u.expires_at.isoformat() if u.expires_at else None,'subscription_url':f'/s/{u.subscription_token}'}
