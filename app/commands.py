...
DEFAULT_SETTINGS = {
    'public_host': None,
    'tunnel_host': '',
    'active_protocols': 'openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2',
    'port_panel': '8080',
    ...
    'license_features': '{"nodes": false, "sales_bot": false, "billing": false, "network": false, "updates": true, "xray": true, "outbound": true, "backup": true, "monitoring": true, "api": true, "subscription": true, "node_agent": false, "outbound_failover": true}',
    ...
    'hysteria2_tls_cert_file': '/etc/letsencrypt/live/YOUR_DOMAIN/fullchain.pem',
    'hysteria2_tls_key_file': '/etc/letsencrypt/live/YOUR_DOMAIN/privkey.pem',
    ...
    with engine.begin() as conn:
        ...
        if 'vpn_user' in tables:
            ...
            additions = {
                ...
                'protocols': "ALTER TABLE vpn_user ADD COLUMN protocols VARCHAR(120) DEFAULT 'openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2'",
                ...
                'protocol_permissions': "ALTER TABLE vpn_user ADD COLUMN protocol_permissions VARCHAR(120) DEFAULT 'openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2'",
            ...
            conn.exec_driver_sql("UPDATE vpn_user SET protocols='openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2' WHERE protocols IS NULL OR protocols=''")
            conn.exec_driver_sql("UPDATE vpn_user SET protocol_permissions='openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2' WHERE protocol_permissions IS NULL OR protocol_permissions=''")
            ...
            try:
                for extra in ['xray','pptp','hysteria2']:
...
def register_commands(app):
    ...
    def init_db(admin_user, admin_pass):
        ...
        defaults=[('openvpn',1194,'udp'),('ocserv',8445,'tcp'),('ocserv',8445,'udp'),('l2tp',1701,'udp'),('ipsec',500,'udp'),('ipsec',4500,'udp'),('wireguard',51820,'udp'),('xray',443,'tcp'),('xray-api',10085,'tcp'),('pptp',1723,'tcp'),('hysteria2',4433,'udp')]
    ...
    def upgrade_db():
        ...
        if not key_row or not (key_row.value or '').strip():
            free_features = {
                ...
                'node_agent': False, 'outbound_failover': True,
            }
            for key, value in {
                'license_status': 'free',
                'license_message': 'نسخه رایگان Beginner فعال است.',
