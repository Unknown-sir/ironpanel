    ...
    with engine.begin() as conn:
        ...
        if 'vpn_user' in tables:
            ...
            additions = {
                ...
                'protocols': "ALTER TABLE vpn_user ADD COLUMN protocols VARCHAR(120) DEFAULT 'openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2,telegram_proxy'",
                ...
                'protocol_permissions': "ALTER TABLE vpn_user ADD COLUMN protocol_permissions VARCHAR(120) DEFAULT 'openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2,telegram_proxy'",
            ...
            conn.exec_driver_sql("UPDATE vpn_user SET protocols='openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2,telegram_proxy' WHERE protocols IS NULL OR protocols=''")
            conn.exec_driver_sql("UPDATE vpn_user SET protocol_permissions='openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2,telegram_proxy' WHERE protocol_permissions IS NULL OR protocol_permissions=''")
            ...
            try:
                for extra in ['xray','pptp','hysteria2','telegram_proxy']:
...
def register_commands(app):
    ...
    def init_db(admin_user, admin_pass):
        ...
        defaults=[('openvpn',1194,'udp'),('ocserv',8445,'tcp'),('ocserv',8445,'udp'),('l2tp',1701,'udp'),('ipsec',500,'udp'),('ipsec',4500,'udp'),('wireguard',51820,'udp'),('xray',443,'tcp'),('xray-api',10085,'tcp'),('pptp',1723,'tcp'),('hysteria2',4433,'udp'),('telegram-proxy',6969,'tcp')]
