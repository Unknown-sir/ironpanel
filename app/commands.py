import click, secrets, json, re, time
from .core.extensions import db
from .core.models import Admin, VpnUser, Port, Node, AppSetting, DnsProfile
...
DEFAULT_SETTINGS = {
    ...
    'auto_password_length': '10',
    'auto_password_mode': 'both',
    'port_panel': '8080',
    ...
    # v19.5: default per-user/per-protocol speed limits in Mbps; 0 means unlimited.
    ...
    'speed_limits_apply_mode': 'per_user_protocol',
    'user_speed_limits_json': '{}',
    'node_gateway_enabled': '0',
    'node_gateway_strategy': 'balanced',
    'node_gateway_live_ping': '0',
    'node_failover_enabled': '1',
    'node_auto_sync_enabled': '1',
    'node_gateway_rules': '{}',
    'node_auto_installer_enabled': '1',
    'node_direct_subscription_enabled': '1',
    'node_usage_sync_interval_sec': '180',
...
def _sqlite_retry(label, fn, attempts=12, base_sleep=1.0):
    """Run SQLite-sensitive DB work with retry/backoff for temporary locks.

    IronPanel uses SQLite by default. During an upgrade, short-lived writers such
    as usage-sync, job-worker, sales/admin bots or an active Gunicorn worker may
    hold the DB for a moment. Without retry, migrations can fail with:
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            if not _is_sqlite_lock_error(exc) or attempt >= attempts:
                raise
            sleep_for = min(base_sleep * attempt, 8.0)
            click.echo(f'{label}: SQLite database is locked; retry {attempt}/{attempts} in {sleep_for:.0f}s ...', err=True)
    ...
    with engine.begin() as conn:
        ...
        if 'vpn_user' in tables:
            ...
            additions = {
                ...
                'disabled_reason': "ALTER TABLE vpn_user ADD COLUMN disabled_reason VARCHAR(48) DEFAULT ''",
                ...
                'node_mode': "ALTER TABLE vpn_user ADD COLUMN node_mode VARCHAR(20) DEFAULT 'auto'",
                'preferred_node_id': "ALTER TABLE vpn_user ADD COLUMN preferred_node_id INTEGER",
                'node_sync_status': "ALTER TABLE vpn_user ADD COLUMN node_sync_status VARCHAR(40) DEFAULT 'local'",
                'node_sync_error': "ALTER TABLE vpn_user ADD COLUMN node_sync_error TEXT DEFAULT ''",
            ...
            conn.exec_driver_sql("UPDATE vpn_user SET disabled_reason='' WHERE disabled_reason IS NULL")
            ...
            conn.exec_driver_sql("UPDATE vpn_user SET node_mode='auto' WHERE node_mode IS NULL OR node_mode='' ")
            conn.exec_driver_sql("UPDATE vpn_user SET node_sync_status='local' WHERE node_sync_status IS NULL OR node_sync_status='' ")
            conn.exec_driver_sql("UPDATE vpn_user SET node_sync_error='' WHERE node_sync_error IS NULL")
            conn.exec_driver_sql("UPDATE vpn_user SET speed_limits_json='' WHERE speed_limits_json IS NULL")
            conn.exec_driver_sql("UPDATE vpn_user SET speed_limit_mbps=0 WHERE speed_limit_mbps IS NULL OR speed_limit_mbps < 0")
        ...
        if 'node' in tables:
            ...
            for col, sql in {
                ...
                'ssl_domain': "ALTER TABLE node ADD COLUMN ssl_domain VARCHAR(255) DEFAULT ''",
        ...
        if 'remote_job' in tables:
            cols = {r[1] for r in conn.exec_driver_sql('PRAGMA table_info(remote_job)').fetchall()}
            if 'payload_json' not in cols:
                conn.exec_driver_sql("ALTER TABLE remote_job ADD COLUMN payload_json TEXT DEFAULT ''")
            if 'updated_at' not in cols:
                conn.exec_driver_sql("ALTER TABLE remote_job ADD COLUMN updated_at DATETIME")
        ...
        if 'admin' in tables:
            ...
            if 'reseller_protocols' not in cols:
                conn.exec_driver_sql("ALTER TABLE admin ADD COLUMN reseller_protocols VARCHAR(160) DEFAULT 'openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2,telegram_proxy,ssh'")
            ...
            if 'disabled_reason' not in cols:
                conn.exec_driver_sql("ALTER TABLE admin ADD COLUMN disabled_reason VARCHAR(40) DEFAULT ''")
            if 'reseller_used_bytes' not in cols:
                conn.exec_driver_sql("ALTER TABLE admin ADD COLUMN reseller_used_bytes BIGINT DEFAULT 0")
            conn.exec_driver_sql("UPDATE admin SET enabled=1 WHERE enabled IS NULL")
            conn.exec_driver_sql("UPDATE admin SET disabled_reason='' WHERE disabled_reason IS NULL")
            conn.exec_driver_sql("UPDATE admin SET reseller_protocols='openvpn,ocserv,l2tp,wireguard,xray,pptp,hysteria2,telegram_proxy,ssh' WHERE reseller_protocols IS NULL OR reseller_protocols=''")
            conn.exec_driver_sql("UPDATE admin SET reseller_used_bytes=0 WHERE reseller_used_bytes IS NULL")
            if 'created_at' not in cols:
                conn.exec_driver_sql("ALTER TABLE admin ADD COLUMN created_at DATETIME")

            # v19.4: owner-aware reseller sales bots and reseller traffic ledger.
            for table in ['sales_bot_customer', 'sales_bot_plan', 'sales_bot_order']:
                try:
                    cols2 = {r[1] for r in conn.exec_driver_sql(f'PRAGMA table_info({table})').fetchall()}
                    if cols2 and 'owner_id' not in cols2:
                        conn.exec_driver_sql(f'ALTER TABLE {table} ADD COLUMN owner_id INTEGER')
                except Exception:
                    pass
...
def _ensure_reseller_portals():
    ...
    for reseller in Admin.query.filter_by(role='sub_admin').all():
        ...
        if getattr(reseller, 'traffic_quota_gb', None) is None:
            ...
            changed = True
        if getattr(reseller, 'disabled_reason', None) is None:
            reseller.disabled_reason = ''
            changed = True
        if not bool(getattr(reseller, 'enabled', True)) and not str(getattr(reseller, 'disabled_reason', '') or ''):
            quota_bytes = int(getattr(reseller, 'traffic_quota_gb', 0) or 0) * 1024 * 1024 * 1024
            used_bytes = int(getattr(reseller, 'reseller_used_bytes', 0) or 0)
            user_count = VpnUser.query.filter_by(owner_id=reseller.id).count()
            user_limit = int(getattr(reseller, 'user_limit', 0) or 0)
            if quota_bytes and used_bytes >= quota_bytes:
                reseller.disabled_reason = 'traffic_quota'
            elif user_limit and user_count > user_limit:
                reseller.disabled_reason = 'user_limit'
            else:
...
def register_commands(app):
    ...
    def init_db(admin_user, admin_pass):
        def _do_init_db():
            ...
            db.session.commit()
        _sqlite_retry('init-db', _do_init_db)
    ...
    def upgrade_db():
        def _do_upgrade_db():
            ...
            _ensure_reseller_portals()
            db.session.commit()
        _sqlite_retry('upgrade-db', _do_upgrade_db)
        click.echo('Ironpanel database upgraded')
    @app.cli.command('license-heartbeat')
    def license_heartbeat_cmd():
        from .services.license import send_heartbeat
        ok = send_heartbeat(force=True)
        click.echo('IronPanel license heartbeat sent' if ok else 'IronPanel license heartbeat failed or skipped')


    @app.cli.command('reconcile-resellers')
    def reconcile_resellers_cmd():
        from .services.provisioning import reconcile_all_resellers
        results = reconcile_all_resellers(source='cli', sync_runtime=True)
        restored = sum(int((state or {}).get('restored') or 0) for _rid, state in results)
        suspended = sum(1 for _rid, state in results if not bool((state or {}).get('enabled', True)))
        click.echo(f'IronPanel reseller reconciliation complete. Resellers: {len(results)}. Suspended: {suspended}. Restored users: {restored}.')
    ...
    def sync_usage():
        ...
        try:
            ...
            if speed_limits_configured():
                speed_ok, speed_out = refresh_speed_limits_runtime_if_changed()
    ...
    @app.cli.command('apply-speed-limits')
    def apply_speed_limits_cmd():
        from .services.speed_limit import apply_speed_limits_runtime
        ok, out = apply_speed_limits_runtime()
        click.echo(out or ('IronPanel speed limits applied' if ok else 'IronPanel speed limit apply failed'))
        if not ok:
            raise click.ClickException('speed limit runtime apply failed')


    @app.cli.command('safe-backup')
    def safe_backup_cmd():
        from .services.maintenance import create_safe_backup
        p = create_safe_backup(note='manual cli migration backup', include_source=False)
        click.echo(str(p))

    @app.cli.command('restore-backup')
    @click.argument('backup_path', type=click.Path(exists=True, dir_okay=False))
    @click.option('--restore-source', is_flag=True, help='Also replace /opt/ironpanel source from the archive (normally not recommended).')
    def restore_backup_cmd(backup_path, restore_source):
        from pathlib import Path
        from .services.maintenance import restore_safe_backup
        ok, detail = restore_safe_backup(Path(backup_path), restore_source=restore_source)
        click.echo(detail)
        if not ok:
            raise click.ClickException('restore failed')

    @app.cli.command('doctor')
    @click.option('--repair', is_flag=True, help='Run best-effort repair before printing status.')
    def doctor_cmd(repair):
        from .services.maintenance import doctor_status, doctor_repair
        import json
        if repair:
            ok, out = doctor_repair('all')
            click.echo(out[-4000:])
        click.echo(json.dumps(doctor_status(), ensure_ascii=False, default=str, indent=2))
