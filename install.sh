...
SSH_PORT=${IRONPANEL_SSH_PORT:-422}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ssh-port) SSH_PORT="${2:-422}"; shift 2;;
...
backup_existing(){
  if [[ "${IRONPANEL_INSTALL_BACKUP:-0}" != "1" ]]; then
    info "Automatic reinstall backup skipped (set IRONPANEL_INSTALL_BACKUP=1 to enable)"
    return 0
  fi
...
# 4 gthread workers (v19.10.26): slow runtime probes no longer queue every panel request.
ExecStart=/bin/bash -lc 'CERT="\${IRONPANEL_SSL_CERT:-}"; KEY="\${IRONPANEL_SSL_KEY:-}"; SSL_ARGS=""; if [ -n "\$CERT" ] && [ -n "\$KEY" ] && [ -f "\$CERT" ] && [ -f "\$KEY" ]; then SSL_ARGS="--certfile \$CERT --keyfile \$KEY"; fi; exec $APP_DIR/.venv/bin/gunicorn -k gthread -w 4 --threads 2 -b 0.0.0.0:\${IRONPANEL_PORT} \$SSL_ARGS run:app'
Restart=always
RestartSec=2
TimeoutStopSec=20
KillMode=mixed
...
TIMER
  ...
  cat > /etc/systemd/system/ironpanel-license-heartbeat.service <<SERVICE
[Unit]
...
SERVICE
  ...
  cat > /etc/systemd/system/ironpanel-license-heartbeat.timer <<TIMER
[Unit]
...
if is_yes "$CUSTOM_PORTS"; then
  ...
  ask SSH_PORT "SSH tunnel TCP port" "$SSH_PORT"
...
for p in "$PANEL_PORT" "$OPENVPN_UDP" "$OPENVPN_TCP" "$OCSERV_PORT" "$WIREGUARD_PORT" "$XRAY_PORT" "$XRAY_API_PORT" "$PPTP_PORT" "$HYSTERIA2_PORT" "$TELEGRAM_PROXY_BASE" "$SSH_PORT"; do valid_port "$p" || { echo "Invalid port: $p"; exit 1; }; done
[[ "$WIREGUARD_MTU" =~ ^[0-9]+$ ]] || { echo "Invalid WireGuard MTU: $WIREGUARD_MTU"; exit 1; }

# Detect a reinstall before rsync/init-db changes the destination. Existing
# deployments get per-protocol health gating instead of the legacy all-core pass.
EXISTING_IRONPANEL_RUNTIME=0
if [[ -s "$ETC_DIR/ironpanel.db" && -d "$APP_DIR/app" ]]; then EXISTING_IRONPANEL_RUNTIME=1; fi
mkdir -p "$APP_DIR" "$ETC_DIR" "$ETC_DIR/profiles"
backup_existing

info "Installing base dependencies"
apt_install python3 python3-venv python3-pip nginx snapd openssl curl rsync sqlite3 qrencode iptables-persistent ca-certificates lsof net-tools iproute2 ufw unzip git jq gnupg software-properties-common
...
if [[ "$INSTALL_VPN_CORE" == "1" ]]; then
  if [[ "$EXISTING_IRONPANEL_RUNTIME" == "1" && -x "$APP_DIR/scripts/smart_protocol_repair.sh" ]]; then
    info "Existing IronPanel detected: checking protocol files/services first; healthy protocols are skipped."
    if ! env ETC_DIR="$ETC_DIR" APP_DIR="$APP_DIR" PUBLIC_HOST="$PUBLIC_HOST" IRONPANEL_PUBLIC_HOST="$PUBLIC_HOST" \
      WIREGUARD_MTU="$WIREGUARD_MTU" IRONPANEL_WIREGUARD_MTU="$WIREGUARD_MTU" \
      OPENVPN_PROTO="$OPENVPN_PROTO" OPENVPN_UDP="$OPENVPN_UDP" OPENVPN_TCP="$OPENVPN_TCP" \
      OCSERV_PORT="$OCSERV_PORT" OCSERV_TCP="$OCSERV_PORT" OCSERV_UDP="$OCSERV_PORT" \
      WIREGUARD_PORT="$WIREGUARD_PORT" XRAY_PORT="$XRAY_PORT" XRAY_API_PORT="$XRAY_API_PORT" \
      PPTP_PORT="$PPTP_PORT" HYSTERIA2_PORT="$HYSTERIA2_PORT" HYSTERIA2_UP="$HYSTERIA2_UP" HYSTERIA2_DOWN="$HYSTERIA2_DOWN" \
      TELEGRAM_PROXY_BASE="$TELEGRAM_PROXY_BASE" SSH_PORT="$SSH_PORT" IRONPANEL_SSH_PORT="$SSH_PORT" \
      bash "$APP_DIR/scripts/smart_protocol_repair.sh"; then
      warn "One or more active VPN cores still need attention. Install will continue; see /var/log/ironpanel-protocol-health.log or run Doctor."
    fi
  else
    info "Fresh install: installing VPN core prerequisites and initial protocol runtime."
    if ! env ETC_DIR="$ETC_DIR" PUBLIC_HOST="$PUBLIC_HOST" WIREGUARD_MTU="$WIREGUARD_MTU" IRONPANEL_WIREGUARD_MTU="$WIREGUARD_MTU" OPENVPN_PROTO="$OPENVPN_PROTO" OPENVPN_UDP="$OPENVPN_UDP" OPENVPN_TCP="$OPENVPN_TCP" OCSERV_PORT="$OCSERV_PORT" OCSERV_TCP="$OCSERV_PORT" OCSERV_UDP="$OCSERV_PORT" WIREGUARD_PORT="$WIREGUARD_PORT" XRAY_PORT="$XRAY_PORT" XRAY_API_PORT="$XRAY_API_PORT" PPTP_PORT="$PPTP_PORT" HYSTERIA2_PORT="$HYSTERIA2_PORT" HYSTERIA2_UP="$HYSTERIA2_UP" HYSTERIA2_DOWN="$HYSTERIA2_DOWN" TELEGRAM_PROXY_BASE="$TELEGRAM_PROXY_BASE" SSH_PORT="$SSH_PORT" IRONPANEL_SSH_PORT="$SSH_PORT" bash "$APP_DIR/scripts/install_vpn_core.sh"; then
...
# Resolve the supplied license once, then enforce the effective tier. Every core
# remains installed/repairable on Free so Plus/Pro activation is instant, but
# paid-only daemons are stopped until the license grants them.
if [[ -n "${LICENSE_KEY:-}" ]]; then
  "$APP_DIR/.venv/bin/python" - <<'PYLICENSE' || true
from app import create_app
from app.services.license import check_license
app=create_app()
with app.app_context():
    r=check_license(force=True)
    print('License:', r.get('license_type'), r.get('status'), 'valid=', r.get('valid'))
PYLICENSE
fi
bash "$APP_DIR/scripts/apply_speed_limits.sh" --install-service >/dev/null 2>&1 || true
: # v19.9.15 direct-location nodes do not use legacy node-gateway service
...
# Enforce the effective license after every fresh install. All protocol packages
# are prepared above, but Beginner/free must expose only OpenVPN + Xray. A paid
# key supplied during install unlocks/repairs the granted protocols immediately.
if [[ -x "$APP_DIR/scripts/reconcile_license_runtime.sh" ]]; then
  IRONPANEL_RECONCILE_SKIP_PREREQS=1 APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" bash "$APP_DIR/scripts/reconcile_license_runtime.sh" || warn "License runtime reconcile reported a warning; see /var/log/ironpanel-license-reconcile.log"
fi
# Reconcile already repairs only license-active broken protocols. Run one strict
# read-only validation afterwards; do not do a second repair pass during install.
if [[ "$INSTALL_VPN_CORE" == "1" && -x "$APP_DIR/scripts/smart_protocol_repair.sh" ]]; then
  info "Final health validation for license-active protocols"
  if ! APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" bash "$APP_DIR/scripts/smart_protocol_repair.sh" --check-only --strict | tee /var/log/ironpanel-protocol-health.log; then
    warn "One or more license-active protocol cores are incomplete. See /var/log/ironpanel-protocol-health.log"
    if [[ "${IRONPANEL_ALLOW_PARTIAL_CORES:-0}" != "1" ]]; then exit 30; fi
  fi
fi
