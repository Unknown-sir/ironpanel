...
if [[ "$INSTALL_VPN_CORE" == "1" ]]; then
  info "Running protocol-specific final repairs"
  bash "$APP_DIR/scripts/repair_openvpn.sh" || true
  bash "$APP_DIR/scripts/repair_wireguard.sh" || true
  env ETC_DIR="$ETC_DIR" PUBLIC_HOST="$PUBLIC_HOST" IRONPANEL_PUBLIC_HOST="$PUBLIC_HOST" bash "$APP_DIR/scripts/repair_ocserv.sh" || true
  env ETC_DIR="$ETC_DIR" PUBLIC_HOST="$PUBLIC_HOST" IRONPANEL_PUBLIC_HOST="$PUBLIC_HOST" bash "$APP_DIR/scripts/repair_l2tp.sh" || true
  bash "$APP_DIR/scripts/repair_pptp.sh" || true
  bash "$APP_DIR/scripts/repair_xray.sh" || true
  bash "$APP_DIR/scripts/repair_hysteria2.sh" || true
  bash "$APP_DIR/scripts/repair_telegram_proxy.sh" --sync || true
  bash "$APP_DIR/scripts/repair_ssh.sh" --sync || true
  info "Validating all protocol binaries, files, configs and services"
  if ! APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" bash "$APP_DIR/scripts/protocol_health_check.sh" --strict | tee /var/log/ironpanel-protocol-health.log; then
    warn "One or more protocol cores are incomplete. See /var/log/ironpanel-protocol-health.log and /var/log/ironpanel-protocol-prerequisites.log"
