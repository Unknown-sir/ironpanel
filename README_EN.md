...
- Reseller management is now fully visible in the dashboard and sidebar.
- Existing resellers automatically receive a portal path if they were created before this feature.
- Reseller quota editing, suspend/resume, and portal URL copy controls are shown in the main Resellers page.
- GitHub quick upgrade now runs a full system sync: source, Python dependencies, DB migrations, systemd unit rewrite, timers, VPN core repair, and user/service sync.
- README image updated.
- Subscription page redesigned with a modern visual layout, better mobile UI, clear usage cards, quick QR/download/copy actions, and improved accessibility.
...
IronPanel checks the `VERSION` file on GitHub and shows an update notice on the dashboard when a newer version exists. The main admin can click **Quick Upgrade** to run the updater in the background.

Manual updater command:
...
### v18.5.8 DB Migration Fix
اگر هنگام نصب/آپگرید با خطای `no such column: admin.enabled` روبه‌رو شدید، نسخه 18.5.8 migration سبک SQLite را قبل از query اولیه اجرا می‌کند و نصب را بدون حذف دیتابیس قبلی ادامه می‌دهد.

## v18.6.3 Highlights

- Fully detached GitHub Update Manager using a dedicated systemd transient unit so the upgrade is not killed when the panel restarts.
- Better mobile responsiveness across the admin panel: sidebar drawer, cards, forms, action buttons, tables, logs and settings pages.
- New Subscription v4 page with a more modern animated glass/aurora design and improved mobile import flow.

## v18.6.0 Highlights

- Installer questions are now English.
- WireGuard MTU defaults to 1360 with PersistentKeepalive 25.
- Full Light/Dark/Auto theme foundation and language selector.
- Local job worker timer, reseller API and improved audit logs.


### Telegram MTProto Proxy
IronPanel can deliver a separate Telegram MTProto proxy link for every user with its own port, secret and traffic quota enforcement. Enable `telegram_proxy` in Active Protocols and run `scripts/repair_telegram_proxy.sh --sync` if needed.


## v18.6.8

- README.md and README_FA.md now include the full Persian project guide.
- Update Manager, Telegram MTProto Proxy management and per-user Telegram proxy traffic accounting were improved.


## v18.6.9 — Shared Telegram Proxy

Telegram MTProto Proxy now runs as a single shared service on one TCP port (`ironpanel-tgproxy.service`). Each user receives a unique MTProto secret and IronPanel accounts usage per secret/user ID from `/opt/ironpanel-telegram-proxy/ironpanel/usage.json`, so Telegram Proxy traffic is included in the user's total quota without opening one port per user.


## v18.6.11 — Telegram Proxy Service Crash Fix

The shared `ironpanel-tgproxy.service` startup flow was hardened: config validation/rebuild, real NodeJS binary detection, old per-user unit cleanup, orphan process cleanup, wrapper syntax check, persistent `/var/log/ironpanel-tgproxy.log`, and safer socket exception handling.
