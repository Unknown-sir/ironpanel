...
## v19.10.18 — نصب پایدار پروتکل‌ها، ارتقای لایسنس و سیاست رمز

- نصب/Repair پروتکل‌ها اکنون وضعیت نیمه‌کاره `dpkg` را قبل از ادامه ترمیم می‌کند و سلامت باینری، کانفیگ، کلید/گواهی و سرویس را کنترل می‌کند.
- OpenVPN فقط از یک PKI مرجع استفاده می‌کند تا CA سرور و Certificate کاربران دوباره از دو CA متفاوت ساخته نشوند.
- WireGuard قوانین پایدار UDP/FORWARD/NAT را در `wg0.conf` نگه می‌دارد.
- در Beginner فقط OpenVPN و Xray قابل استفاده‌اند، اما پیش‌نیاز سایر پروتکل‌ها آماده می‌ماند. با ثبت Plus/Pro پروتکل‌های مجاز بدون نصب مجدد باز، Repair و Sync می‌شوند.
- در Settings می‌توان طول و نوع رمز خودکار کاربران را به‌صورت دائمی تعیین کرد؛ حداقل طول ۳ و حالت‌ها: فقط حروف، فقط اعداد، یا هر دو.
