...
IronPanel features are enabled based on the active license type. Xray and the base protocol stack can be enabled for all license types, while nodes, sales, billing and network modules are controlled by the license level.

| License Type | Capabilities |
|---|---|
| Beginer | Base modules, without nodes, sales, billing and network modules |
| Plus | More modules, without sales bot, billing and network modules |
| Pro | All modules except billing |
| Admin License | All modules |
| Trial | All modules for 7 days |
...
## Cisco / Ocserv v18.2 Notes
Cisco AnyConnect / OpenConnect is configured on port `8445` with a complete IronPanel-managed `ocserv.conf`: socket file, device, certificate paths, IPv4 pool, DNS, default route and NAT.

## Hysteria2 v18.2 Notes
Hysteria2 does not use Let's Encrypt by default in IronPanel v18.2. The panel creates a local self-signed certificate and generates user configs with `insecure: true`, so no public SSL/domain certificate is required.
