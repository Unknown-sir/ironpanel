## 🆕 v19.10.25 — بکاپ/ریستور کامل برای انتقال سرور

بکاپ Migration حالا دیتابیس اتمیک، همه کاربران/نماینده‌ها/تنظیمات/دامنه‌ها و Identity کامل پروتکل‌ها (OpenVPN PKI و tls-crypt، WireGuard، SSL، Ocserv، L2TP/IKEv2، Xray، Hysteria2 و...) را نگه می‌دارد. بعد از Restore روی سرور جدید، Tokenها و کلیدهای قبلی حفظ می‌شوند و Runtime با همان هویت‌ها بازسازی می‌شود.

## 🆕 v19.10.24 — رفع نصب ناقص OpenVPN روی سرور جدید

در نصب تازه از GitHub، OpenVPN حالا قبل از Start شدن تمام فایل‌های PKI (`ca.crt`, `server.crt`, `server.key`, `dh.pem`, `tls-crypt.key`) را از یک Easy-RSA PKI واحد می‌سازد/بازیابی و اعتبارسنجی می‌کند. اگر CA سالم از قبل وجود داشته باشد حفظ می‌شود و فقط اجزای ناقص ساخته می‌شوند. سرویس دیگر با PKI ناقص وارد Restart Loop نمی‌شود.

## 🆕 v19.10.23 — بازیابی هوشمند نماینده و حفظ مالکیت کاربران
- توقف خودکار نماینده هنگام اتمام حجم یا عبور تعداد اکانت‌ها از سقف، با ثبت دلیل پایدار.
- بعد از افزایش سقف حجم/اکانت یا کاهش تعداد کاربران، پنل نماینده خودکار فعال می‌شود و فقط کاربران سالمی که به علت محدودیت نماینده قطع شده‌اند دوباره وصل می‌شوند.
- کاربران منقضی، تمام‌حجم، IP-Limit یا دستی‌غیرفعال‌شده در بازیابی نماینده روشن نمی‌شوند.
- کاربران غیرفعال نماینده همچنان در پنل مدیر با نام مالک/نماینده و دلیل دقیق غیرفعال بودن نمایش داده می‌شوند.


## 🆕 v19.10.22 — MirzaBot API
- API مستقل `/api/mirzabot/v1` برای اتصال Custom Panel در MirzaBot.
- APIهای قبلی v1/v2 بدون تغییر باقی مانده‌اند.
- کلید اختصاصی `X-API-Key` و انتخاب پروتکل‌های پیش‌فرض از صفحه MirzaBot API.

<div align="center">

## 🆕 تغییرات v19.10.21

- کاربر ساخته‌شده توسط نماینده همان لحظه روی Runtime پروتکل‌ها Provision می‌شود و دیگر برای اتصال نیاز به «Sync همه کاربران» ندارد.
- پاکسازی کاربران منقضی یا تمام‌حجم به یک دکمه ساده تبدیل شد.


## v19.10.20 — Speed Limit اختصاصی هر کاربر

ادمین می‌تواند روی هر کاربر یک سقف سرعت مستقل بر حسب Mbps قرار دهد؛ نماینده فروش نیز همین کنترل را فقط برای کاربران خودش دارد. مقدار 0 یعنی محدودیت اختصاصی کاربر غیرفعال است و قواعد قبلی پروتکل استفاده می‌شوند.

## v19.10.19 — کنترل کاربر، پاکسازی هوشمند، Update سریع و Safari Fix

- قطع/وصل دستی کاربران با اعمال فوری روی Runtime؛ نماینده فقط روی کاربران خودش دسترسی دارد.
- حذف دسته‌ای امن کاربران Auto-disabled به علت پایان حجم/تاریخ.
- Install/Update پروتکل‌ها health-check-first است و هسته سالم را Skip می‌کند؛ Repairها timeout دارند و OpenVPN از بازسازی غیرضروری DH/PKI عبور می‌کند.
- Copy صفحه Subscription روی Safari آیفون fallback سازگار دارد.
- قابلیت‌های v19.10.18 شامل فعال‌سازی خودکار Plus/Pro و سیاست دائمی پسورد حفظ شده‌اند.
...
## 🧮 ثبت و نمایش مصرف واقعی (v19.10.14)

مصرف OpenVPN، WireGuard، Xray، Cisco/Ocserv، L2TP، PPTP، Hysteria2 و Telegram Proxy از runtime جمع‌آوری و در یک سهمیه مشترک کاربر ثبت می‌شود. در v19.10.14 مسیر V2Ray/Xray اصلاح شده است: اجرای Stats API دیگر به‌خاطر پارامتر timeout متوقف نمی‌شود و خروجی‌های جدید Xray با مقدار رشته‌ای، خروجی عددی قدیمی و text-protobuf همگی خوانده می‌شوند. صفحه کاربران، صفحه کانفیگ و لینک ساب قبل از نمایش، همگام‌سازی کوتاه و قفل‌شده انجام می‌دهند تا مقدار قدیمی دیتابیس نمایش داده نشود.

برای عیب‌یابی روی سرور اصلی اجرا کنید:
...
مصرف تاریخی‌ای که پیش از نصب این نسخه نه در دیتابیس و نه در شمارنده فعال هسته باقی مانده باشد قابل بازسازی نیست؛ مصرف جدید و شمارنده نشست‌های فعال/قطع‌شده بعد از ارتقا ثبت می‌شود.
...
آپدیت امن با health check و لاگ کامل (بکاپ خودکار برای سرعت بیشتر پیش‌فرض غیرفعال است):

```bash
sudo bash /opt/ironpanel/scripts/safe_update.sh
```

برای فعال‌کردن بکاپ اختیاری قبل از آپدیت:
...
## 🧩 محدودیت پروتکل نمایندگان (v19.10.17)

برای هر نماینده می‌توان پروتکل‌های مجاز را مشخص کرد. نماینده در ساخت سریع، عادی، عمده، ویرایش کاربر و ربات فروش فقط از بین همان پروتکل‌ها انتخاب می‌کند و محدودیت سمت سرور نیز کنترل می‌شود.
