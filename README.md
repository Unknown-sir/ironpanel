...
- ربات فروش مستقل برای هر نماینده (owner-aware)
...
- درخواست‌ها در پنل مدیر به‌صورت **کارت‌های قابل‌بازشدن** نمایش داده می‌شوند: عکس فیش، حجم درخواستی، مبلغ و نماینده قابل بررسی است. **«تأیید»** حجم را به سقف نماینده اضافه می‌کند و پنلِ متوقف‌شده‌شده را دوباره فعال می‌کند؛ **«رد»** بدون اضافه‌کردن حجم، درخواست را می‌بندد.
- اگر حجم یک نماینده تمام شود، پنل به‌صورت خودکار متوقف، پیام **«پنل غیر فعال به علت تمام شدن حجم»** نمایش داده می‌شود و نماینده فقط به بخش شارژ دسترسی دارد؛ ربات فروش او هم تا تأیید شارژ، ساخت/تمدید/شارژ سرویس را مسدود می‌کند.
