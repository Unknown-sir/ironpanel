#!/usr/bin/env bash
set -euo pipefail
# v19.10.25 GitHub upgrade: migrations, usage accounting repair, node sync, and optional full core reconciliation.
...
cd "$APP_DIR"
run_upgrade_db_safe
# v19.10.24: reconcile automatic reseller suspensions after schema/limit changes.
"$APP_DIR/.venv/bin/flask" --app run.py reconcile-resellers || true
...
if [[ "$SKIP_CORE_REPAIR" != "1" ]]; then
  echo "Smart protocol validation: healthy protocols are skipped; only broken active protocols are repaired"
  if [[ -x "$APP_DIR/scripts/smart_protocol_repair.sh" ]]; then
    env $PORT_EXPORTS APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" PUBLIC_HOST="${IRONPANEL_PUBLIC_HOST:-}" IRONPANEL_PUBLIC_HOST="${IRONPANEL_PUBLIC_HOST:-}" \
      bash "$APP_DIR/scripts/smart_protocol_repair.sh" || true
  fi
  if [[ -x "$APP_DIR/scripts/repair_cisco_auth.sh" ]]; then timeout 180 bash "$APP_DIR/scripts/repair_cisco_auth.sh" || true; fi
  if [[ -f "$APP_DIR/scripts/apply_speed_limits.sh" ]]; then bash "$APP_DIR/scripts/apply_speed_limits.sh" --install-service || true; fi
  : # v19.9.15 direct-location nodes do not use legacy node-gateway service
else
  echo "Fast update: skipped protocol repair; only runtime/auth files are refreshed later."
  if [[ -f "$APP_DIR/scripts/apply_speed_limits.sh" ]]; then bash "$APP_DIR/scripts/apply_speed_limits.sh" --install-service || true; fi
fi
...
[Unit]
Description=Run Ironpanel VPN Traffic Usage Sync frequently
...
if [[ "$SKIP_CORE_REPAIR" != "1" ]]; then
  if [[ "$FULL_SERVICE_SYNC" == "1" && -x "$APP_DIR/scripts/ironpanel_doctor.sh" ]]; then
    # Full-sync is explicit; still bound the doctor so an unsupported protocol cannot freeze GitHub update.
    timeout 420 bash "$APP_DIR/scripts/ironpanel_doctor.sh" --repair >/dev/null 2>&1 || true
  fi
fi

# Build credentials/peers/configs once, after all package/runtime changes. This
# replaces the old two full syncs and multiple duplicate protocol repair passes.
...
# Reconcile the saved edition/license before final validation. On an existing
# Plus/Pro key this also unlocks protocols that older Free installations left disabled.
if [[ -x "$APP_DIR/scripts/reconcile_license_runtime.sh" ]]; then
  IRONPANEL_RECONCILE_SKIP_PREREQS=1 APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" bash "$APP_DIR/scripts/reconcile_license_runtime.sh" || true
fi

if [[ "$SKIP_CORE_REPAIR" != "1" && -x "$APP_DIR/scripts/smart_protocol_repair.sh" ]]; then
  echo "Final health validation for license-active protocols"
  if ! APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" bash "$APP_DIR/scripts/smart_protocol_repair.sh" --check-only --strict | tee /var/log/ironpanel-protocol-health.log; then
    echo "ERROR: one or more active protocol cores are incomplete. See /var/log/ironpanel-protocol-health.log" >&2
