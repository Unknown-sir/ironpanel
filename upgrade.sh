...
import os, sqlite3
p=os.environ.get('DATABASE_URL','')
vals={'OPENVPN_UDP':'1194','OPENVPN_TCP':'1195','OPENVPN_PROTO':'udp','OCSERV_PORT':'8443','OCSERV_TCP':'8443','OCSERV_UDP':'8443','WIREGUARD_PORT':'51820','XRAY_PORT':'443','XRAY_API_PORT':'10085','OCSERV_TRANSPORT':'tcp_udp','WIREGUARD_TRANSPORT':'udp','L2TP_TRANSPORT':'udp'}
mapk={'OPENVPN_UDP':'port_openvpn_udp','OPENVPN_TCP':'port_openvpn_tcp','OPENVPN_PROTO':'openvpn_transport','OCSERV_PORT':'port_ocserv_tcp','OCSERV_TCP':'port_ocserv_tcp','OCSERV_UDP':'port_ocserv_udp','WIREGUARD_PORT':'port_wireguard_udp','XRAY_PORT':'port_xray_tcp','XRAY_API_PORT':'port_xray_api','OCSERV_TRANSPORT':'ocserv_transport','WIREGUARD_TRANSPORT':'wireguard_transport','L2TP_TRANSPORT':'l2tp_transport'}
if p.startswith('sqlite:///'):
    db=p.replace('sqlite:///','',1)
    try:
        con=sqlite3.connect(db); cur=con.cursor()
