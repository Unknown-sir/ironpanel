#!/usr/bin/env bash
set -euo pipefail
# v18.5.9 full GitHub upgrade: rewrite systemd units, run DB migrations, repair VPN cores, and reconcile timers/services.
...
if [[ -f "$APP_DIR/scripts/repair_telegram_proxy.sh" ]]; then bash "$APP_DIR/scripts/repair_telegram_proxy.sh" --install-only || true; fi
