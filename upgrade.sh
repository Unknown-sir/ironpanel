#!/usr/bin/env bash
set -euo pipefail
# v19.10.18 GitHub upgrade: migrations, usage accounting repair, node sync, and optional full core reconciliation.
...
if [[ -x "$APP_DIR/scripts/install_protocol_prerequisites.sh" ]]; then
  echo "Installing and validating protocol prerequisites"
  if ! APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" bash "$APP_DIR/scripts/install_protocol_prerequisites.sh" --install-all; then
    echo "WARNING: protocol prerequisite installation reported an error; final health validation will show the exact protocol" >&2
  fi
fi
...
if [[ "$SKIP_CORE_REPAIR" != "1" && -x "$APP_DIR/scripts/protocol_health_check.sh" ]]; then
  echo "Final protocol health validation"
  if ! APP_DIR="$APP_DIR" ETC_DIR="$ETC_DIR" bash "$APP_DIR/scripts/protocol_health_check.sh" --strict | tee /var/log/ironpanel-protocol-health.log; then
    echo "ERROR: one or more protocol cores are incomplete. See /var/log/ironpanel-protocol-health.log" >&2
    if [[ "${IRONPANEL_ALLOW_PARTIAL_CORES:-0}" != "1" ]]; then exit 30; fi
  fi
fi

# Final license/runtime reconciliation also fixes installations that already had a paid key.
